...
|:---|:---|:---|:---|:---|:---|
| **§0** | 決策摘要層 | 摘要 | 6-box 卡片 + thesis + PM 綠卡 | TL;DR / oneliner / PM Implication | 200-400 |
| **§1** | 產業白話定義 + 歷史脈絡 | 一 | 敘事 + 1 表 | 歷史 cycle 統計表 | 900-1,400 |
| **§2** | 技術成熟度 + S 曲線 | 一 | 敘事 + S 曲線 + 1 小表 | S-curve（強制）+ kingmaker 小表 | 600-900 |
| **§3** | 供給側：現在與未來 + 利潤池 | 二 | 敘事 + 2-3 表 | 玩家矩陣 / 利潤池遷移 / 成本曲線 | 1,200-1,800 |
| **§4** | 需求側：現在與未來 + 三角驗證 | 二 | 敘事 + 1-2 表 | TAM 三情境推導 / 需求三角對帳 | 1,000-1,500 |
| **§5** | 供需裁決 + 短中長期推估 + 投資時鐘 | 二 | 敘事 + 1-2 表 | 資本週期證據 / 三視野×三情境 / 庫存訂單指標 / phase 判定 | 1,200-1,800 |
| **§6** | 產業經濟學與估值傳導 | 三 | 敘事 + 1 表 | unit economics / ASP / multiple pass-through | 700-1,100 |
| **§7** | Non-Consensus + Priced-in + Kill Scenario | 三 | 敘事 / bullet | 3 分歧（含 priced-in）+ 3 steel-man | 900-1,400 |
| **§8** | Catalyst Timeline + 證偽表 | 三 | bullet + 1 表 | 5-8 catalyst 雙路徑 + 證偽表 | 700-1,100 |
| **§9** | 關聯個股 | 三 | 表格 + 敘事 | 🔴🟡🟢 表（≤16 行）+ non-obvious | 400-700 |

**全文可見字數 target：8,000–12,000 字**（深於 DS 的 4,000-6,500、短於舊 ID）。
...
掃讀的 PM 不讀 9 章，他讀 §0 就要拿到行動結論。本層四件事：

1. **TL;DR 6-box 卡片**：TAM / 5Y CAGR / 投資時鐘 Phase / **供需裁決（過剩/平衡/短缺三選一）** / conviction pill（high/mid/low）/ top picks（2-3 ticker）。
2. **一句話 thesis**（≤200 字，必帶 [I:] 或 [X:] tag）→ 同步寫進 id-meta `oneliner`。這是本 ID 最核心的非共識觀點。
3. **PM Implication 綠卡**（沿用舊 §0.7，**必填，缺即 Gate 9 阻斷**）：五 bullet + ①②③④ 四行動 + conviction pill。
4. **legacy cross-link callout**（若同主題已有 legacy ID / DS）：頂部加 callout 連回去，標明本份為 v2 敘事版。
