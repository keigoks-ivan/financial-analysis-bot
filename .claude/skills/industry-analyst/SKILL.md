...
~/.claude/skills/industry-analyst/
  ...
  templates/
    ...
    lean_template.md               # ★ canonical 精煉版（決策卡）template — 7 PARTS + dual-output 規則
...
1. **寫 HTML（dual-output 兩份）**：
   - `docs/id/ID_{Theme}_{YYYYMMDD}.html` = **canonical 精煉版決策卡**（`templates/lean_template.md` 的 7 PARTS；`<head>` **必含 id-meta JSON** + Google Fonts link）。
...
- **v2.3（2026-06-20，dual-output 雙軌 + 精煉版編輯風 + canonical=精煉版）**：① **dual-output** — 每跑一次同產兩份：**canonical `ID_{Theme}_{date}.html` = 精煉版決策卡**（帶 id-meta，索引/screener 認這份）+ `_full.html` = 完整考證版（9 章，無 id-meta、companion）。canonical 翻成精煉版（一點進來就是決策卡），完整版搬 `_full.html`，雙向連結。新 `templates/lean_template.md`。② **精煉版 7 PARTS**（決策摘要 / 歷史類比 / 供給 / 需求 / 裁決三情境 / 個股 tier；**砍掉舊草稿的「System Integration / Pure MA」那節** — 賣出訊號併入 PART I monitor）；「決策層完整、證據層濃縮」。③ **個股只輸特徵非推薦**：PART VI + epilogue 必含「特徵/分類、買賣由個股 DD + 系統決定、非買入推薦」。④ **視覺改精煉版編輯風**（取代 v2.2 海軍藍 IB）：暖紙 `#faf7f2` + 墨黑 `#1c1916` + 鏽金 `#7a3d1c`/`#b08840` + 功能語意色；Noto Serif TC/Crimson Pro 襯線 body + IBM Plex Mono 標籤（引 Google Fonts）；元件：masthead / stat-grid / abstract / toc / dsum / keycall / inference（含 deduction-chain + 可證偽）/ scenario-set（主觀機率）/ timeline / monitor / tier-list / keystone / epilogue / footnotes / colophon。**完整版與精煉版同皮**。⑤ Pilot：AIComputeCapexCycle 已套全套（canonical=精煉版 + `_full.html`）。
