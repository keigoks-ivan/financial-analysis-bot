...
- 結果寫入 dd-meta `endo_growth_ceiling`。**本節是 ROIIC／再投資率／內生成長上界的唯一推導處**（2026-09-10 規則精簡）：§6.D 與情境樹 sanity check 直接引用 `moat.roic_durability` 的數字，不重算、不另抄一組，只寫「與共識口徑差在哪」（營運成長 vs 每股盈餘成長、SBC、回購、margin、收購與負營運資金須分辨）。此式是成長合理性的參考上界，不是所有公司的硬天花板。
