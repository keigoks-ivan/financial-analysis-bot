# stock-analyst v15.0 — dd-meta-schema.md（條件載入 reference）
...
```json
{
  ...
  "catalysts": [{"date": "2026-09-10", "type": "product", "event": "Rubin CPX 出貨節點", "impact": "高", "watch": "若延後一季 → §10.5 Bull 機率下修"}],
...
- `dca_verdict`：`進場` | `觀望` | `迴避`（取自 §13 統一裁決，與頁首儀表板 + §13 裁決晶片三處一致）
- `dca_role`：`核心` | `衛星` | `追蹤` | `不持有`（v14.12 四值稅制，2026-07-10 持有人拍板「大道至簡」；取自 §13a，對齊 `aggregate_dca_stats.py` CATEGORY_ORDER）。**legacy 值**（`核心持倉`/`條件式核心持倉`/`衛星持倉`/`條件式衛星持倉`/`投機部位`/`不持有/迴避`/`候選/追蹤池`）僅為既有報告 dual-read 保留——新報告禁用，「條件式」語意一律改由 §13a 執行語承載
- `moat_trend`：`↑` | `→` | `↓`（**單一 Unicode 箭頭**，取自 §5 權威護城河趨勢;禁止寫「持平」「holding」等文字）
- `runway_post_y5`：`🟢` | `🟡` | `🔴`（取自 §6 Y5 後跑道）
- `ev5y_pct`：**number**（取自 §10.5 的 **5Y 累積機率加權期望值 %**，**不是**年化 IRR;下游 `compute_dca_irr` 會自行年化）

**v13 選填欄（present 時才驗型）**：
- `irr_base_pct`：number（§10.5/11.6 Base case 年化 IRR %）
- `max_dd_pct`：number（§12c Max DD 範圍取下界，負數，例 −45.0）
- `asym_ratio`：number（v14.3;§10.5 不對稱比 AR，1 位小數;Bear 5Y% ≥ 0 時省略此欄）
- `bull_5y_price` / `bear_5y_price`：number（v14.3 F4;§10.5 Bull/Bear 5Y 目標價，AR Live 掛單輸入）
- `p_bull_pct` / `p_bear_pct`：number（v14.3 F4;§10.5 Bull/Bear 機率 %，與 asym_ratio 同源）
- `catalysts`：list（2026-07-05 T-minus 鏈;§1/§14 已寫入的非財報 catalyst 日程，3-6 條）
...
- `endo_growth_ceiling`：number（§6.D 內生成長天花板 %，供 §10.6 IRR sanity check;既有欄，v14.5 補登記）
- `capalloc_grade`：str（§9 資本配置等級 `A`｜`B`｜`C`，供 §13 row 7b;既有欄，v14.5 補登記）
- `moat_execution` / `moat_pricing_power`：number 1-10（§5 二維護城河 sub-scores;既有欄，v14.5 補登記）
- `upside_5y_pct`：number（§10.5 5Y 目標價 upside %，QC-36 四處一致;既有欄，v14.5 補登記）
- `archetype`：str（v14.5;QC-43 判定的 primary archetype，7 類 enum 之一，格式照 QC-43 詞彙;三軌路由需要它——grp_route 現以 moat 分軌，分不出衛星結構 vs 衛星循環）
- `rearm_trigger`：str ≤120 字（v14.5;§13a 觀望分支「觸發條件：___」的機器可讀版——在等什麼：事件／估值門檻／回檔位。觀望裁決建議填，soft，不進 validator 必填）
...
- `industry_clock_phase`：str enum `I`｜`II`｜`III`｜`IV`（§3 產業時鐘，v14.12 全 archetype 必答;與 cycle_position 分工——那是循環股自身位置，本欄是產業層時鐘）
- `kill_metrics`：list（P2，2026-07-19;結構化證偽表——把 §13b 減碼／清倉條件 + §14 複審觸發抽成機器可讀條目，供市場偵測器 kill-watch 監控。**選填不阻斷**，但新 DD、**尤其裁決＝進場者，應填 3–5 條**。與 ID 的 `kill_metrics[]` 同結構;詳見下方「結構化證偽表」小節）
...
6. `dca_verdict` 與頁首儀表板 + §13 裁決晶片三處完全一致
7. `oneliner` ≤ 200 chars
8. `schema` = `v15.0`，且 HTML `<head>` 有 `<meta name="dd-schema-version" content="v15.0">`
...
**提醒**:yfinance 批量採集抓到的「下次財報日期」繼續**只進 §14，不進 dd-meta**（動態資料不落底稿）。
...
**何時填**:新 DD 都建議填;**裁決＝進場（`dca_verdict: "進場"`）者應填 3–5 條**——進場就等於押注 thesis，必須同時交出「什麼會證明我錯」。來源限本報告 §13b（加碼與減碼條件）、§14（複審觸發與保質期）、§10.5 Bear 情境已寫出的觸發點。
