...
§3.F市場邊界與利潤池(E3；**條件式**，見§0.5(二))：市場邊界與利益流向每份都判斷，寫進`reasoning.industry`。**展開量化表的觸發**(任一)：估值依賴低滲透／份額擴張／新品類／利潤池遷移；或有次要但高風險、高獲利、新成長的分部(不得只看營收占比)。展開＝每段給TAM(現/5Y)｜SAM｜滲透率｜段CAGR｜價值鏈位置｜OI池占比5年前→現｜流向＋一句「成長天花板與被替代路徑」；取不到完整池→「龍頭OI margin×環節營收」代理標「代理估算」。硬接線(展開時)：環節OI池占比5年淨流出≥5pp→§6.A Runway降一檔，QC-39三軸裁決不得標「結構性轉好」；淨流入→可作估值燈盲點1救援佐證。
...
2. 持續期四檢查點(需求基礎值/決策層級/價值鏈分配/社會容忍度，各給🟢🟡🔴＋sourced證據，與QC-39軸B共用不另搜)：判準與代理變數見`roic-durability.md`§二，本節不重述。
3. 再投資空間(**ROIIC／再投資率／內生上界的唯一推導處**)：內生成長率=增量ROIC×再投資率，寫`moat.roic_durability`的`roiic`／`reinvest_rate`／`endo_ceiling`。再投資率口徑`(Capex−D&A+ΔWC+收購淨額)÷NOPAT`；負CCC業務公式失效，改以ROIIC為上界。**§6.D與§9 sanity check引用本節數字、不重算**，只寫與共識的口徑差；此式是參考上界，非硬天花板。
...
D｜ROIC與FCF長期可維持性：ROIC(頂尖且>WACC)/ROIC−WACC超額(擴大)/FCF Margin(>15%)/FCF-NI轉換率(>85%)，各給當前｜3年前｜趨勢。**ROIIC／再投資率／內生天花板直引§5.R不重算**(ROIIC(3Y)=(NOPAT_t−NOPAT_t−3)÷(投入資本_t−投入資本_t−3))。本節只做一件新事：缺口=共識CAGR−天花板，歸因margin擴張/淨回購/無法歸因——**營運成長≠每股盈餘成長**，SBC、回購、margin、收購與負營運資金須分辨。無法歸因→加註「依賴re-rate」，長期持有信心上限「中」。
...
衰退信號亮燈數→兩個評級一次算完：價值陷阱風險0個🟢/1~2個🟡/3~4個🔴/5+個⛔(預設迴避，需明確反駁才改進場)；長期成長性🟢高確信(Runway≥10年、有機+定價驅動、強化型、ROIC擴大、信號0)/🟡中等(5~9年，1~2信號)/🔴存疑(<5年、無機高、消耗型或ROIC下滑、信號≥3)。

價值陷阱觸發(A1後只需一次)：估值偏低(PE或PEG)或衰退信號≥1→`trap_analysis.verdict`必給🟢/🟡/🔴，判斷依據寫進反證紀錄(§0.5(一))，並在該處回應對應的侵蝕信號，不另開四層問答。
...
I｜分部前瞻建模(E8；**條件式**，見§0.5(二))：完整分部地圖(各段營收占比＋驅動一句)每份都交。**展開三年量價模型的觸發**(任一)：重要分部驅動力不同／mix決定利潤／合併預估可能掩蓋某段衰退。展開＝>10%營收段各給FY0營收｜驅動式(量×價)｜FY+1E｜FY+2E｜FY+3E｜段OM軌跡｜對合併EPS貢獻%，加總對得上整體CAGR(差異>5pp須解釋)。未觸發→用合併驅動模型並填未展開標記，理由須說明「合併模型為何不會掩蓋分部風險」。
...
三年趨勢：FCF Margin/EBITDA Margin/ROIC−WACC/SBC/Revenue/ROE，各給2023｜2024｜2025(E)｜同業中位數｜相對評估。(QC-5：同業組同§10，至少3家)獲利品質警訊四項：淨利vs OCF連動性/AR成長vs營收成長/存貨成長vs營收成長/FCF轉換率趨勢。
...
D｜資本配置軌跡(**條件式**，見§0.5(二))：激勵、資本配置方向與本期重要交易每份都查。**展開十年全史(逐案M&A ROIIC＋回購/股息十年軌跡＋內部人與薪酬掛鉤)的觸發**(任一)：連續收購型公司／本期有重大新交易／成長有實質部分靠併購撐。未觸發→有機成長公司不必硬湊十年故事，填未展開標記；但**已知的重大失敗交易仍須納入，不得以年份截斷規避**。E｜FCF去向+M&A飛輪：①FCF去向拆解(股息/回購/去債/再投資/下一筆M&A各佔%+增量ROIC)②飛輪判定(成長多少來自飛輪vs有機)③一句結論(成長變股東現金/被飛輪回收/被capex吃掉)。

---

## 8｜§10估值與報酬(第二問；判斷面)

主尺＋一把真正獨立的交叉檢查(品質複利預設；archetype換尺見§1)。**每份必交(下游估值燈與screener直讀)**：Forward P/E(NTM)五年分位(QC-4公式=(當前−5Y低)/(5Y高−5Y低)×100%，整數位→`valuation.percentile_5y`)＋Forward PEG(Non-GAAP 3Y EPS CAGR，<1.0便宜/1~2合理/>2貴→`valuation.peg`)；分母窗口硬規則：CAGR基期含一次性效應→分母前瞻錨定(FY當年共識→FY+3外推)，禁用被污染trailing窗。共識EPS與guidance照留。

**條件式加尺**(見§0.5(二))：其餘倍數尺(Trailing P/E／EV/EBITDA／P/FCF／P/S)、GAAP與5年PEG、同業tier比較(`valuation.peers`)、賣方目標價(`valuation.targets.sell_side`)——**主尺與交叉檢查衝突、或結論靠re-rate撐**時才加。展開同業比較時先判業務模式tier(IP company/Turnkey ASIC/Foundry/SaaS訂閱型/寡占消費品牌)，禁跨tier高倍數當anchor，無同tier→標「⚠️無ideal peer group，溢折價需獨立推導」。目標價非每檔必要論證。3年vs5年PEG差距>0.5(有算才適用)須在成長熄火反映。
...
Pattern match(**條件式**)：有可信類比才寫(類似個股/setup/5年實現報酬含IRR/最像最不像處)。**「無可信類比」是合法輸出**，一句理由即可——不得為湊條目牽強比附，類比不是情境估值的必要證據。
...
3. 與上一份報告的交叉矛盾——每則含矛盾點/本次結論/上一份結論/可能原因(執行細則見3b)。
3b. 前份逐欄漂移歸因(`evidence.prior_dd.drift_watch`固定20欄；QC-49執行細則，非新判斷規則)——20欄**仍逐欄記新舊值，但文字按原因分組**：現價一次變動連帶改IRR／EV／AR／估值燈時開**一個**`contradictions[]`條目，`cause`三選一(`價格變動`／`新證據`／`方法變動`)，`prior_field`填該原因涵蓋的**全部欄名陣列**(單欄仍可填字串)，各欄本次值／前份值在條目內逐欄列清。**每個漂移欄都必須映射到某一個原因條目**(validate以`prior_field`對帳，漏一欄＝FAIL)；「更新數據」不算歸因，方法驅動須明標。裁決(`dca_verdict`)、核心假設與情境方法的改變仍各自一條實質解釋，不得併進價格那條。
4. ⚖強制裁決(每個「不可調和」矛盾必填)——矛盾/我選哪邊/依據(不能是「直覺」「平衡考慮」)/會settle此衝突的硬數據點/執行路徑。執行路徑：至少一條if-then＋一條反向條件(證據往反方向走時做什麼)，動作具體(升級小倉測試/減持/加碼至X%/清倉)，禁「再評估」「持續觀察」；**期限依該矛盾實際可觀測的時間窗給，不硬配2Y／5Y／10Y三段**。能settle的數據今天不存在→「不可裁決至{時點}」是合法輸出(手冊條目6)。

4b裁決推理品質三檢：①分母爭議檢查——「估值便宜」的分母(forward EPS/FY+2 FCF/轉正年估計)是否即本章爭點？是→該便宜論證無效，設`decision_inputs.val_denominator_disputed=true`。②證據權重三級制：L1已實現事實>L2 sourced前瞻估計>L3敘事，裁決預設站L1較高側，以L2/L3反駁L1須明寫理由。③Steelman義務：裁決為觀望/迴避→「現在就買的最強論證」；裁決為進場→「現在就賣的最強論證」。**論證本身寫進§0.5(一)的反證紀錄**(視角＝論點失敗或價格已反映太多)，本節只引用該條目並逐點回應，不另寫一段散文。回應只覆述原立場=裁決不成立，重寫。
...
## 13｜§12 反證紀錄與Max DD(`premortem`)

1. **反證紀錄**(形狀見§0.5(一))三視角各至少一條：①`論點失敗`＝5年後這部位虧50%最可能的故事；②`論點成功但股東經濟變差`＝thesis完全兌現但以___形態兌現，估值框架從___切到___，5年報酬變成___；③`價格已反映太多`。
2. 與Single Thing對帳(承接①)：✅直接撞上→不動；⚠部分重疊→回補secondary trigger；❌完全獨立→回Single Thing重寫/新增primary。⚠/❌卻未改動Single Thing→自我打回重做。②成立且機率不可忽略→反映進情境樹Bull終端倍數假設。
3. Max DD路徑壓力測試(必填)：Max DD範圍−__%~−__%(禁單點)/路徑風險🟢0～−30%/🟡−30～−50%/🔴<−50%。**觸發時點與恢復年數改選填**(`trigger_time`)——有依據才寫，否則留空或標「未知」，**不得為填欄位生成精確時間**；不會恢復則明寫「thesis已破」。Guardrail：範圍寬度≥10%p，<10%p=假精準打回。🔴且thesis脆弱(moat_trend↓或runway_post_y5🔴或估值依賴型)→倉位上限下修(例6%→≤3%)+持有年限警示；🔴但thesis完整→不因波動砍倉，註記「深回撤心理準備」+警示。`premortem.max_dd.lo`取範圍下界(負數)，dd-meta `max_dd_pct`直讀，**永遠必填**。
...
## 17｜QC-53情境判斷手冊(觸發式問題字典)

`references/judgment-playbook.md`是**問題字典，不是作業簿**。Part II動筆前掃「觸發索引」，命中條目當提問檢查自己的判斷——**核心研究(§0–§14)已涵蓋的引用該處結論，不逐條另寫答案**；只有手冊問到而核心研究沒答到的才補寫進`contradictions`/`premortem`/`decision_out`/`triggers`。未命中不作答。手冊編號不進輸出面。(QC-53)

## 18｜QC-13自我攻擊裁決(定稿前)

`decision_out`確定後、寫檔前跑inner monologue：「要推翻此裁決，最強3個反駁點是什麼。」觸及核心論據者→檢查對應模組、**併進§0.5(一)的反證紀錄**(新增條目或補既有條目`ruling`)、反駁成立則修正終判。禁止只列結論不做反駁測試，也**不另開一份自我攻擊清單**。(QC-13)

trap定性(`trap_analysis`)：本欄**只交`verdict`**🟢非陷阱/🟡觀察期/🔴高風險陷阱(餵`decision_inputs.trap`，同源同值)＋一句`label`。判斷依據(陷阱模式、正反最強論據須引具體財務數字、空頭最強一擊＝18個月內造成30%+虧損的最可能路徑＋監測指標、持有期間怎麼判斷陷阱正在發生)**一律寫在反證紀錄、於`reasoning.trap_analysis`引用，不在本欄重寫**。

## 19｜QC-33推導可追溯(`reasoning` 每模組必填)

任何承重結論數字(PE/PEG/訊號燈/品質等級/目標價/漂移判定/IRR/Max DD/moat分數/runway燈)須附推導：`輸入數字→計算過程→對下游implication`。禁止光寫結論不寫過程。`judgment.reasoning`每個模組key必填，**無行數地板也無三行上限**——一行講得完就一行，口徑複雜就寫清楚。(QC-33)
