...
> **這份檔怎麼來的**：由 `python3 scripts/dd_rules.py build-v19` 從唯一人工維護的規則來源 `.claude/skills/stock-analyst/references/v16/judgment-rules.md` 抽出（段落標記 `<!-- only:v18 -->`／`<!-- only:v19 -->`），**內容一字未改寫**。要改規則請改來源檔再重跑 build-v19；直接改本檔會在下一次組判斷包時被版本戳比對擋下。
>
