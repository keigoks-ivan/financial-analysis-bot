...
> 2026-07-08 自 18 份 v14.x DD 決策層反萃取的強模型判斷動作（Fable 5 蒸餾，四個挖掘 agent＋去重）。**用法（QC-53，2026-09-10 改制）**：本檔是**問題字典，不是作業簿**——Part II 動筆前掃「觸發索引」，命中條目當提問來檢查自己的判斷；**核心研究（judgment-rules §0–§14）已涵蓋的就引用該處結論，不再逐條另寫一段答案**，只有手冊問到而核心研究沒答到的才補寫進對應欄位。未命中不作答、不佔篇幅。每條格式＝觸發｜必答｜原型。條目個別可審計：2026-10 校準時逐條統計「觸發後是否改變過任何裁決/倉位/觸發器」，純儀式條目降選做或刪（rule_ledger）。
