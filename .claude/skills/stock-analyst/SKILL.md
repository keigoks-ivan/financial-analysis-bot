...
version: v14.10
released: 2026-07-08
description: "收到股票 ticker 後，產出單一 v14.10 DD 報告（docs/dd/DD_{TICKER}_{YYYYMMDD}.html）— 基本面深度 Part I（骨幹 ≥60%）＋投資決策層 Part II（§12 矛盾裁決 / §13 pre-mortem+Max DD / §14 統一裁決 / §15 複審），收斂為一個人面對的統一裁決：進場 / 觀望 / 迴避（+ 倉位角色）。A+/A/B/C/X 降為 metadata-only 基本面評級（餵 screener）；短期擇時降為附錄純資訊不主導裁決。dd-meta schema=v14.10（22 個 v12 必填 + 5 個 v13 必填 + 20 個選填欄，validator 強制），下游 8 pipeline 直讀決策層欄位。QC-1~QC-53 規則體系＋v14.9/v14.10 判斷反萃取（v14.9 三件套:§12.4b 三檢/§13b' 成功但劣化/§11 多尺矛盾;v14.10 QC-53 情境判斷手冊 32 條〔references/judgment-playbook.md 觸發索引式必答〕＋§14 裁決品質四問＋§12 拓撲判定與翻面三元歸因）（§12.4b 裁決推理三檢:分母爭議/證據權重 L1-L3/steelman 義務；§13b' 成功但劣化第二敗局；§11 多尺矛盾明文化——把強模型的 override 判斷動作固化為任何底座都必須走的程序）（sourcing 紀律 / 深度地板 / 裁決紀律含 hysteresis 與反向 critic / archetype 分類路由 / 循環交易鏡頭 / 寫稿後獨立 critic / peer 裁決一致性 / DD↔ID 對帳）。v14.8 新增 QC-52 DD↔ID 接線（事實先讀、結論後對：Part I 只讀 ID 事實區塊防錨定，草稿後強制對帳 sd_verdict/clock_phase/priced_in——sd_verdict 只當事實錨、Phase II 須自身位置閘交叉驗證、分歧輸出 ID 重跑訊號；q.py 主題行已印 ID 機器欄）。v14.7 結構拆分：核心 SKILL.md ＋ references/ 條件載入（數據採集、archetype gate-sets、循環鏡頭+附錄 B、dd-meta schema、HTML 模板、制度沿革），載入時點見核心路由表；判斷類規則治理（kill condition 登記）見 knowledge/rule_ledger.md，完整制度沿革（v12.6→v14.10 每條規則的 WHY 與歷版摘要）見 references/changelog.md。觸發語：ticker / 個股分析 / DD 報告 / 股票研究 / 估值分析 / 『{ticker} dca』『{ticker} 定見』『conviction analysis {ticker}』『最終判斷 {ticker}』『該不該進場 {ticker}』『買不買 {ticker}』。"
---
...
| **dd-meta v13** | `schema: "v14.10"`、`<meta name="dd-schema-version" content="v14.10">`；保留 22 個 v12 欄 + 5 個 v13 必填欄（`dca_verdict`/`dca_role`/`moat_trend`/`runway_post_y5`/`ev5y_pct`）+ 20 個選填欄（`irr_base_pct`/`max_dd_pct`/`asym_ratio`/… + v14.5 新增 `archetype`/`rearm_trigger`/`cycle_position`/`cycle_verdict`）。見 QC-32。 |
...
v12.6→v14.10 每條規則的誕生版本、觸發事故與 WHY 全文 → `references/changelog.md`。**寫報告時不需載入**；修改本 skill 任何規則前必讀，且判斷類規則的增刪必同步 `knowledge/rule_ledger.md`（kill condition 登記簿，見 repo CLAUDE.md 規則治理條款）。
...
### QC-32｜dd-meta JSON 硬性 schema（v14.10，validator 強制；全文條件載入）

dd-meta = 8 個下游 pipeline 的資料契約：22 個 v12 必填欄 + 5 個 v13 必填欄（`dca_verdict`/`dca_role`/`moat_trend`/`runway_post_y5`/`ev5y_pct`）+ 20 個選填欄；`schema: "v14.10"`。完整欄位定義 / enum / 對映 / 自驗腳本 → **Write 之前必 Read `references/dd-meta-schema.md`**。生成後必跑 `python3 scripts/validate_dd_meta.py --report` 全綠才 commit——validator 是最終權威，reference 漏讀會被它擋下（漏載保險）。
...
收到 ticker 後，直接開始深度版分析（單一 v14.10 DD 報告:基本面 Part I + 決策層 Part II + 擇時附錄 A）。無需詢問模式選擇。
...
4. 搜尋與 critic gate 完成後唯一允許輸出的文字：「搜尋完成，正在生成 v14.10 DD 報告（含 critic gate）...」，然後直接呼叫 Write。
...
**唯一正確流程：** ① 執行所有 web_search（不輸出文字）② 草稿完成後跑 critic gate（QC-41 / QC-48 / row 8b 循環 critic，命中時）並回填修正 + QC-40 機械 sweep ③ 輸出一行「搜尋完成，正在生成 v14.10 DD 報告（含 critic gate）...」④ 立即呼叫 Write 生成完整 HTML ⑤（Claude Code 本地環境,不需 present_files）。
...
單一檔 `docs/dd/DD_{TICKER}_{YYYYMMDD}.html`。鐵律：靜默輸出（搜尋與 critic gate 期間零文字，完成後只輸出一行「搜尋完成，正在生成 v14.10 DD 報告（含 critic gate）...」再直接 Write）；版本一號到底（frontmatter version / dd-meta schema / `<meta dd-schema-version>` / 頁首字串 / INDEX.md Schema 欄全部 = v14.10）。
