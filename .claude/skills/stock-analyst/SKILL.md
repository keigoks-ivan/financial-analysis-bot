...
| **dd-meta v13** | `schema: "v14.1"`、`<meta name="dd-schema-version" content="v14.1">`；保留 22 個 v12 欄 + 新增 `dca_verdict`/`dca_role`/`moat_trend`/`runway_post_y5`/`ev5y_pct`（必填）+ `irr_base_pct`/`max_dd_pct`（選填）。見 QC-32。 |
...
### QC-32｜dd-meta JSON 硬性 schema（v14.1，validator 強制）
...
8. `schema` = `v14.1`，且 HTML `<head>` 有 `<meta name="dd-schema-version" content="v14.1">`
...
- **不碰 §14 統一裁決;`cycle_position`/`trade_stance` 僅出現在附錄 B HTML，不寫入 dd-meta JSON**（schema 維持 v14.1，下游 6 pipeline 零影響）。
...
| EPS CAGR（未來 3 年） | > 20% | | 基期/FY+1E/FY+2E/FY+3E + GAAP/Non-GAAP CAGR + 口徑差距 + 資料來源 |
...
**Guardrail**:範圍寬度（上界 − 下界）必須 ≥ 10%p;< 10%p 視為假精準，打回。評級 🔴 必須在 §14a 倉位上限自動下修（例:原計畫 6% → 強制 ≤ 3%），並在 §14c 持有年限警示「中途出場風險高」。
...
| 5（Soft Veto） | 動能過熱（RSI 14d > 70 或 4 週漂移 > +10%，附錄 A） | ≥ 觀望 |
...
**衝突解決**:max-severity wins — 任一 Hard Veto 命中 → 直接鎖定迴避;Soft Veto 命中 → 上限觀望（不得輸出進場）。**dd-meta `dca_verdict` = 本矩陣最終輸出**。

### 14a. 倉位角色與進場計畫 + opportunity cost

> **分支規則**（依裁決晶片）:進場 → 全部欄位完整填寫;觀望 → 「初始建倉倉位」= 0%、「目標倉位」= 條件成立後 __%、「進場節奏」改為「觸發條件:___」、§14b 只填「進場條件」path;迴避 → 整表替換為「不持有理由」（≥ 2 條具體論點，對應命中的 Veto）+「重啟條件」（具體事件/數字門檻 OR 標「永久迴避」），§14b/14c 全 N/A。
...
| 進場價位區間 | $__ ~ $__ |
| 進場節奏 | 一次建倉 / 分批（幾次、間隔）;若附錄 A 動能過熱 → 強制條件式（等 RSI 回 50-60 / 等回測 MA60 / 等 Bollinger 中軌） |
...
> **§13c Max DD 路徑風險 🔴 → 此處倉位上限自動下修**並在 §14c 標註「中途出場風險高」。
...
> **降級說明（v13）**:舊 DD §2 的 Pure MA 六態機、短期 R:R、估值燈、品質分機制收進此附錄。它們**不是讀者頭銜，也不主導 §14 統一裁決**。其角色:① Pure MA / 動能過熱 / R:R 作為 §14 決策矩陣的 Soft Veto 輸入（時機訊號，最多把進場壓成觀望，不單獨觸發迴避）;② 品質分 / 估值燈餵頁首儀表板與 dd-meta `signal`/`val`/`ma`（基本面評級 metadata）。完整基本面壓測在 §6.E;此附錄不重複。
...
- 頁首固定:標的代碼 ｜ 資料抓取時間 ｜ 最新股價 ｜ DD Schema v14.1
- **版本一號到底（v14.0 起）**:frontmatter `version`、dd-meta `schema`、`<meta dd-schema-version>`、頁首字串、INDEX.md Schema 欄**全部一致（目前 = `v14.1`）**（沿用本 repo 歷史慣例:一個版號到底）。**下游相容**:validator（`^v1[234]\.\d+$`）、pre-commit floor（`"schema":"v1[34]`）、`aggregate_dca_stats` / `update_dd_index` / `dd_screener_dd_loader`（`startswith(("v13","v14"))`）已放寬接受 v13.x ∪ v14.x，既有 v13.0 報告照常運作，不需回溯重跑。**未來再升版時:bump 此 5 欄一致 + 放寬上述 6 個 pipeline 檢查多接受新版號。**
- **`<head>` 機讀標籤（全部必填）**:
  - `<meta charset="utf-8">` 之後緊接 `<meta name="robots" content="noindex,nofollow">`（私站防爬，research.investmquest.com noindex 政策）
  - `<meta name="dd-schema-version" content="v14.1">`（= frontmatter version，一號到底）
...
□ dd-meta schema="v14.1" + <meta dd-schema-version v14.1>？
...
□ §13c Max DD 為範圍（寬度 ≥ 10%p）；🔴 評級時 §14a 倉位自動下修 + max_dd_pct 寫 dd-meta？
