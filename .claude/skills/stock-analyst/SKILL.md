...
**pre-commit gate**:`scripts/hooks/pre-commit` 對 added 的 `"schema":"v1[34]` 檔強制 **110KB hard floor / 150KB soft-warn 上界**（下界防偷懶且維持原值、上界只警告不阻斷）;legacy v12 DD 維持 80/90KB;真要放行 lean-but-complete 報告用 `git commit --no-verify`。
...
5. **Part I 佔比 ≥ 60%**;檔案 < 110KB（與 hard floor 一致，未因控 token 而鬆）→ 深度不足，優先補上表缺的量化表格。檔案 > 150KB → **不補內容、改套三條省法**:先停渲染程序性自檢、再收斂深度表為承重列、最後把跨章節複述改為「見 §X.Y」（見【篇幅預算指令】），**不得靠刪掉整個量化模組或降低 sourcing 達標**。
