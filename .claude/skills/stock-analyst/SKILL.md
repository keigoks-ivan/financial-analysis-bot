...
| **篇幅預算** | **目標 110-150KB 單檔，兩模式共用**（2026-07-30 改制；廢除舊「隨附文獻模式 250-400KB」分流帶）。hard floor **維持 110KB**（added-only，防偷懶下界，**不因控 token 而鬆**）／上界 150KB 只軟警告（**無硬上界**——寫完才擋會逼重寫，比超帶更燒 token）。Part I 基本面 ≥60%；Part II 決策層是淨增深度（非灌水）。**第一槓桿是 critic 只餵摘錄（省 85% token、零分析損失），不是砍篇幅**——見【Token 紀律】。 |
...
### QC-38｜篇幅預算與深度標準（目標 110-150KB；hard floor 110KB 不變；added-only）

**目標單檔 HTML 110-150KB，標準模式與隨附文獻模式共用同一帶**，**Part I 基本面（§1+§3+§5+§6+§7+§8+§9+§10+§11）佔 ≥ 60%**;Part II 決策層（§12-§15）是淨增深度（不是灌水）。達標方式 = 完成 v12.6 五個深度模組（§6.I 分部前瞻 / §7.F 對手 P&L / §8.E DuPont+營運資金 / §9.F 逐段 TAM/SAM / §10.D 10Y 資本配置）+ Part II 四個決策模組（§12 矛盾裁決 / §13 pre-mortem+Max DD / §14 決策 / §15 複審），**不是拉長結論或回填估值/技術面**。
...
**誠實聲明（不得對自己或用戶粉飾）**:實測 12-gram 字面重複率僅 **7.2-7.5%**，主敘事**不是**複製貼上灌水。所以把總篇幅由 ~400KB 壓到 110-150KB，**確實等於少寫下來約 60% 的推導過程**——這是持有人在知情下為控 token 成本所做的取捨，不是「原本就有 60% 廢話」。因此每一次省法都要問:「這段是**做過但不必寫下**，還是**根本沒做**？」後者是違規。

**目標 110-150KB。三條省法依序套用，砍到帶內為止:**
...
**Write 分段**:單一 Write 若裝不下，仍可先寫前半再以 Edit 追加（機制不變），但**目標帶內的報告通常一次 Write 即可**;需要分三次以上追加＝篇幅已失控，回頭套上面三條。
...
- **critic 一律只收摘錄**:spawn QC-41/48/50/row 8b critic 時，prompt 內嵌**相關章節文字**（如 §9 產業判讀、§11.5 Bull 三項），**禁止把整份 HTML 或檔案路徑丟給 critic 讓它自己讀全檔**。實測:一次全檔冷讀約 **180k tokens**，三個 critic ≈ **540k**（≈ 整份 DD 的成本）;改餵摘錄約 **8k**，省 85%。**critic 的關卡數量與嚴格度一律不變，只改讀法**——這是省 token 唯一不用付品質代價的地方，優先做滿。
...
□ 檔案落在 **110-150KB 帶內**？< 110KB（hard floor，未變）→ 補上量化表格/決策模組深度,非拉長結論。> 150KB → 套【篇幅預算指令】三條省法（不印程序性自檢／表只留承重列／同一數字只出現一次），**砍重複不砍證據**。
