...
- **v14.0（方法論升級，dd-meta schema 仍 v13.0）**：QC-39 產業態勢變化雙向掃描（防 AVGO 型「沒搜到競爭惡化→過度樂觀」+ SNDK 型「只用歷史 pattern 外推→對結構性產業順風過嚴」）；QC-40 輸出潔淨（內部 QC 鷹架不得渲染進讀者面前的 HTML）。WHY：AVGO 把 moat ↑/進場 開在 Broadcom 於 Google TPU 份額 95%→65% 流失之上；SNDK 在 NAND 結構性缺貨下仍機械套歷史硬反轉 bear。兩者同根:產業態勢被當靜態處理。
...
- 頁首固定:標的代碼 ｜ 資料抓取時間 ｜ 最新股價 ｜ DD Schema v13.0（資料契約版）｜ skill v14.0（方法論版，可選顯示）
- **版本雙軌（v14.0 起，務必分清）**:① **dd-meta `schema` 與 `<meta dd-schema-version>` = 資料契約版 = `v13.0` 固定不變**（欄位結構未變，下游 validator/aggregator 依賴此），**不要因 skill 升 v14.0 就改成 v14.0**;② **frontmatter `version` = 方法論版 = v14.0**（QC-39/QC-40 等規則升級），僅供 lineage 與可選的頁首小字顯示。兩者解耦:methodology 升版不動 data contract。
- **`<head>` 機讀標籤（全部必填）**:
