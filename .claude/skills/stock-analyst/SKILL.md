...
version: v14.7
released: 2026-07-07
description: "收到股票 ticker 後，產出單一 v14.7 DD 報告（docs/dd/DD_{TICKER}_{YYYYMMDD}.html）— 基本面深度 Part I（骨幹 ≥60%）＋投資決策層 Part II（§12 矛盾裁決 / §13 pre-mortem+Max DD / §14 統一裁決 / §15 複審），收斂為一個人面對的統一裁決：進場 / 觀望 / 迴避（+ 倉位角色）。A+/A/B/C/X 降為 metadata-only 基本面評級（餵 screener）；短期擇時降為附錄純資訊不主導裁決。dd-meta schema=v14.7（22 個 v12 必填 + 5 個 v13 必填 + 20 個選填欄，validator 強制），下游 8 pipeline 直讀決策層欄位。QC-1~QC-51 規則體系（sourcing 紀律 / 深度地板 / 裁決紀律含 hysteresis 與反向 critic / archetype 分類路由 / 循環交易鏡頭 / 寫稿後獨立 critic / peer 裁決一致性）。v14.7 結構拆分：核心 SKILL.md ＋ references/ 條件載入（數據採集、archetype gate-sets、循環鏡頭+附錄 B、dd-meta schema、HTML 模板、制度沿革），載入時點見核心路由表；判斷類規則治理（kill condition 登記）見 knowledge/rule_ledger.md，完整制度沿革（v12.6→v14.7 每條規則的 WHY 與歷版摘要）見 references/changelog.md。觸發語：ticker / 個股分析 / DD 報告 / 股票研究 / 估值分析 / 『{ticker} dca』『{ticker} 定見』『conviction analysis {ticker}』『最終判斷 {ticker}』『該不該進場 {ticker}』『買不買 {ticker}』。"
---
...
| **dd-meta v13** | `schema: "v14.7"`、`<meta name="dd-schema-version" content="v14.7">`；保留 22 個 v12 欄 + 5 個 v13 必填欄（`dca_verdict`/`dca_role`/`moat_trend`/`runway_post_y5`/`ev5y_pct`）+ 20 個選填欄（`irr_base_pct`/`max_dd_pct`/`asym_ratio`/… + v14.5 新增 `archetype`/`rearm_trigger`/`cycle_position`/`cycle_verdict`）。見 QC-32。 |
...
v12.6→v14.7 每條規則的誕生版本、觸發事故與 WHY 全文 → `references/changelog.md`。**寫報告時不需載入**；修改本 skill 任何規則前必讀，且判斷類規則的增刪必同步 `knowledge/rule_ledger.md`（kill condition 登記簿，見 repo CLAUDE.md 規則治理條款）。
...
### QC-32｜dd-meta JSON 硬性 schema（v14.7，validator 強制；全文條件載入）

dd-meta = 8 個下游 pipeline 的資料契約：22 個 v12 必填欄 + 5 個 v13 必填欄（`dca_verdict`/`dca_role`/`moat_trend`/`runway_post_y5`/`ev5y_pct`）+ 20 個選填欄；`schema: "v14.7"`。完整欄位定義 / enum / 對映 / 自驗腳本 → **Write 之前必 Read `references/dd-meta-schema.md`**。生成後必跑 `python3 scripts/validate_dd_meta.py --report` 全綠才 commit——validator 是最終權威，reference 漏讀會被它擋下（漏載保險）。
...
收到 ticker 後，直接開始深度版分析（單一 v14.7 DD 報告:基本面 Part I + 決策層 Part II + 擇時附錄 A）。無需詢問模式選擇。
...
4. 搜尋與 critic gate 完成後唯一允許輸出的文字：「搜尋完成，正在生成 v14.7 DD 報告（含 critic gate）...」，然後直接呼叫 Write。
...
**唯一正確流程：** ① 執行所有 web_search（不輸出文字）② 草稿完成後跑 critic gate（QC-41 / QC-48 / row 8b 循環 critic，命中時）並回填修正 + QC-40 機械 sweep ③ 輸出一行「搜尋完成，正在生成 v14.7 DD 報告（含 critic gate）...」④ 立即呼叫 Write 生成完整 HTML ⑤（Claude Code 本地環境,不需 present_files）。
...
單一檔 `docs/dd/DD_{TICKER}_{YYYYMMDD}.html`。鐵律：靜默輸出（搜尋與 critic gate 期間零文字，完成後只輸出一行「搜尋完成，正在生成 v14.7 DD 報告（含 critic gate）...」再直接 Write）；版本一號到底（frontmatter version / dd-meta schema / `<meta dd-schema-version>` / 頁首字串 / INDEX.md Schema 欄全部 = v14.7）。
