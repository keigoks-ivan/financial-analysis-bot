...
| **篇幅預算** | **目標 150-200KB 單檔，兩模式共用**（2026-07-31 校準；廢除舊「隨附文獻模式 250-400KB」分流帶）。hard floor **維持 110KB**（added-only，防偷懶下界，**不因控 token 而鬆**）／上界 200KB 只軟警告。**真正的槓桿是分章節預算（§6 ≤20／§7 ≤18／§11 ≤15／§9 ≤12KB）＋§8 邊界即時 byte 檢查**，不是總檔案數字（**無硬上界**——寫完才擋會逼重寫，比超帶更燒 token）。Part I 基本面 ≥60%；Part II 決策層是淨增深度（非灌水）。**第一槓桿是 critic 只餵摘錄（實測省約 63% token、零分析損失），不是砍篇幅**——見【Token 紀律】。 |
...
### QC-38｜篇幅預算與深度標準（目標 150-200KB＋分章節預算；hard floor 110KB 不變；added-only）

**目標單檔 HTML 150-200KB，標準模式與隨附文獻模式共用同一帶**，**Part I 基本面（§1+§3+§5+§6+§7+§8+§9+§10+§11）佔 ≥ 60%**;Part II 決策層（§12-§15）是淨增深度（不是灌水）。達標方式 = 完成 v12.6 五個深度模組（§6.I 分部前瞻 / §7.F 對手 P&L / §8.E DuPont+營運資金 / §9.F 逐段 TAM/SAM / §10.D 10Y 資本配置）+ Part II 四個決策模組（§12 矛盾裁決 / §13 pre-mortem+Max DD / §14 決策 / §15 複審），**不是拉長結論或回填估值/技術面**。

**2026-07-30 改制（WHY，持有人裁決＝控 token 成本）**:舊規格分流帶「隨附文獻模式 250-400KB」以實測雙峰為據，但**因果歸因是錯的**——2026-07-30 對 SBUX 411KB／CMG 403KB／RMS 378KB 三份做位元組解剖，§4.5 逐字稿 read-through **只佔 3-5%**（12-20KB），肥肉在主敘事（226-236KB，58-60%）與程序性自檢（68-75KB，17-19%）。可見文字佔 72-76%，故非模板肥、是內容真有約 10 萬中文字。附文獻不是篇幅膨脹的原因，舊分流帶因此**廢除**。改制的目標不是「好讀」而是**降低 token 消耗**（實測每份 DD 約 620-645k tokens，輸出約佔 150k、其餘為反覆全檔讀取）。三條省法依序：① 不印程序性自檢 ② 深度表只留承重列 ③ 主敘事同一數字只出現一次。

**pre-commit gate**:`scripts/hooks/pre-commit` 對 added 的 `"schema":"v1[34]` 檔強制 **110KB hard floor / 200KB soft-warn 上界**（下界防偷懶且維持原值、上界只警告不阻斷;數字與 hook 第 86／136 行一致，勿再寫成 150KB——**帶內就是帶內，不得對 150-200KB 之間的檔案啟動省法**）;legacy v12 DD 維持 80/90KB;真要放行 lean-but-complete 報告用 `git commit --no-verify`。
...
5. **Part I 佔比 ≥ 60%**;檔案 < 110KB（與 hard floor 一致，未因控 token 而鬆）→ 深度不足，優先補上表缺的量化表格。檔案 > 200KB → **不補內容、改套三條省法**:先停渲染程序性自檢、再收斂深度表為承重列、最後把跨章節複述改為「見 §X.Y」（見【篇幅預算指令】），**不得靠刪掉整個量化模組或降低 sourcing 達標**。**150-200KB ＝ 帶內合格，不觸發任何省法**（門檻與檢核表第 2039 行、hook 上界一致;寫成 150KB 會讓帶內報告誤判超標而啟動壓縮重寫——那正是第 436 行明文禁止、且 2026-07-31 造成檔案毀損的失敗路徑）。
6. Part II 是否真的疊加（決策矩陣 / opportunity cost / Max DD / pre-mortem 三段倒推都到位），而非把 Part I 結論換句話重貼？

**反灌水（不變的底線）**:篇幅是深度的結果，不是目標本身。達標的正道是「五個基本面模組 × 真量化表格 + 四個決策模組 × 真決策內容」，**不是**同一數字換句話重寫、無 source 的長篇定性、或回填估值/技術。寧可 100KB 全自算，不要 150KB 注水;隨附文獻模式同理——§4.5 是壓縮成 read-through，逐頁摘要＝違規。**2026-07-30 起反向亦成立**:超過 150KB 不是深度勳章，但**不得以「那是複述」草率打回**——實測 12-gram 字面重複率僅 7.2-7.5%，超帶檔案的文字大多是真內容。故超帶的正確處置是套【篇幅預算指令】的「少寫下來」三條（自檢不渲染／表只留承重列／跨章節引用不複述），**不是刪分析、也不是指控灌水**。

**深度標竿（flagship 全深度，v14.2 起預設）**:每份新 DD **第一次生成就要達 flagship 全深度**（110KB hard floor 不是事後補、更不是用 `--no-verify` 放行 lean 版），且**一次寫到 150-200KB 帶內**——禁止「先寫完整版再壓縮重寫」（重寫等於把同一份報告的 output token 付兩次，是本次改制要消滅的最大單筆浪費）。參考範本——**篇幅與結構** = `docs/dd/DD_TSM_20260623.html`（110KB，帶內）;**§4.5 壓縮 read-through ＋跨文件分歧裁決的寫法** = `docs/dd/DD_NFLX_20260717.html` 之 §4.5 章節（僅取該章節寫法為範例，該檔總篇幅 308KB 屬舊制超帶，**不作篇幅標竿**）。兩模式的模組要求相同：五個量化模組全部「yfinance 三表自算的真數字表」——§8.E（DuPont 三因子 + CCC 逐年 DSO/DIO/DPO + ROIC 含/剔現金時序）、§10.D（多年 capex/股息/FCF/折舊 track + M&A 全史）、§6.I（分部 量×價 $ build）、§7.F（對手完整 P&L + 市佔演變 + 各對手策略敘述）、§9.F（逐段 TAM/SAM + 利潤池 OI margin 代理 + 三維營收 + 單位經濟）。**執行紀律**:搜尋階段就把 §8.E/§10.D 需要的多年三表（income/balance/cashflow 全欄 + AR/Inv/AP 營運資金項）一次抓齊（見【即時數據協議】yfinance 腳本），章節撰寫時直接建表，不要先寫精簡版再回頭補。`--no-verify` 僅限「純驗證/拋棄用、不上站」的報告;要上站的報告一律走 flagship 全深度 + 正常 commit 過 floor。
...
**品質不可退讓清單（持有人 2026-07-30 明示「DD 品質不能下降」;下列一律不得因控篇幅而動）**:
① 五個量化深度模組（§6.I 分部前瞻／§7.F 對手 P&L／§8.E DuPont+營運資金／§9.F 逐段 TAM/SAM／§10.D 資本配置）**全部照算照交**;② 七個量化表格照出;③ 所有 critic gate（QC-41／48／50／row 8b）**關卡數與嚴格度不變**，只改「餵摘錄不餵全檔」;④ sourcing 密度、每數字帶 source ＋ as-of 不變;⑤ §4.5 read-through 不砍（只佔 3-5%）;⑥ hard floor 110KB 不鬆;⑦ QC-52 DD↔ID 對帳、QC-49 hysteresis、知識帳本先讀後裁照跑。
**省的是「寫下來的推導」，不是「做過的研究」與「驗證的關卡」。**

**誠實聲明（不得對自己或用戶粉飾）**:實測 12-gram 字面重複率僅 **7.2-7.5%**，主敘事**不是**複製貼上灌水。所以把總篇幅由 ~400KB 壓到 150-200KB，**確實等於少寫下來約 50% 的推導過程**——這是持有人在知情下為控 token 成本所做的取捨，不是「原本就有 60% 廢話」。因此每一次省法都要問:「這段是**做過但不必寫下**，還是**根本沒做**？」後者是違規。

**目標 150-200KB。分章節預算為主閘、三條省法為手段:**

1. **程序性自檢執行但不渲染**（可省 ~68-75KB，17-19%）。裁決品質四問、§12.4b 推理三檢、§13a 盲點清單、決策矩陣執行紀錄、門檻檢核表——**照跑，答案影響裁決，但不印進 HTML**;只把「因此改了什麼」寫進對應章節。這是 QC-40（過程對帳不進 HTML）本來的精神，過去沒套用到自檢類，現在明文套用。
2. **深度表只留承重列**（可省 ~40KB）。七個量化表格全部保留，但每表只印「進入裁決的列」:多年時序收斂為「關鍵年 + 變化率」兩三列，完整逐年矩陣不印（算過、用了、不渲染）。§7.F 對手 P&L 只留與裁決有關的科目。
3. **主敘事同一數字只出現一次**（可省 ~80-100KB）。跨章節引用寫「見 §X.Y」而非複述數字與論證;§1 摘要不預演 §14 的裁決推理;§12/§14 不重述 §3/§11 已建立的事實。

**不准用來達標的手段**:刪掉整個量化模組、把表換成無 source 的定性段、砍掉 §4.5 read-through（只佔 3-5%，是最便宜的新鮮證據）、或降低 sourcing 密度。**篇幅省在重複，不省在證據。**

### 分章節 byte 預算（2026-07-31 新增，**取代總檔案數字成為主閘**）

**WHY**:2026-07-31 三份實測證明總檔案目標無效——AMZN 197.5KB（+32%）／PWR 180.1KB（+20%）／**MPWR 218.5KB（+46%，且是三者中最單純的公司）**。超帶幅度**與標的複雜度零相關**，故不是「複雜標的例外」，是總量目標本身不可操作:寫手要寫到第 12 節才知道超支，而那時重寫已被禁止（且該禁）。三份的具名分解高度一致——**§6／§7／§11（＋§9）是共同成本中心**，合計佔檔案 37-39%。

| 章節 | 預算 | 實測（MPWR／PWR） | 超支時砍什麼 |
|---|---|---|---|
| §6 成長 | **≤ 20KB** | 26.8／19.7 | 十個子區塊每個都寫解釋段 → 只有進入裁決的子區塊留解釋，其餘表格自證 |
| §7 護城河 | **≤ 18KB** | 23.7／19.7 | 七個子模組每個一段散文 → 二維評分與記帳裁定留解釋，§7.D/§7.E 表格自證 |
| §11 估值 | **≤ 15KB** | 20.5／15.7 | 兩套 PEG 基準與六指標分位表並存 → 保留裁決採用的那把尺，另一把一行帶過 |
| §9 產業 | **≤ 12KB** | 14.4／11.6 | 逐段 TAM/SAM 表保留，段落解釋合併為一段 |
| §4.5 read-through | 無上限 | 13.7／12.2 | **不砍**（只佔 3-5%，最便宜的新鮮證據） |

**§8 邊界即時檢查（硬規則）**:寫完 §8 當下**實際量一次目前檔案大小**（不是估計）。若 **> 110KB 而 §9-§15 未寫**，立刻對剩餘章節加嚴省法並回頭收 §6/§7 的解釋段。WHY:MPWR 自陳「三條省法我從 §9 才開始套，寫到 §8 已經 130KB」——**省法必須從 §3 就 binding，不是後半段補救**。

**表格與解釋的取捨方向（2026-07-31 反轉）**:舊指令是「表格全留、少解釋」。實測回饋指出這在 §6/§7/§11 是錯的——**那三節的產品是推理本身，不是表**（例:MPWR 的「Blackwell 失守記在等級不記趨勢」這個記帳裁定，寫成一段話才可稽核，壓成「見 §12」會讓 byte 下降但思考消失，而讀者失去的正是決定裁決的那件事）。**新方向:七個必交量化表格不動（仍是不可退讓），但「非必交的補充表」優先砍，換取承重判斷的解釋空間。** 判準:這張表有沒有進入裁決？沒有 → 砍表留解釋;有 → 表留、解釋收斂為結論句。
...
- **封閉式事實查證外包 sonnet（搜尋是第二大頭）**:實測本批多支 agent 反覆撞 WebSearch 200/200 上限。**能在派工前就把問題問死的查證**——同業倍數、第三方產能／出貨數據、法規與訴訟進度、對手單季數字、政策生效日——一律 spawn sonnet 子 agent 代查，prompt 必須是封閉式:「{具體問題}？給數字＋來源 URL＋as-of 日期;**查不到就回報查不到，不得推估、不得以訓練知識代替**」。回收後由本體判斷是否採信與如何入表（證據等級 L1/L2/L3 仍由本體判定）。
  **界線（違反此線＝品質下降，非省 token）**:可外包的是「**有唯一正確答案、且派工前就能定義清楚**」的查證;**不可外包**的是「**需要先知道自己在找什麼才看得見答案**」的閱讀——隨附逐字稿（§4.5）、前份 DD 的假設與觸發器、ID 的事實區塊，**一律本體自讀，禁止先 digest 再分析**。WHY:本批最高價值的發現全屬後者——AAPL 整場法說「**完全沒提到 Google／Gemini**」（一個不存在，只有帶著 TAC 假設去讀才看得見）、TT「7/29 −4.94% 發生在自家財報**之前**」（需對照時間軸）、MSFT「FY27 營益影響極小為真，因折舊省下被移到營益線之上的租金抵銷」（需追兩項會計變更的交互作用）。摘要器會在分析者知道要找什麼之前就先決定什麼重要，這些一律會被摘掉。且逐字稿全讀僅約 25-35k tokens ≈ 單份總成本 6%——**省 6% 去賭最高價值發現，是壞交易**。
  **不得因省 token 改動 critic 的模型指派**:QC-41/48/50 critic 用 sonnet 是**刻意的跨模型冷讀**（writer 不得自任 critic），是品質設計不是成本設計。
- **最終回報上限 400 字＋INDEX 行**:編排者（主線程）的 context 是共用資源。最終回報只給:路徑＋KB＋Part I 佔比、§14 裁決與前份對比、3-5 條決策相關變化、未解 critic 項、gate 狀態、INDEX 行。**不要複述報告內容**——報告本身就在磁碟上。
...
□ 檔案落在 **150-200KB 帶內**？< 110KB（hard floor，未變）→ 補上量化表格/決策模組深度,非拉長結論。> 200KB → 先查分章節預算哪一節超支（§6 ≤20／§7 ≤18／§11 ≤15／§9 ≤12KB），對超支節套三條省法,**砍重複與非必要表格,不砍證據與解釋**。
□ **§8 邊界即時 byte 檢查跑了嗎**？寫完 §8 當下量一次檔案大小;若已 > 110KB 而 §9-§15 未寫,立刻對剩餘章節加嚴省法（此時重寫已被禁止,只能往前收）。
□ **Token 紀律**:critic 是否只收摘錄（未把整份 HTML 餵給它）？前份 DD 是否只讀 dd-meta ＋ §14b 觸發器（未 Read 全檔）？是否一次寫成（未壓縮重寫）？
