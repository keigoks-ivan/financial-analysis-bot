...
```
目的:抓住生成報告當週可能剛發布的產業新聞。若有「發布日期 < 30 天」且與 §3 核心假設或 §7 護城河相關的事件,必須納入 §3 C 風險表或 §7 威脅清單,不得省略。
...
| **pre-mortem 校驗狀態（§13b 回填）** | ✅ 不需修正 / ⚠ 已補 N 條 secondary trigger / ❌ 已重寫 primary |
...
列出 24 個月內最可能瓦解護城河的技術或市場變化，按 QC-23 三級分類排序。
...
### 13b. Pre-mortem：失敗故事三段倒推（故事 → 校驗 §3.F → 修正 §3.F，強制三段式）

> §13a 是靜態列假設，pre-mortem 強迫具體化失敗路徑。和 §3.F 互補:§3.F 是單點觸發、pre-mortem 是路徑想像。**最常見偷懶 = 寫了失敗故事卻沒回頭校驗 §3.F。** 故強制三段。
...
**第二段｜校驗 §3.F Single Thing**（必填）:

| 檢查問題 | 答案 |
|:---|:---|
| 上面失敗故事的關鍵觸發事件，是否就是 §3.F 寫的 Single Thing？ | ✅ 是（直接撞上）/ ⚠ 部分重疊 / ❌ 完全獨立 |

**第三段｜修正 §3.F**（依第二段結果分流，**必須實際執行**）:
- 若 ✅ **是**:確認「§3.F 抓對了風險，不需修正」，標註「pre-mortem 故事 = §3.F trigger 的具體展開」。
- 若 ⚠ **部分重疊**:明列「§3.F 漏抓的風險面向:___」，**回頭在 §3.F 區塊實際補一條 secondary trigger**（HTML 中 §3.F「Single Thing 描述」顯示為 N+1 條）。
- 若 ❌ **完全獨立**:§3.F 選錯了 trigger，**必須回 §3.F 重寫**或新增「primary trigger（原 §3.F）」+「pre-mortem 派生 trigger」兩條，並說明取捨。

**校驗紀錄（必填）**:「§3.F 修正狀態:✅ 不需修正 / ⚠ 已補 N 條 secondary trigger / ❌ 已重寫 primary trigger」（同步回填 §3.F 表末「pre-mortem 校驗狀態」格）。
**Guardrail**:若第三段沒實際導致 §3.F 區塊變動（HTML 中 §3.F trigger 數 = pre-mortem 之前的數），且第二段標的是 ⚠ 或 ❌ → **整個 §13b 打回重寫。Pre-mortem 不是裝飾品。**
...
- 頁首固定:標的代碼 ｜ 資料抓取時間 ｜ 最新股價 ｜ DD Schema v13.0
- 頁首 Schema 版本字串必須取自本檔 frontmatter version 欄位
- **`<head>` 機讀標籤（全部必填）**:
  - `<meta charset="utf-8">` 之後緊接 `<meta name="robots" content="noindex,nofollow">`（私站防爬，research.investmquest.com noindex 政策）
  - `<meta name="dd-schema-version" content="v13.0">`（content 取自 frontmatter version）
...
□ §13b pre-mortem 三段式（故事倒推→校驗 §3.F→修正 §3.F）且依校驗實際更新 §3.F（或標「不需修正」）？
