...
version: v14.3
released: 2026-06-22
description: "收到股票 ticker 後，產出單一 v13 個股深度報告 — 基本面深度（Part I，骨幹 ≥60%）＋ 投資決策層（Part II），收斂為**一個人面對的統一裁決：進場 / 觀望 / 迴避**（+ 倉位角色）。v13 把舊 DD（v12.7）與舊 DCA（v1.5）整併成單一 `DD_{TICKER}_{YYYYMMDD}.html`，不再產獨立 DCA 檔。基本面研究只做一次（Part I），決策層（§12 矛盾裁決 / §13 pre-mortem+Max DD / §14 決策 / §15 複審）疊在其上不重搜。A+/A/B/C/X 訊號燈降為 metadata-only 的基本面評級（餵 screener），不再是並列 headline。短期擇時（Pure MA 六態機 / R:R / 估值燈）降為末段擇時附錄，純資訊不主導裁決。dd-meta schema=v14.3，新增 dca_verdict / dca_role / moat_trend（權威）/ runway_post_y5 / ev5y_pct 等決策層欄位供下游聚合器直接讀。保留 v12.6/12.7 全部深度量化模組（分部前瞻建模 / 對手 P&L 對照 / DuPont+營運資金 / 逐段 TAM/SAM / 10Y 資本配置）、yfinance 批量採集、Excel buy-side consensus（步驟零'）、QC-1~QC-48 規則、dd-meta JSON validator、隨附文件處理協議。**v14.x（schema 與 skill 版號一號到底，現 v14.3；下游 pipeline 相容讀取 v13.x 舊報告）方法論升級**：QC-39 產業態勢變化**三軸**掃描（強制搜 + 回填「競爭惡化」「結構 durability」「其他結構變數＝法規/通路/替代/商業模式的開放式 catch-all」，避免 AVGO 型過度樂觀與 SNDK 型對結構性順風 credit 不足）+ QC-40 輸出潔淨（內部 QC 鷹架不渲染進 HTML）+ QC-41 寫稿後產業態勢獨立 critic（不同 model instance 冷讀紅隊，兜底模板沒覆蓋的全新形狀）+ QC-42 循環交易鏡頭（附錄 B，僅循環/商品 archetype 觸發；平行投機軌，5 檔循環位置 + 反動能硬閘，不碰 §14/dd-meta，治 SNDK/MU 型「一路噴卻一路不推薦」的單一鏡頭脫節）+ QC-43 archetype 分類器（§0 判 primary+secondary+信心，路由 §5/§11/QC-31 換 gate-set;適配基石）+ QC-44 金融 gate-set（bank/insurer/broker 子型:ROTCE/CET1/NIM/P-TBV 取代 FCF/ROIC/D/E/EV-EBITDA;JPM 原型驗證）+ QC-45 未獲利高成長 gate-set（Rule-of-40/NRR/EV-S 取代 EPS-CAGR/PEG;MDB 原型）+ QC-46 轉機·受監管公用 gate-set（資產重估/SOTP/DDM/regulated ROE）+ QC-47 archetype×QC 適用矩陣（Stage 5 修剪:非複利下通用 tripwire 由 gate-set 取代不重複套，深度·契約·流程紀律永不修剪）+ **v14.3 爆發候選路徑（長抱數倍股 mandate 校準）**：§6.A'' runway_post_y5 🟢/🟡/🔴 邊界量化（🟢=Y5 末滲透率 ≤35% 或 sourced 第二 S 曲線）+ §11.5 不對稱比 AR=(P_bull×|Bull%|)/(P_bear×|Bear%|) 與 10Y 二段延伸（runway 🟢 必填）+ §14 row 8a 進場·條件式（爆發候選）（估值 🟠 可被 runway 🟢+AR≥4+非估值依賴型+moat≠↓ 推翻，衛星級 2-3% starter、雙軌加碼含論點增強觸發）+ §14b 長抱賣出分軌（核心/爆發候選的清倉觸發必須 thesis 級，估值/漲幅只准 trim）+ QC-48 爆發候選 Bull 冷讀 gate（row 8a 命中強制獨立 critic，打回降觀望）;dd-meta 新增選填 asym_ratio，enum 不變、下游 6 pipeline 零改動。觸發語：ticker / 個股分析 / DD 報告 / 股票研究 / 估值分析 / 『{ticker} dca』『{ticker} 定見』『conviction analysis {ticker}』『最終判斷 {ticker}』『該不該進場 {ticker}』『買不買 {ticker}』。"
---
...
| **擇時降級** | Pure MA 六態機 + 短期 R:R + 估值燈收進**附錄 A（擇時，~3%）**，純資訊，**不主導 §14 裁決**。其品質 gate / 估值燈仍餵儀表板與 metadata。 |
...
| **dd-meta v13** | `schema: "v14.3"`、`<meta name="dd-schema-version" content="v14.3">`；保留 22 個 v12 欄 + 新增 `dca_verdict`/`dca_role`/`moat_trend`/`runway_post_y5`/`ev5y_pct`（必填）+ `irr_base_pct`/`max_dd_pct`/`asym_ratio`（選填）。見 QC-32。 |
...
附錄 A — 擇時（降級，~3%）
        Pure MA 六態機 + 短期 R:R(3數字) + 估值燈。INFORMATIONAL ONLY，不主導 §14。

附錄 B — 循環交易讀數（條件性，僅循環/商品 archetype；~3%）
        循環位置（深谷投降/早/中/晚/過熱頂部）+ 交易姿態 + 反動能硬閘。
        SPECULATIVE（明標投機/交易，非投資），不主導 §14、不寫 dd-meta。成長股整段省略。
```
...
- **v14.3 F1/F2/F3（2026-07-02，WING/RKLB/ALAB 決策脊椎乾跑抓到的三個規格洞）**：乾跑 = 只抓最小市場數據把 §0→§6.A''→§11.5→§14 走一遍，測機制不測深度。① **F1 QC-45 估值燈懸空（大洞，RKLB 暴露）**:估值燈定義只有 Fwd PE 分位 + PEG，QC-45 未獲利股兩者全 undefined，但 `val` 必填且 §14 全部 baseline row 依賴它 → 未獲利高成長（十倍股最常出現的類別）§14 結構性走不完、row 8a 永遠不可達。修:附錄 A 加 QC-45 版雙尺（growth-adjusted EV/S = fwd EV/S ÷ fwd 營收成長%，0.5/1.0/1.5 切點 + 自身上市 EV/S 分位，取較嚴;毛利 < 50% 附 EV/GP 對照），§11.6 三分量改「營收 CAGR / EV/S re-rate / 稀釋」拆解、§11.5 以轉正年推 5Y 目標。② **F2 估值 🔴 裁決落空（ALAB 暴露）**:row 8 只寫 🟠，🔴 + 無 veto + signal ≥ B 對不到任何 row → row 8 擴為 ∈ {🟠, 🔴}（🔴 時重啟條件必含估值回落門檻）。③ **F3 ma = "-" 落空（ALAB 暴露，IPO < 250 週）**:baseline row 全要求 MA 狀態，樣本不足的新股落空 → 視同 🟡 走 row 9b（年輕股風險歸 Max DD 與倉位，不歸 MA 閘）。乾跑同時確認:row 4（WING 破 W250 → 觀望）、8a 對 🔴 的封鎖（ALAB）、QC-45 分流（RKLB）機制皆如設計運作。
...
**v13 注意**：A+/A/B/C/X 計算邏輯不變（quality × valuation × MA × 大盤 × trap，見附錄 A 機制 + QC-31），但它**不是讀者面對的頭銜**。讀者頭銜 = §14 統一裁決。兩者關係：基本面評級是「這標的本身值不值得持有」的靜態品質分類；§14 統一裁決是「現在/這個價位該不該進場 + 在組合中扮演什麼角色」的決策。§14 裁決由基本面驅動（決策矩陣 Hard/Soft Veto + Baseline，見 §14）。
...
MA104w（104 週移動均線）**必須**從 yfinance / web_search 查詢實際數值（用於附錄 A 擇時 + Pure MA 六態機）。
```
[TICKER] 104-week moving average
[TICKER] MA104 weekly TradingView
```
**禁止**使用「雙年均值外推」「起點終點平均」或任何估算法。若 2 次搜尋無法取得 MA104w,可用 MA100w 或 MA520d 替代,但必須備注「替代指標:MA100w（非 MA104w）」。同理 MA60→MA50 須備注。
...
附錄 A 開頭（基本面評級機制）必須先計算品質分:
  ...
  體質檢核 5 項各 ±0.5 分,結果加到底分上。
  品質分決定品質等級:≥ 7.5 → A 級 / 6.0-7.4 → B 級 / < 6.0 → 迴避。
（v13：品質分是基本面評級的一個輸入維度，metadata-only；不直接輸出倉位 %。）
...
執行前,先檢查 `docs/dd/DD_[TICKER]_*.html` 是否存在。**若存在,僅讀取三個區塊,其他章節嚴禁讀取:**① 最近一份的【版本修訂紀錄】② §3 B 三個核心假設 H1/H2/H3 + §3 C 三個風險 R1/R3 ③ §1 最關鍵監測指標表。**禁止讀取:** §1 統一裁決（避免錨定）、§11 估值/附錄 A 數字（用當下數據重算）、§6/§7 成長護城河評分（重新獨立打分）、§9 TAM 產業數據（重搜最新）。
...
|:---|:---|:---|
| **A+** | 品質 ≥ 7.5 + 估值🟢 + 短期 R:R ≥ 2.0 + 中期 R:R ≥ 2.0 + Pure MA 多頭 |
| **A** | 品質 ≥ 7.5 + 中期 R:R ≥ 1.0 + 長期持有信心 = 高 |
| **B** | 品質 ≥ 6.0 + thesis 完整，但時機不到（R:R 不足 / 估值🔴 / MA 過熱 / Bollinger 上軌之上） |
| **C** | 任一觸發:品質分 < 6.0;護城河 X 級或侵蝕信號 ≥ 3;陷阱定性 = 🔴;§6F AI 取代風險 = 🔴 |
| **X** | 任一觸發:重大治理問題/舞弊;結構性產業衰退（TAM 萎縮 + 倍數系統下移）;獲利品質崩壞（FCF/NI < 0.5 連 2 年） |
...
**v13 新增 5 個必填欄（schema 為 v13.x 時 validator 強制）+ Enum**：
...
8. `schema` = `v14.3`，且 HTML `<head>` 有 `<meta name="dd-schema-version" content="v14.3">`
...
5. **Part I 佔比 ≥ 60%**;檔案 < 100KB → 深度不足，優先補上表缺的量化表格。
...
**觸發**:row 8a 五條件（runway 🟢 + AR ≥ 4 + 非估值依賴型 + moat ≠ ↓ + 估值 🟠）全過時，**必須 spawn 不同 model instance 的獨立 critic**（spawn 機制同 QC-41），專門冷讀 §11.5 Bull 列三件事:
...
### QC-42｜循環交易鏡頭（v14.1 增補，附錄 B；SNDK / MU 教訓）

**WHY**:本 skill 是**單一鏡頭**——只問「值不值得長抱的好公司、在合理價」。循環/商品股在 cycle **每一個點**（含底部 GAAP 虧損、Munger gate 最紅）都答不過這題 → 投資軌（§14）結構上幾乎永遠「迴避/觀望」。但循環股的錢在 **Game 2**（在投降買、在延伸賣），是**另一種紀律**。SNDK（$40→$2,185）、MU（$103→$1,134）一路噴而 DD 一路「不推薦」的脫節，根因**不是 gate 算錯**（基本面裁決是對的——SNDK 報告已用 §11.5 正規化盈餘力算出公允 $150-500、識破 peak-EPS 假象、moat↓ 迴避），而是**缺一條鏡頭**：單一鏡頭量得到「長抱品質」，量不到「循環位置 + 交易姿態」。QC-42 加一條**平行、明標投機**的循環交易讀數（附錄 B），**不碰 §14、不寫 dd-meta**。

**① 觸發**（≥2 即啟動附錄 B;否則整段省略，成長複利股零行為改變）:
...
**② 循環位置（5 檔，錨在 through-cycle 客觀訊號，非動能）**:深谷投降 / 早循環 / 中循環 / 晚循環 / 過熱頂部。投票錶六訊號——P/B vs **自身**區間（非絕對值）/ 量 vs 價（bit·出貨 vs ASP）/ GM vs 自身區間 / 供給紀律 / 庫存 / 情緒部位——多數決 + 一句 sourced 裁決。

**③ 交易姿態 + 反動能硬閘**:
位置→姿態:深谷投降→積極分批建立 / 早→分批建立 / 中→持有不加 / 晚→分批了結 / 頂→清光不碰。
硬閘（override 上表，反動能;逐條檢查）:
- 閘 1:位置 = 晚/頂 → 姿態**不得**為任何「建立」，無論漲多兇。
- 閘 2:成長 = ASP-only + 量持平/降 → 最多「持有」，傾向了結。
- 閘 3:P/B > 自身 through-cycle 中位 2x → 禁新建立。
- 閘 4:訊號互相矛盾 → 標「位置不明，觀望」，不硬給姿態。
- 核心:**最敢在投降出手、最戒備在延伸段。若它在 blow-off 喊買 = 設計失敗。**

**④ 標籤 / 倉位 / 與投資軌關係**:
- 全程冠「投機/交易，非投資」;時間框架 = 循環（季-年）;倉位 = **獨立交易桶、小、與投資倉分計**（PM 不當核心/衛星）。
- 兩軌可（且常）背離:頂部多收斂（皆遠離）;**底部背離**（投資「迴避」+ 交易「積極建立」）= 現行 skill 產不出、本鏡頭的**獨門價值**。
- **不碰 §14 統一裁決;`cycle_position`/`trade_stance` 僅出現在附錄 B HTML，不寫入 dd-meta JSON**（不增任何 dd-meta 欄;schema 版號見 QC-32，下游 6 pipeline 零影響）。

**⑤ 雙制度（MU 型）**:有 secular sleeve（如 HBM）+ commodity sleeve → 循環鏡頭只套 commodity sleeve，secular sleeve 一句話另計（不隨循環賣）。

**⑥ 復用 + 反灌水**:~80% 是把**已有模組**（§11.5 正規化盈餘力 / QC-39 軸 B 供需 durability / §8 GM·庫存 / P/B）合成成交易讀數 + cycle 位置錶 + 紀律閘，**不新增搜尋、不另立冗長章**。產出 = 位置一句 + 姿態一句 + 六訊號投票表 + 兩軌背離一句。QC-40 適用（不渲染鷹架語言）。
...
**archetype 列舉（v14.1 起手 6 類）**:
1. `品質複利成長`（default;現行通用 gate）
2. `循環/商品`（投資軌維持 + QC-39 閘 B normalized + QC-42 附錄 B 交易軌）
...
**路由規則**:
- primary 決定 §5 門檻組 / §11 估值主錨 / QC-31 signal 對映用哪套（複利 = 現行;金融 = QC-44;循環 = 現行投資軌 + 附錄 B）。
...
**護欄（同 QC-42 精神，防新僵化）**:archetype 只換 **gate-set / 估值主錨 / signal 對映**;**永不碰**深度地板（QC-38）、資料契約（dd-meta schema）、流程紀律（QC-13/39/41）——那些全 archetype 通用。模型必須明寫「archetype + 信心 + 換了哪套 gate」（隱性 override → 可稽核 logged decision）。**輸出 §0 一句不入 dd-meta**（Stage 1 HTML-only;未來要餵 screener 分群再走 dual-read，見記憶 `dd_skill_version_vs_schema`）。
...
收到 ticker 後，直接開始深度版分析（單一 v13 報告:基本面 Part I + 決策層 Part II + 擇時附錄 A）。無需詢問模式選擇。
...
1.5. **Archetype 判定（QC-43）**:搜尋資料齊後、進章節前，判定 primary（+ secondary）+ 信心，路由 gate-set（金融 → QC-44 / 循環 → 投資軌 normalized + 附錄 B / 未獲利 → Rule-of-40 / default 品質複利）。在 §1 開頭/頁首明寫「archetype + 信心 + 換了哪套 gate」。
2. **⛔ 禁止在對話框輸出任何分析文字章節** —— 所有章節內容（頁首儀表板、§1 到 §15、附錄 A）全部直接寫入 HTML 檔案。
3. **立即使用 Write 工具生成完整 HTML 檔案**，包含所有章節的完整分析。
4. 搜尋完成後唯一允許輸出的文字：「搜尋完成，正在生成 v13 報告...」，然後直接呼叫 Write。
...
**唯一正確流程：** ① 執行所有 web_search（不輸出文字）② 輸出一行「搜尋完成，正在生成 v13 報告...」③ 立即呼叫 Write 生成完整 HTML ④（Claude Code 本地環境,不需 present_files）。
...
**本節的唯一目的：在分析開始前，把「這筆投資在賭什麼」顯性化。** 沒有清晰的論點錨定，後續章節的分析只是資訊搬運，無法轉化為可執行的決策。本節在開始搜尋前完成，作為整份報告的骨架;報告完成後回頭檢查每個章節的結論是否強化或挑戰了這裡寫下的假設。
...
|:--:|:---|:---|:---|:---|
| 1 | 成長是**結構性**還是**週期反彈**？ | 結構為主 / 週期為主 / 混合 + 理由 | 催化、能見度年數、backlog | §6.A' |
...
**目的**：Runway（§6.A）量的是「現在起算的成長年數」;runway_post_y5 量的是「**Y5 之後 5-10 年是否還有跑道**」——長抱者真正在賭的尾巴。**v13 把舊 DCA Phase A2 的此項移入此處就地產出，dd-meta `runway_post_y5` 由此填**;§11.5/§14d 直接引用，不另搜。
...
**硬接線（v14.3 起雙向）**:**🔴 直接觸發 §14d 持有年限 ≤ 3Y 警示 + §14 決策矩陣 Soft Veto（≥ 觀望）**;**🟢 為 §14 row 8a 爆發候選路徑的必要條件之一**（非充分，還需 AR ≥ 4 等，見 §14），且觸發 §11.5「10Y 二段延伸」必填。寫入 dd-meta `runway_post_y5`。
...
- **利潤池位置/流向結論（必填，v13 吸收 DCA A2）**:本標的所在環節利潤池占比方向 **↑ 流入 / → 持平 / ↓ 流出** + 1 個 sourced 證據 + 對 §6.A Runway 的含義。**↓ 流出 = 量增利不增警示，餵入 §14d 持有年限**（取代舊 DCA 的 A2 利潤池獨立搜尋 — 此處就地產出，§14 直接引用不另搜）。
**判定規則（硬接線）**:本標的環節 OI 池占比 5 年**淨流出 ≥ 5pp** → §6.A Runway 評級自動降一檔,且附錄 A 0 產業風向燈**不得標 🟢**;淨流入 → 可作附錄 A 盲點 1 救援佐證。
...
## §11｜估值與報酬（最大融合點：DD 估值 + DCA Asymmetry IRR 合一）
...
### 11.5｜不對稱報酬 Bull/Base/Bear 5Y + 機率 + IRR（v13 吸收 DCA §4）
...
**IRR 落點解讀**:< 8%/yr 弱（不及大盤平均）/ 8-12%/yr 中（合理 mid-conviction）/ > 12%/yr 強（high-conviction，core 候選）/ > 15%/yr 罕見（檢查機率分配是否過度樂觀）。
**追加壓力測試**:thesis 對但時間拉長兩倍（10 年），10Y IRR 是多少?考慮機會成本後的答案?（同期可比標的 IRR ≈ ?）

**不對稱比 AR（v14.3，必填一行）**:`AR = (P_bull × |Bull 5Y%|) / (P_bear × |Bear 5Y%|)`，取 1 位小數，寫入 dd-meta 選填欄 `asym_ratio`。解讀:< 2 平庸 / 2-4 偏正 / **≥ 4 顯著不對稱（§14 row 8a 爆發候選門檻）**。**AR 不是放鬆機率防線的理由**——下方反偏差防線（Bear 下限/Base 上限）照舊全部適用;AR 的意義正是「在誠實機率下仍然成立的不對稱」。Bear 5Y% ≥ 0（無下行情境）→ AR 標 N/A 並省略 dd-meta 欄（禁止用零下行製造無限大）;Bull 依據必須引用 §9.F 滲透率算術與 §11.7 pattern match IRR，且含關鍵分部的量×價骨架（用 §9.F 滲透率推 Bull 營收即可，不必全分部），不接受敘事式 Bull。
...
## §13｜Pre-mortem 與 Max DD（DCA §6 移入）
...
| 4（Soft Veto） | Pure MA ❌（附錄 A 不支持） | ≥ 觀望 |
| 5（節奏修飾，**非裁決閘**） | 動能過熱（RSI 14d > 70 或 4 週漂移 > +10%，附錄 A） | **不改裁決頭銜**——裁決仍照 row 8-10 算。命中 → §14a 進場節奏強制條件式分批（首階小倉＋回檔加碼）＋頁首掛「⚠️ 動能過熱，勿追高」。估值另由 row 8（🟠）獨立把關，故過熱+偏貴仍落觀望。 |
...
| 8a（Baseline·爆發候選，v14.3） | 無 Hard Veto + 無 Soft Veto（row 4/6/7/7a 皆未命中）+ signal ≥ B + runway_post_y5 = 🟢（§6.A''）+ AR ≥ 4（§11.5）+ 非估值依賴型（§11.6）+ moat_trend ≠ ↓ + 估值 = 🟠 | 進場·條件式（爆發候選）——衛星級 starter，倉位/加碼見 §14a 爆發候選分支;**命中即強制 QC-48 冷讀，critic 打回 → 降 row 8 觀望** |
...
**衝突解決**:max-severity wins — 任一 Hard Veto 命中 → 直接鎖定迴避;Soft Veto（row 4/6/7/7a）命中 → 上限觀望（不得輸出進場）。**row 5 動能過熱不在 Soft Veto 之列**——它只修飾 §14a 進場節奏（條件式分批），不改裁決頭銜（對齊 QC-7b:技術面不主導 §14）。**row 8a 在 row 8 之前判定（v14.3）**:估值 🟠 先測爆發候選五條件，全過才推翻觀望，任一不過 → 落 row 8。**估值 🔴 不適用 row 8a**（明顯貴仍然擋）;估值 ≤ 🟡 者本來就走 row 9/9b/10，與 8a 無關。Soft Veto（row 4/6/7/7a）對 8a 同樣封鎖——爆發候選不是 veto 繞行道。**dd-meta `dca_verdict` = 本矩陣最終輸出**。

**MA 狀態語意（v14.2 F1）**:🟢 最佳進場 / ✅ 強勢進場 → 進場（row 9/10）;🟡/🟠 回檔或成熟（W250 斜率未轉負）→ 進場·條件式（row 9b，長波段逢回佈局）;**只有 ❌ 系統失效（價 < W250 或 W250 斜率轉負）才把進場壓成觀望**（row 4）。**ma = "-"（IPO < 250 週樣本不足，v14.3 F3）**:baseline row 的 MA 條件視同 🟡 → 走 row 9b 進場·條件式（新股天然無 5 年均線，不得因樣本不足落空或視同 ❌;年輕股的波動風險由 §13c Max DD 與倉位處理，不由 MA 閘代管）。設計意圖:長波段 skill 鼓勵在結構上升的好公司回檔時佈局，**不要求「股價已站上所有均線」才准進場**（原 row 9/10 字面 MA ✅ 會把 🟢 最佳進場與 🟠 回檔擋在外、且造成裁決落空，已修）。

### 14a. 倉位角色與進場計畫 + opportunity cost

> **分支規則**（依裁決晶片）:進場 → 全部欄位完整填寫;**進場·條件式（row 5 動能過熱命中）→ 初始建倉倉位 = 目標的 1/3（首階小倉，可現在建）、其餘 2/3 掛回檔觸發、進場節奏 = 條件式分批、晶片副標註「進場（條件式:首階小倉＋回檔加碼）」**;**進場·條件式（爆發候選，row 8a 命中，v14.3）→ `dca_role` 取「衛星持倉」或「投機部位」（禁核心）、目標倉位上限 2-3%（小輸大贏，倉位本身就是風控）、首階 = 目標的 1/3、其餘 2/3 掛雙軌加碼——(a) 回檔觸發（價格）或 (b) 論點增強觸發（§9.F 滲透率 / 訂單 / design win 實績超前推演軌跡，須寫具體數字門檻）、§14c 持有年限標「長期 5-10Y」+ §13c F2 深回撤心理準備註記（爆發候選幾乎必有 >40% 回撤，thesis 完整不因波動砍倉）、晶片副標註「進場（爆發候選:衛星級分批）」**;觀望 → 「初始建倉倉位」= 0%、「目標倉位」= 條件成立後 __%、「進場節奏」改為「觸發條件:___」、§14b 只填「進場條件」path;迴避 → 整表替換為「不持有理由」（≥ 2 條具體論點，對應命中的 Veto）+「重啟條件」（具體事件/數字門檻 OR 標「永久迴避」），§14b/14c 全 N/A。
...
| **vs 現有組合的同類曝險（opportunity cost，必填）** | **___（具體點名 portfolio 中已持有的同類，比較 R:R 或 conviction 強度，說明為何邊際資金放新標的更優;若已持有同類但 conviction 較弱 → 該減舊加新還是兩個都減？）**（進場與觀望必填;迴避時整行省略，由「不持有理由」取代） |
...
> **迴避路徑終止點**:裁決 = 迴避時，§14c 整節替換為:不持有理由（1-3 句，對應命中 Veto）+ 重啟觀察條件（具體:若 ___ 發生 → 重新啟動 DCA 流程;或標「結構性迴避，無重啟條件」）。
...
> **降級說明（v13）**:舊 DD §2 的 Pure MA 六態機、短期 R:R、估值燈、品質分機制收進此附錄。它們**不是讀者頭銜，也不主導 §14 統一裁決**。其角色:① Pure MA ❌ 作為 §14 決策矩陣 Soft Veto（row 4，最多把進場壓成觀望，不單獨觸發迴避）;動能過熱 / 短期 R:R 為「節奏修飾」（row 5，只把進場改條件式分批，**不改裁決頭銜**——對齊本附錄「不主導 §14」與 QC-7b）;② 品質分 / 估值燈餵頁首儀表板與 dd-meta `signal`/`val`/`ma`（基本面評級 metadata）。完整基本面壓測在 §6.E;此附錄不重複。

### 基本面評級機制（品質分 → A+/A/B/C/X，metadata-only）
...
**綜合訊號 final_signal = A+/A/B/C/X**:依 quality × 估值燈 × Pure MA × 大盤豁免 × trap 組合（步驟見下表;與 QC-31 衝突時以 QC-31 為準）。**輸出進 dd-meta `signal`/`verdict`**。

| 步驟 | 條件 | 輸出 |
|:---|:---|:---|
| 1 拒絕檢核 | trap 🔴 或 gate 🔴 或 MA ❌ 或 quality 拒絕 | X（但 QC-31 cross-check:thesis 完整、僅估值/MA 過熱 → 降為 B） |
...
### Pure MA 六狀態機（週線 W52 / W104 / W250，SMA）
...
| ❌ 系統失效 | 價 < W250 或 W250 斜率轉負 | 0 |

W250 斜率定義:當週 W250 vs 13 週前 W250（正 > +3% / 持平 −3~+3% / 負 < −3%）。**盲點 2 救援**:MA ❌ 系統失效但三條件全滿足（附錄 A I 長期持有信心高 + §6 綜合 🟢 + 護城河 §7 ≥ 8）→ 降級為「🟠 暫不進場（追蹤池）」而非拒絕;例外失效（大盤破 W104 / QC-19 重大利空 / W250 斜率 < −10%）。Python 計算同 yfinance 批量採集腳本第 4 段。**輸出進 dd-meta `ma`**。
...
## 附錄 B｜循環交易讀數（條件性，僅循環/商品 archetype；SPECULATIVE — 不主導 §14 裁決）

> **定位（v14.1，QC-42）**:與附錄 A 並列的另一條 informational 軌，但答的是 **Game 2**——「做為**循環交易（非投資）**，現在在循環哪一格、該怎麼動」。**僅當 QC-42 觸發條件 ≥2 才渲染**;成長複利股整段省略，不留佔位。**不碰 §14 統一裁決、不寫 dd-meta**（`cycle_position`/`trade_stance` 只在此 HTML 出現）。全程明標「投機/交易」。觸發/紀律規格見 QC-42。

### B.0 觸發確認（一行）

本標的命中 QC-42 archetype 觸發項:__（列出命中的 ≥2 項）→ 啟動附錄 B。

### B.1 循環位置（投票錶，錨**自身 through-cycle 區間**，非動能）
...
→ **循環位置 = 深谷投降 / 早循環 / 中循環 / 晚循環 / 過熱頂部**（多數投票 + 一句 sourced 裁決）

### B.2 交易姿態（位置 → 姿態 + 反動能硬閘）

**交易姿態 = __**（深谷投降→積極分批建立 / 早→分批建立 / 中→持有不加 / 晚→分批了結 / 過熱頂部→清光不碰）

硬閘檢查（逐條，觸發即 override 向保守）:閘 1 晚/頂禁「建立」｜閘 2 ASP-only+量平 封頂「持有」｜閘 3 P/B > 自身中位 2x 禁新建｜閘 4 訊號矛盾 → 「位置不明，觀望」。

### B.3 兩軌對照（一句）

投資軌（§14）= __；交易軌（本附錄）= __。{收斂 / 背離} + 一句。**底部背離（投資迴避 + 交易積極建立）是本鏡頭獨門訊號;頂部多收斂（皆遠離）。**

### B.4 雙制度（如有，否則省略）

secular sleeve（如 HBM）= __（結構需求，不隨循環賣）；commodity sleeve = __（循環交易標的）。

> **倉位提醒**:交易倉為**獨立投機桶、小、與投資倉分計**;時間框架 = 循環（季-年）。實際組合佔比由 portfolio-manager 決定;此處只給循環位置 + 交易姿態作**交易輸入**，非投資裁決。**附錄 B 不主導 §14、不寫 dd-meta**(QC-42)。
...
| **5Y 機率加權 EV／IRR** | §11.5 | EV +__% 5Y ／ IRR __%/yr（爆發候選裁決時加註 AR __） |
...
| 附錄 B | 循環交易讀數（條件性） | **僅循環/商品 archetype（QC-42 ≥2 觸發）才渲染**;循環位置 5 檔 + 交易姿態 + 反動能硬閘，折疊式;明標投機,不主導 §14,**不寫 dd-meta**;成長股省略 |

### 功能規格

- 頁首固定:標的代碼 ｜ 資料抓取時間 ｜ 最新股價 ｜ DD Schema v14.3
- **版本一號到底（v14.0 起）**:frontmatter `version`、dd-meta `schema`、`<meta dd-schema-version>`、頁首字串、INDEX.md Schema 欄**全部一致（目前 = `v14.3`）**（沿用本 repo 歷史慣例:一個版號到底）。**下游相容**:validator（`^v1[234]\.\d+$`）、pre-commit floor（`"schema":"v1[34]`）、`aggregate_dca_stats` / `update_dd_index` / `dd_screener_dd_loader`（`startswith(("v13","v14"))`）已放寬接受 v13.x ∪ v14.x，既有 v13.0 報告照常運作，不需回溯重跑。**未來再升版時:bump 此 5 欄一致 + 放寬上述 6 個 pipeline 檢查多接受新版號。**
- **`<head>` 機讀標籤（全部必填）**:
  - `<meta charset="utf-8">` 之後緊接 `<meta name="robots" content="noindex,nofollow">`（私站防爬，research.investmquest.com noindex 政策）
  - `<meta name="dd-schema-version" content="v14.3">`（= frontmatter version，一號到底）
...
- **輸出完成後必須執行以下步驟，不得省略**:
  ...
  2. **更新 INDEX.md**:Edit append 一行到 `docs/dd/INDEX.md`，v13 格式（8 欄）:`| YYYY-MM-DD | TICKER | v14.0 | 統一裁決(進場/觀望/迴避) | 陷阱定性 | 護城河等級/估值燈/MA | DD_TICKER_YYYYMMDD.html | 備註 |`。第 4 欄為**統一裁決**（取自 §14;基本面評級 A+/A/B/C/X 不放此欄，已在 dd-meta `signal`）。備註限 3 句，每句 30-50 字，`<br>` 分隔（第 1 句 產業位置+品質;第 2 句 估值+護城河趨勢;第 3 句 關鍵判斷/觀察點）。
  3. **觸發網站同步**:執行 `python scripts/update_dd_index.py`（同步 research 頁主表 + dd-screener;v13 報告由 script 直接讀 dd-meta dca 欄位，定見欄連 `/dd/DD_X.html#decision`）。失敗則提示用戶手動執行，不得跳過。
  4. **terminal 摘要**（v13 格式）:
     ```
     ✅ v13 報告完成:[TICKER]
...
### 最終自檢清單（HTML 輸出前必須在對話中靜默列出 ✅/❌ + 實際數據）
...
□ dd-meta schema="v14.3" + <meta dd-schema-version v14.3>？
...
□ QC-42：循環/商品 archetype（≥2 觸發）→ 附錄 B 已渲染（循環位置 5 檔 + 交易姿態 + 反動能硬閘 + 兩軌對照）、明標投機、cycle_position/trade_stance **僅在 HTML 未寫入 dd-meta**;非循環股則附錄 B 整段省略不留佔位？
□ QC-43~47：§0 已宣告 archetype（primary + secondary + 信心 + 證據）;若非品質複利，§5/§11/QC-31 已按路由換對應 gate-set（金融 QC-44 ROTCE/CET1/P-TBV｜未獲利 QC-45 Rule-of-40/NRR/EV-S 且 NI 負未誤觸發 X｜轉機·公用 QC-46 資產/DDM）且明寫換了哪套;被取代的通用 PE/EPS/FCF tripwire 未重複機械套（QC-47）;支付網絡/資產輕金融科技未誤歸金融?
```
任一 ❌:在對話中說明「項 N 未達標:原因 ___」，回頭補完，重跑自檢，全部 ✅ 才能輸出 HTML。**禁止**:跳過自檢直接 Write;把 ❌ 偽報為 ✅;用「整體已達標」模糊報告替代逐條 ✅/❌。
