...
| 8（Baseline） | 無 Hard Veto + signal ≥ B + 估值 = 🟠 | 觀望（等估值） |
...
**MA 狀態語意（v14.2 F1）**:🟢 最佳進場 / ✅ 強勢進場 → 進場（row 9/10）;🟡/🟠 回檔或成熟（W250 斜率未轉負）→ 進場·條件式（row 9b，長波段逢回佈局）;**只有 ❌ 系統失效（價 < W250 或 W250 斜率轉負）才把進場壓成觀望**（row 4）。設計意圖:長波段 skill 鼓勵在結構上升的好公司回檔時佈局，**不要求「股價已站上所有均線」才准進場**（原 row 9/10 字面 MA ✅ 會把 🟢 最佳進場與 🟠 回檔擋在外、且造成裁決落空，已修）。
