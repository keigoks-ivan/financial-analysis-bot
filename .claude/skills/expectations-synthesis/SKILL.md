...
- **決策時 critic**（repo 規則，見 repo CLAUDE.md「Decision-time critic」）：spawn `industry-thesis-critic` 對相關 ID / theme 冷讀，存 `docs/id/_critic_{TICKER}_{YYYYMMDD}.md`，回 thesis-alive vs priced-in / top kill-risks / 現價是否該進。
...
- **§退出觸發**：4-6 條可監控賣出訊號 + 各「目前狀態」+ 操作含義。
- **§8 結論**：錨點收斂 + 分批表 + 淨效果（若與 critic 分歧誠實標明取捨）+ 總結。

### Step 5 — 接連結 + 維護索引
- 研究首頁 `docs/research/index.html` hero：在既有 synthesis 卡片後 insert 一張新卡（不同 accent 色），連到新報告。**只動 hero（靜態區），避開 `update_dd_index.py` 重生區（dd-tbody-v12 / PM_* / WR_CANDIDATES / DD_AUTO_STATS / OUTDATED_DDS_BANNER）。**
- 報告數 ≥3 時：建/更新 `docs/research/synthesis/index.html` 列表頁，hero 卡改指向列表頁。
...
- 用戶說 push 後走 commit flow：
  - `git add` **只加三檔**：`docs/research/synthesis/{T}.html`、`docs/research/index.html`、`docs/id/_critic_{T}.md`（**不要 `git add -A`**；先 `git status` 看 `??` 沒 orphan）。
