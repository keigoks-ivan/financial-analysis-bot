...
或該 ticker 在 DD 快篩追蹤名單上（見下方 v2.7 規則）


【DD 快篩追蹤名單（v2.7，持有人 2026-08-28 指定）】
除 $50B+ 篩選外，每次執行必須讀取 repo 內 `docs/dd-screener/latest.json`
（即 https://research.investmquest.com/dd-screener/ 主榜的資料源），
取 `stocks[].ticker` 欄位為追蹤名單；名單上的<b>美股上市</b> ticker
（排除含「.」後綴者如 2330.TW、LPK.DE）當日發財報者，<b>不論市值一律納入</b>。
- 卡片 meta-right 標注「（DD 快篩名單）」；最終摘要家數公式改為
  「本報告分析 N 家：$50B+ M 家 + DD 名單 J 家 + 用戶指定 K 家」。
- 搜尋預算：DD 名單且 <$50B 者按尾部公司等級（合併查詢）處理，
  除非它同時也是當日前 5 大市值。
- 讀不到該 JSON 檔（repo checkout 異常）時：在報告開頭聲明「DD 快篩名單未能載入，
  本日僅覆蓋 $50B+」，不可默默跳過。

如果排除後當天 $50B+ 公司不足 3 家，可放寬到 $30B+，並在報告開頭聲明「當天 $50B+ 公司僅 N 家，已放寬至 $30B+」
不允許「為了湊 10 家」而納入 $5-25B 的小公司（DD 名單者除外）

篩選範例：

✅ VZ ($200B)、PSA ($54B)、CDNS ($80-90B) — 自動納入
❌ AMKR ($5-20B)、DPZ ($15B)、NUE ($30B)、VTR ($25B) — 不在 DD 名單、用戶未指定 → 排除
✅ RMBS ($8B/$17B) — 用戶明確指定，納入並標注
✅ BESI、MPWR 等 — 若當日發財報且在 latest.json 名單上，不論市值納入並標注（DD 快篩名單）

Step 5：展示公司清單並鎖定為「完整性清單」
在對話框輸出一個簡短 table（ticker / market cap / EPS est / time），標注哪些是用戶指定加入。

⛔【v2.3 完整性硬規則 — 最高優先級，不可為任何理由豁免】
Step 5 產出的清單一經展示即「鎖定」。清單上每一家（$50B+ 全部 + DD 快篩名單當日 reporter + 用戶指定者）都<b>必須</b>進入 Phase 2 搜尋，並在 HTML 的 §1 擁有自己的 company-card。
...
- 兩者數量一致 → 才可進入 Phase 3
在最終摘要中必須明確寫出「本報告分析 N 家：$50B+ M 家 + DD 名單 J 家 + 用戶指定 K 家」，且 N 必須等於 Step 5 清單家數。
