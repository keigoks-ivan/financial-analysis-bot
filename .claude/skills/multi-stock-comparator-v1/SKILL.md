...
分析內容（§A-§E 章節文字、表格、bullet）只寫進 HTML，不在對話框整段重貼。找報告與寫檔過程中可以簡短說明目前在做什麼；找不到報告時要停下來問用戶（見下方缺漏處理），環境常數異常時也要先確認。寫完後回報檔案路徑與 git push 提示。
...
### 13. 白話呈現條款（2026-09-01 持有人拍板，全站適用）
