...
收到 2-5 檔 ticker(可選 DCA / DD)後,執行**基本面財務體質對比(§A.2)+ 競爭態勢對比(§A.3)+ 四層時間框架對比 + 推薦結論**,自動輸出 HTML 至 `docs/comparisons/`,可一鍵 `push` 上線。

> 📊 **核心定位**:不是重新做研究,而是強制框架化對比 — 從現有 DCA/DD 報告抽取關鍵維度,先做基本面四表(成長/獲利/資負/經營質量)與競爭態勢(份額消長/二維護城河/head-to-head/圈外威脅)的橫向解剖,再在「短/中/長/超長」四層時間軸下打分,最後給出明確排序與推薦標的。
...
Anti-laziness size floor(對齊 DD ≥ 80KB / DCA ≥ 50KB 政策):
...
| 成長質量拆解(價 vs 量) | Phase A3 | §8.B 成長品質 |
| 客戶結構(top1/5/10、NRR、dual-track) | Phase A3 | §8.H 客戶結構深度 |
| 護城河二維(execution / pricing power) | Phase A1(v12.3 對齊版二維拆解) | §9 二維拆解 + 5 級品質 |
| 威脅三級分類 | §6b Pre-mortem | §9 QC-23 威脅分類 |
| 議價權三段(上游 / 下游 / 地緣) | Phase A3 | §11 議價權三段獨立 |
| 資本配置 / SBC / buyback | Phase A3 | §10、§12 |
| Guidance 兌現紀錄(beat/miss) | Phase A3 / §6c | §8、§12 |

**若報告類型混合(DCA + DD)**:優先取 DCA 的 IRR composition + Pattern Match,優先取 DD 的 §13 同業估值 + §9.D 護城河變遷時間軸 + §9 二維拆解 + §11 議價權三段。舊版報告(pre-v12.3 DD / 舊 DCA)缺二維拆解或議價權三段時,從 §9 / Phase A1 內文推導並在表格 footnote 標「推導自內文,非報告原生欄位」。
...
- **報告 vintage 表**(v1.7 強制):每檔 1 row — 報告類型(DCA/DD)/ 報告日 / 之後是否有財報 / 漂移判定(無 / 輕微 / 重大 + 1 行說明)。重大漂移檔在 verdict card 加警語。
...
這節要回答:**「N 檔的財務體質誰最硬?成長的質量誰最高?」** — 不是把 DCA/DD 數字搬過來排排站,是橫向對照後給出「體質排序」。固定四張表,每表 N+1 columns(維度 + 每檔一欄),**每張表後緊跟 1 段該表的橫向洞察**(2-4 句,點名差異最大的格子,不是重述表格)。
...
| 成長質量拆解 | 價 vs 量貢獻具體 %(DD §8.B / DCA Phase A3),缺載標 n/a |
...
**格子紀律**:每格必須是 sourced number(來源 = DCA/DD 章節,或 WebSearch 補抓標 webfill)。**禁止「高 / 中 / 不錯」定性詞充數**。報告真的沒載 → 標「n/a(來源未載)」不准瞎掰;但 ROIC / FCF margin / NRR 這三個關鍵格若缺,必須 WebSearch 補 1 輪再標 n/a。
...
每份 DCA/DD 的 bull/base/bear 是獨立寫的,直接競爭的兩檔的 base case 可能**重複計算同一塊 TAM / 份額**(例:ALAB base 假設從 CRDO 搶份額,CRDO base 同時假設守住份額 — 兩個 base 不可能同時成立)。對每對直接互打的組合:
...
**來源紀律**:份額數字 / win-loss 案例優先取 DD §9(二維 + QC-23)+ §11、DCA Phase A1/A2;報告未載的份額與近期 win/loss,WebSearch 補 1-2 輪並標 webfill。**禁止用「競爭激烈」「龍頭地位穩固」這種口號充數** — 每個判斷要有案例或數字。
...
§A.2 四張表的每一格必須是具體數字 + 來源可追(DCA/DD 章節,或 WebSearch webfill 標註)。**禁止「高 / 中等 / 不錯 / 健康」這類定性詞充數**。報告未載 → 標「n/a(來源未載)」,不准瞎掰;但 ROIC / FCF margin / NRR 三個關鍵格缺值時必須先 WebSearch 補 1 輪。同一數字跨章節(§A.2 vs §C vs §B)必須一致 — EPS CAGR 在 §A.2 表 1 與 §C IRR 拆解用的是同一個數,不一致就是 bug。
...
### 11. 混合來源(DCA + DD)的公平性(v1.7)

DCA 檔有 IRR composition + Pattern Match,DD 檔沒有 — 資料多的檔不准因此自動占優。某維度只有部分檔有資料時:(a) webfill 補齊,或 (b) 該維度從**排序依據**中剔除(仍可呈現但標「僅供參考,非排序依據」)。禁止拿 A 檔有、B 檔沒有的維度當 tiebreaker。
...
4. 漂移比對結果寫進 §A 的「報告 vintage 表」(每檔 1 row:報告類型 / 報告日 / 之後財報? / 漂移判定)
