...
> **Deepening 點 G**：Guardrails 16-19 是「寫的時候不准違規」，這份自檢清單是「寫完後對照確認沒違規」。Skill 在輸出 HTML **之前**必須在對話中靜默列出此 8 條的 ✅/❌ 狀態，任一 ❌ 必須回頭補完才能呼叫 Write 工具。

**8 條檢查項（HTML 輸出前必須逐條報告 ✅/❌ + 實際數據）**：
...
□ HTML 預估檔案大小 ≥ 50KB（< 50KB 觸發深度自審：哪一節最薄？通常是 Phase A 引文密度不足或 Phase B 裁決一句話）
```
