...
7. **ROIC／FCF Margin／ROIC 5Y Avg 三欄選填（2026-09-09）**：若用戶提供的 Excel 在既有 7 欄之後多帶「ROIC %」「FCF Margin %」「ROIC 5Y Avg %」欄，`load_eps_estimates_xlsx.py` 會自動解析並在 build 時優先於 QGM／yfinance 採用（同姊妹 skill `refresh-eps-screener-web` §Step 6）；「ROIC 5Y Avg %」餵給 v3 席位資格的核心席耐久判定（`durable_5y`，≥15% 為 True，缺欄 fallback QGM 五年穩定度）。沒有這些欄的 Excel 完全照舊，不用手動處理。
