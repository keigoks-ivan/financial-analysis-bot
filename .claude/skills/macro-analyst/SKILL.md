...
8. **白話呈現條款（2026-09-01 持有人拍板，全站適用）**：所有輸出給讀者的顯示文字遵守 `notes/site-internal/root/_plainlang_styleguide.md`（『二補、實作定案』節優先）——①白話為主、術語為輔（對照表已定白話主名的詞一律用白話主名，原代號降小字或首現括號）；②新造術語前先查表，表上沒有的要先照鐵律③讀機制查證、定白話名並回寫對照表；③解釋深入淺出：每個承重判斷用讀者能懂的話講一遍，不堆行話。
...
- **機械針防循環（雙針獨立性鐵律）**：寫稿全程**不得**讀 `docs/macro/data/clock.json` 或引用機械針位置當判讀輸入——編輯針（報告水位裁決）與機械針（FRED 自動）的分歧資訊量建立在來源獨立上，寫稿讀機械針＝編輯針開始抄機械針、測謊器失效。若要對帳，放在**寫稿完成後**（比照 stock-analyst QC-52 事實先讀結論後對）：分歧照實記錄，不回頭改判讀去對齊。
