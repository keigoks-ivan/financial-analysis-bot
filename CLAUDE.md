...
| **v20 DD（2026-09-17 起預設，並行期一週）** | 入口 `scripts/dd2/run.py`（skill `ddreport` v5.0）：採證 **sonnet** 每軸一通／判斷 **Fable** 單輪預設思考／閘 **opus** 單輪＋一通 patch map／散文 **sonnet** 前後半並行；週線均線六態與程式欄漂移歸因由程式算；舊鏈 `ddreport.py run` 保留一週，任一檔出現舊鏈不會有的失敗即回退並記錄 |
