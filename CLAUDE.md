...
DD 報告有雙向 size budget：**下界是深度閘門（擋 commit），上界是 token 成本預算（只警告）**。達標的正道永遠是「真量化模組 × sourced 數字」，反灌水鐵律不變（skill：「寧可 100KB 全自算，不要 150KB 注水」）；但 2026-07-30 起**反向亦成立**——超帶不是深度勳章。惟 2026-07-31 實測證明超帶**不是**「同一判斷講了三遍」（字面重複率僅 7.2–7.5%），而是「每張表底下都再寫一段解釋」，故處置是分章節預算而非指控灌水。
...
**實測禁止誤讀（防後續 session 拿錯數字當理由）**：曾有一版草稿寫「主敘事 58–60% 是複述」——**該說法經量測否證，不得引用**。12-gram shingle 字面重複率**僅 7.2–7.5%**，散文不是複製貼上；數字 token 重複 75% 的表象由年份（2026×183 等）與整數百分比（10%／15%／20%）主導，屬財報文件常態，非「同一發現講三遍」。**誠實結論：那 290KB 文字大多是真實且不重複的內容，把總篇幅壓到 150–200KB ＝ 少寫下來約 50% 的推導過程。**
