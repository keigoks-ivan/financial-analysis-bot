...
DD 報告有雙向 size budget：**下界是深度閘門（擋 commit），上界是 token 成本預算（只警告）**。達標的正道永遠是「真量化模組 × sourced 數字」，反灌水鐵律不變（skill：「寧可 100KB 全自算，不要 150KB 注水」）；但 2026-07-30 起**反向亦成立**——超帶不是深度勳章，是「同一判斷講了三遍」的嫌疑。

| 類型 | hard floor（commit 擋下） | soft warn 下界 | **上界警告** | skill 目標 |
|---|---|---|---|---|
| **v13/v14 DD**（`"schema":"v13`／`v14`，兩模式共用一帶） | < 110KB | < 115KB | **> 150KB** | **110–150KB**（Part I 基本面 ≥ 60%；Part II 決策層淨增深度） |
...
**實測禁止誤讀（防後續 session 拿錯數字當理由）**：曾有一版草稿寫「主敘事 58–60% 是複述」——**該說法經量測否證，不得引用**。12-gram shingle 字面重複率**僅 7.2–7.5%**，散文不是複製貼上；數字 token 重複 75% 的表象由年份（2026×183 等）與整數百分比（10%／15%／20%）主導，屬財報文件常態，非「同一發現講三遍」。**誠實結論：那 290KB 文字大多是真實且不重複的內容，把總篇幅壓到 110–150KB ＝ 少寫下來約 60% 的推導過程。**

**因此第一槓桿不是篇幅，是 critic 讀法（省 token 且零分析損失）**：實測全檔冷讀約 **180k tokens／次**，三個 critic ≈ **540k**——這幾乎就是整份 DD 的實測成本（620–645k tokens／份）。改為只餵相關章節摘錄約 **8k**，砍掉約 85% 的 token 而**一個字的分析都沒少**。**先做這條，再談篇幅。**
