...
判斷只吐五個答案，字數砍到現在的十分之一。沒有修補輪。證據按軸過期，不按 run 過期。
估計全新一檔從 $17 到 $22 降到 $8 到 $12，重跑一檔從 $16 降到 $4 到 $6。時間從約 60 分降到約 25 分。以上是估計，驗收見 §10。
...
估計依據：判斷通輸出從 55K 到 93K token 降到 ≤ 10K，這一項占現在 62% 的成本。修補輪與閘二輪歸零，占 15% 到 25%。收證據與散文不變。
...
4. 判斷輸出 token 上限（§4.2）。拍板：15K。五塊寫滿約 6K 到 8K，15K 為兩倍餘裕。撞上限先查多寫了什麼，不先調高。
