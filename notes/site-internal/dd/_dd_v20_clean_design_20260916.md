...
| 處置 | 紅燈 ≥ 1 → 一通 patch map（判斷模型單輪、只回被點名欄位、程式在候選目錄整份驗證後才套用）→ 再閘一次；仍紅 → 停在 `gated_fail`，印清單，不發布。黃燈只記錄。`--no-gate-patch` 可關 |

閘後修補只允許一通（2026-09-16 晚間持有人改拍板，見 §13 第 2 題）：TXN 首跑閘抓 3 紅，整份重判 $3.5 且裁決有跑間變異，定點修補約 $1 且不動其他判斷。仍紅就停下交人。閘的覆蓋面掃描與量化模組抽查兩項職責保留（2026-08-06 拍板）。
...
- 不拆多個 skill。
- 不做 replay、delta 判斷路由、short 與 loop 雙模式、JSON repair agent。閘後 patch map 只留一通（§4.3）。
...
2. 閘紅燈不自動修（§4.3）。原拍板：不自動修。**同日晚間改拍板：允許一通 patch map 再閘一次，仍紅才停**。依據：TXN 首跑閘抓 3 紅，整份重判 $3.5 且三跑三種裁決；定點修補約 $1、由程式整份驗證後才套用，第二輪閘仍是跨模型冷讀。
...
## 14｜實作期間的規則發現（2026-09-16）

- **均線六態改程式算**（`dd_screener_ma`＋六態映射，事實表 `f_ma_state`，落檔強制覆寫 `decision_inputs.ma`）。TXN／TSM 首跑同一個洞：判斷者無資料時一個硬填 ✅、一個填 `-`。
- **MA ✅/🟡 對矩陣合併**（持有人拍板，登記 rule_ledger）：265 檔母體 W250 13 週斜率中位 1.4%，只有 19% 過 +3%；排列完整的 140 檔多頭七成被標 🟡。5 年均線斜率是落後指標，不該獨力決定核心／衛星。
- **程式欄位的漂移歸因由程式生成**（ma／price_at_dd／因 ma 改列的裁決與角色），判斷者只歸因基本面欄位。
