...
| 2026-09-05 | **批次 1：CDNS** | 0 | 8.5M（首跑覆蓋 8 輪太緊六個被砍，resume 補） | 1.63M | 0.88M | **11.1M**（$23） | 進場｜衛星 9b（前份進場｜核心）；白話角色與矩陣不一致由 orchestrator 對齊 → J5 validator |
| 2026-09-05 | **批次 1：STRL** | 0 | 5.5M（a_5 批撞 10 輪兩次，手動 16 輪過） | 1.33M | 0.14M | **7.0M**（$16） | 進場｜衛星 9b；閘 🔴 0 🟡 4；證據原文半形逗號被 pre-push QC 擋 → dd_brief 正規化待補 |
| 2026-09-05 | **批次 1：GRAB** | 0 | 6.5M | 1.56M | 0.13M | **8.2M**（$18） | 進場｜衛星 9（MA❌例外）；前份觀望→翻面，白話自述主因方法論（週線只管節奏）— 校準輪檢視；閘 🔴 1 修補後 0 |
| 2026-09-05 | **批次 2：FN**（WP8 後首檔，finish 自走 worktree 推成功） | 0 | 9.4M（Stage 0 重派 2 次、156 輪；koyfin 必讀 4） | 1.70M（判斷 8 輪一次過＋修補 5 輪；輸出 76K） | 0.15M（閘一次） | **11.2M**（≈$20＋重派） | 觀望｜追蹤 row 8（估值爭議）；前份 5/5 v12.3 B 級觀望同向；價 717.8→407.4（−43%）；閘 🔴 1＝散文全寫「進場｜衛星、三段建倉」而機械欄觀望（QC-49 互斥），修補以機械欄為準並補 rearm（11 月 Q1 FY27 總資料中心 QoQ>0 或 ≤$350）— **Fable 直覺 vs 機械列打架案例，校準輪檢視**；資料級：Q4 FY26（8/17）逐字稿缺、EMS／ODM 換尺未執行、週線缺 ma「-」 |
| 2026-09-05 | **批次 2：BWXT** | 0 | 6.2M（Stage 0 一次過無重派） | 1.51M（判斷 9 輪＋修補 2 輪；輸出 67K） | 0.14M | **7.9M**（$18） | 進場·條件式（爆發候選）｜衛星 8a；前份 5/5 v12.3 B（衛星候選／觀望）→翻面，自述主因價格 216.68→157.59（−27%）、次因方法論；閘 🔴 1＝ROIIC 推導不可複算（增量投入 $450M vs 括號 $347M、比率皆對不上）修補後 0；🟡 2 資料級（訴訟頁疑 B&W 混淆、second_source 軸 queries 未附）|
| 2026-09-05 | **批次 2：ETN** | 0 | 7.3M（koyfin session 過期、downloader 逾時 300s 改磁碟逐字稿） | 0.94M（判斷 6 輪＋修補 2 輪；輸出 51K） | 0.15M | **8.4M**（$18） | 觀望｜追蹤 row 8；前份 5/6 v12.3 觀望同向；價 410.85 持平（val 🟡→🔴 主因方法論改尺）；閘 🔴 1＝商模轉移軸兩筆（Revenera 訂閱化／Brightlayer）judgment 零引用亦未列 dismissed（J1 型缺口）修補後 0；ma「✅」係推定非實測、Q2 26 逐字稿缺 |
| 2026-09-06 | **CAMT（單檔乾淨量測，凌晨無其他 session）** | 期間本 session 收通知 51 輪 13.7M／$6.6（另計） | 20.2M（transcript 全計：a_5 十輪撞牆 3 次＋兩輪重派＋手動 16 輪；存查 agents/ 只留末版 6.5M） | 3.5M（判斷 11 輪＋修補；ledger 只記 resume 後 1.79M） | 0.13M | **23.8M**（$38，transcript 全計；finish ledger 誤報 1.9M） | 觀望｜追蹤 row 8；前份 5/14 v12.3 B 觀望同向；價 168.88→145.72；rearm ≤$125 且 Q3 營收 ≥$158M；閘 🔴 1＝reinvest_rate 無算式卻撐起 endo_ceiling 12%＋「3.4×18=$86」不對帳，修補後 0；無 ID 含 CAMT → peers 需手給（ONTO／NVMI／KLAC）；**resume 後 manifest 重置致 ledger 只算 resume 段**（待修） |

**額度尺（2026-09-06 CAMT 單檔量測，Max 20x 訂閱）**：Claude Code 餵 statusline 的 `rate_limits`（已在 `~/.claude/statusline-command.sh` 加一行記到 `~/.claude/rate_limits_log.jsonl`）起訖讀數：02:11 五小時條 6%／七天條 60% → 03:01 五小時條 24%／七天條 63%。窗內用量＝CAMT $38（23.8M cache_read）＋orchestrator $6.6（13.7M）≈ $45 等值 API 價。換算：**五小時條 1 點 ≈ $2.5 等值，滿格 ≈ $250**；**七天條 1 點 ≈ $15 等值（整數顯示，±1 點 → $11–22），滿格 ≈ $1,500（$1,100–2,200）**。交叉檢查：本機 jsonl 自 9/1 06:00（七天條重置點）起全部 session 等值 ≈ $1,700 對應 63% → 滿格 ≈ $2,700，兩尺差 1.8×，差在額度對 cache read 的權重未知（DD 是 cache read 密集型，等值價法會低估它占額度的比例）。**實用結論**：一檔乾淨快速版（$18–23、8–11M）≈ 七天額度 0.7–1.5%、五小時額度 8–9 點；含重派的 CAMT ≈ 1.4–2.5%。一個五小時窗最多約 5 檔（昨天實測 3–4 檔＋orchestrator 撞牆吻合）；一週若只跑 DD 約 40–70 檔，但實際七天額度九成被 Fable 互動 session 吃掉（8/31–9/5 全週 $1,775 中互動 $1,574、9 檔 DD 合計 $290）——**省額度要省的是 orchestrator 對話，不是 DD 管線**。七天條重置 9/8 06:00（台北）。

**批次 2 註記（2026-09-05 深夜）**：三檔全帳 27.5M／≈$56、每檔 38–48 分鐘無人接手；WP8 的 finish 自走 worktree cherry-pick 三檔全部推成功（遠端領先 20／24／27 皆未 HOLD）。暴露一個下游缺口：個股樞紐頁 `docs/t/{T}.html`（`build_ticker_hubs.py`）只掃 `docs/dd/DD_*.html`，七檔快速版全部不在樞紐頁——持有人「網站上沒看到 FN」即此。已修（bce79ffe8）：產生器加掃 `docs/dd/brief/`、列「DD 快速版」；finish 白名單加 `docs/t/{T}.html`＋`index.html`；產生器 nav 改帶 `item="thub"`（原本每次重生會翻掉 253 頁的「個股研究」高亮再靠 nav 同步腳本補回）。三檔閘皆 🔴 1 修補後 0，型別各異（散文 vs 機械欄互斥／算術不可複算／J1 零引用）——閘作為前移品質閘的三種攔截型都出現過一次。到期隊列 >120 天 6 檔已全部清完。
