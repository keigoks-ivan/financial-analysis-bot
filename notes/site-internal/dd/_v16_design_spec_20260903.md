...
**散文成本修法（2026-09-04）**：CRDO 54 次工具呼叫拆分——13 次讀規則/表格、3 次讀 `render_dd.py` 原始碼釐清組裝機制、18 次段落 Write、19 次驗證/修補（3 輪 render+validate、3 次誤用 `Edit` 局部補符號、1 次 grep `validate_prose.py` 原始碼、2 次 grep 定位字串）。落實五項：①`validate_prose.py --dump-numbers` 產逐字數字白名單（含 −/%/$ 符號），(b2) 動筆前先讀、逐字複製；②洩漏詞表＋符號正規化規則抽成 11 行摘要內嵌 `agent-prompts.md` (b2) 模板，免讀外部檔／免 grep 原始碼；③六支閘合成 `scripts/dd_gates.sh {T} {D} {OUT_HTML}`，(b2) 只呼叫一次，驗證輪次上限由 3 降 2；④「prose 目錄任何 Edit 視為無效輸出」改模板層明文強調（放棄 mtime 機械偵測——跨 session/跨工具的寫入序號比對成本高於效益，屬本輪刻意選擇非疏漏）；⑤`render-rules.md` 18.4KB 砍歷史/沿革敘述與冗餘表格欄壓縮至 15.9KB（語意不變；未達 ≤12KB 目標——§7 篇幅預算/不可退讓清單是 CLAUDE.md 明文保護的內容本身，不得因篇幅改制再砍，其餘章節已無安全可砍的冗字）。
