...
你負責下列 1 個覆蓋軸，逐軸獨立查證，**本軸最多 3 次 WebSearch**（每一類題目一次），查不到就記 none，不要一直挖。

[regulatory_antitrust] 法規／反壟斷
Q: 本標的是否面臨反壟斷調查、拆分風險或產業特定監管新規？
  - DDOG {COMPANY} antitrust OR monopoly investigation 2026
  - {COMPANY} regulation OR regulatory scrutiny {INDUSTRY} 2026
  - DDOG {COMPANY} DOJ OR FTC OR EU competition 2026
  (na_allowed=true)
...
寫入路徑：`/Users/ivanchang/financial-analysis-bot/.dd_build/runs/DDOG_20260924/parts/axes_5.json`（用 Write 工具寫一次，不要分次 Edit）。

## 輪次紀律
你只有 WebSearch、WebFetch、Write 三個工具，沒有 Bash。**寫完 part 檔就停止**，不要自己驗證、不要重讀、不要回報長文。驗證由程式做；缺欄程式會把該軸記成缺口，不會叫你重寫。
搜尋：本軸最多 3 次；WebFetch 只在搜尋摘要不足以寫出 as_of 與 claim 時才用，每軸最多 2 次。
