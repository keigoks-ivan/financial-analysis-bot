...
## 寫入路徑（一次 Write）
`/Users/ivanchang/financial-analysis-bot/.dd_build/runs/DDOG_20260924/parts/numbers_collect.json`——形狀為 `{"numbers": {"latest_quarter_kpis": {"_required": true, "quarter": "...", "items": [...]}}}`（只含你補的鍵，其餘欄位不重覆寫入，交由 merge 保留 `dd_numbers_extra.py` 原值）。
