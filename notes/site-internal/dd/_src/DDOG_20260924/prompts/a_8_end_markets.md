...
你負責下列 1 個覆蓋軸，逐軸獨立查證，**本軸最多 6 次 WebSearch**（每一類題目一次），查不到就記 none，不要一直挖。

[end_markets] 各主要終端市場
Q: 此營收段的需求動能、市佔與價格走向為何（依 §3 營收段逐一列，per_segment 展開）？
  - DDOG {COMPANY} {SEGMENT} segment revenue growth 2026
...
寫入路徑：`/Users/ivanchang/financial-analysis-bot/.dd_build/runs/DDOG_20260924/parts/axes_8.json`（用 Write 工具寫一次，不要分次 Edit）。

## 輪次紀律
你只有 WebSearch、WebFetch、Write 三個工具，沒有 Bash。**寫完 part 檔就停止**，不要自己驗證、不要重讀、不要回報長文。驗證由程式做；缺欄程式會把該軸記成缺口，不會叫你重寫。
搜尋：本軸最多 6 次；WebFetch 只在搜尋摘要不足以寫出 as_of 與 claim 時才用，每軸最多 2 次。
