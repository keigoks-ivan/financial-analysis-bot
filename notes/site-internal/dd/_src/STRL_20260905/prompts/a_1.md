...
Q: 近 12 個月是否有 M&A、集體訴訟、臨床/FDA、產品下架、SEC 調查/重編等重大事件？
  - STRL {COMPANY} acquisition merger 2025 2026
  - STRL {COMPANY} class action lawsuit securities fraud
  - STRL {COMPANY} clinical trial FDA approval 2025 2026
  - STRL {COMPANY} product launch recall warning letter
  - STRL {COMPANY} SEC investigation restatement
...
寫入路徑：`/Users/ivanchang/financial-analysis-bot/.dd_build/runs/STRL_20260905/parts/axes_1.json`（用 Write 工具寫一次，不要分次 Edit）。

## 交稿前跑（必過才回報）
```
python3 scripts/validate_evidence.py --part /Users/ivanchang/financial-analysis-bot/.dd_build/runs/STRL_20260905/parts/axes_1.json
```
