...
## 寫（只有 Write 工具，最多 6 輪，沒有補寫輪）

一次寫對，寫壞即交卷不補——這一輪沒有機械閘回饋、沒有第二次機會。輸出兩個檔：

1. `/Users/ivanchang/financial-analysis-bot/.dd_build/runs/TSM_20260916/prose_A.html`：依序含 s1、s2、s3、s4、s5、s6、s7 七段。
2. `/Users/ivanchang/financial-analysis-bot/.dd_build/runs/TSM_20260916/prose_B.html`：依序含 s8、s9、s10、s11、s12、decision 六段；s85 若觸發（見散文卡條件）併入這一檔。

每段前面獨立一行標記 `<!-- SID:sX -->`（`decision` 段寫 `<!-- SID:decision -->`），緊接該段完整外層元素，格式與每段固定四塊（`<h2>`／`<p class="lead">`／`<ul class="pts">`／`<div class="mach">`）見散文卡 §6。表格注入標記依散文卡 §3 放在對應位置，未放則系統退到段尾。

內部下筆順序、每章寫什麼與對應 judgment 欄位、篇幅預算、AI 痕跡與機器語言洩漏禁忌，全部在散文卡（`prose_card.md`）正文，動筆前先過一遍。
...
- WebSearch／WebFetch、Read 任何 `docs/dd/`、重讀自己剛寫過的 `/Users/ivanchang/financial-analysis-bot/.dd_build/runs/TSM_20260916/prose_A.html`／`/Users/ivanchang/financial-analysis-bot/.dd_build/runs/TSM_20260916/prose_B.html`。
...
- `audit.html`（4392B）→ `decision` 段 `<!-- AUDIT -->`：決策矩陣稽核表（audit_rows 非空才有）
- `e12.html`（2715B）→ `decision` 段 `<!-- E12 -->`：監測與觸發器表
...
decision       4000-5000        2715           2000-2285      3 條（2–6 內）  
