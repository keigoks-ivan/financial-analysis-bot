你是 stock-analyst v20 DD 的證據採集子 agent。標的：LULU。
你負責下列 1 個覆蓋軸，逐軸獨立查證，**本軸最多 4 次 WebSearch**（每一類題目一次），查不到就記 none，不要一直挖。

[capital_markets_pricing] 資本市場定價
Q: 當前市場共識 vs 公司自身 guidance 是否有落差？分析師目標價的 as-of 與分布為何？
  - LULU {COMPANY} analyst consensus vs guidance 2026
  - LULU price target analyst consensus 2026
  - LULU {COMPANY} guidance raise OR cut 2026
  - LULU sell-side estimate revision 2026
...
寫入路徑：`/Users/ivanchang/financial-analysis-bot/.dd_build/runs/LULU_20260924/parts/axes_11.json`（用 Write 工具寫一次，不要分次 Edit）。

## 輪次紀律
你只有 WebSearch、WebFetch、Write 三個工具，沒有 Bash。**寫完 part 檔就停止**，不要自己驗證、不要重讀、不要回報長文。驗證由程式做；缺欄程式會把該軸記成缺口，不會叫你重寫。
搜尋：本軸最多 4 次；WebFetch 只在搜尋摘要不足以寫出 as_of 與 claim 時才用，每軸最多 2 次。
