...
1. RS 加速：`rs21 > 0` 且 `rs_accel ≥ 20`（v2，見下方版本註記）
2. 曾明顯回檔：`pullback_pct ≤ −25`（v2）
...
**狀態標籤**：`rs_accel ≥ 25` 且 `e21_up` → 「偏加速中」；否則「逐漸轉強」（v2，隨 `rs_accel_min` 10→20 同步由 20 升至 25，讓標籤仍代表閘檻之上的頂層，而非只是「剛過閘」）。

**v2 版本註記（2026-09-08 二次校正，`DEFINITION_VERSION` v1→v2）**：持有人回饋 v1 名單（今日 51 檔）比原帖 ~15 檔規模寬太多；用同一批下載資料對 `rs_accel_min`∈{15,20}×`pullback_min_pct`∈{−15,−20,−25} 六組合掃描今日 `n_full` 與 250 日 `n_full` 中位數，取「今日 n_full 落在 15–25 且 250 日中位數 ≥5」的最嚴組合＝`rs_accel_min=20`／`pullback_min_pct=−25`（該點在整張網格上兩軸皆最嚴，直接合格不需 tie-break）。門檻改變即升版：history.json 整段重算窗口內全部改標 v2，窗口外舊列維持原版號。
...
```json
{
  ...
  "benchmark": "QQQ",
  "params": {"rs_accel_min": 20, "rs21_min": 0, "pullback_min_pct": -25, "dist_high_max_pct": -8, "range_pos_min": 0.7, "adv_min_usd": 20000000, "price_min": 5, "accel_label_min": 25},
...
5. **條件說明**：六條件逐條白話＋數字（例：「①相對強度在加速：近 21 日相對 QQQ 的報酬，比近 63 日的高出 20 個百分點以上（v2），而且近 21 日本身要贏 QQQ」）。附「怎麼算」折疊區放公式與母體來源、as-of、覆蓋率。
