...
- 紅燈＝PEG >2.0，或 PE NTM 相對五年均倍數 >1.75，任一個紅就紅。紅燈不進池。（原定 1.5 沿用 dd-screener 舊警示旗標；2026-09-18 持有人看過分布後改 1.75：今日 39 檔合格名字中只有 DELL 1.74 在 1.5–1.75 之間。）
...
- ③b 太貴不入池（同日追加）：其他資格全過、只有估值紅的名字列在 ③ 之後，給持有者看燈號與壞訊號；估值紅＝不加碼，不是賣出訊號。`grp_score()` 新欄 `pass_ex_valuation`。
