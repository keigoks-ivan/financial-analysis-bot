...
### 5.7 Email 版型標準（2026-09-03 持有人：「寄信的內容格式要美觀易讀」；市況判讀摘要信與每週選股結果信共用）
- **HTML 信**（`html_body: file://…`），同時附純文字 `body` 作備援。單欄、寬 600px 置中、外底色 `#f6f5f2`、內容白卡（1px `#e6e2d8` 邊、圓角 8）、系統字型（`-apple-system, "PingFang TC", "Noto Sans TC", "Segoe UI", sans-serif`）、正文 15px／行高 1.65、深字 `#1c1c1c`、灰字 `#6b6b6b`。**全部 inline style，無外部 CSS／字型／JS／圖片**；顏色全部明寫（深色模式安全）。
- **結構**：①頂欄深藍 `#0f1f3d` 白字：信件名稱＋日期；②「一分鐘版」三個要點（bullet，粗體開頭）；③三顆大數字磚（表格三格，手機下改單欄：用 `width:100%` 的巢狀 table 或 `display:inline-block;min-width:180px`）；④各節：節標題小字金 `#8a6d1f` 字距 0.08em 全大寫英文副標＋中文主標，表格斑馬列 `#faf9f6`、數字靠右、表頭灰底；⑤方向／狀態色票（pill，圓角 999、字 12px）：轉多／新進／通過 `#e8f3ea`／字 `#1f6b3a`；轉空／離席／失敗 `#fbe9e7`／字 `#b3261e`；中性／留任／累積中 `#f0eee9`／字 `#5a5a5a`；⑥底部：站內連結按鈕（深藍底白字）＋一行免責（描述器、非投資建議）。
- **文案**：外資券商白話、全形標點、術語首現括號；每節不超過 8 列，多的寫「其餘見網站」；數字帶 as_of。
- **驗收**：產出的 HTML 在本機 Chrome 開啟截圖檢視（600px 與 375px 寬各一張），Gmail 網頁版可讀（無 `<style>` 依賴）。
