...
4. **Narrative-led, tables-supported（v1.6 反轉）**：章節以**敘述為主幹**，表格用於支撐關鍵數據。<strong>敘述比例 50-60%</strong>（從 v1.5 &lt; 30% 調整），每張表格必須有「前言 + 後解讀」：
   - **前言（1-2 句）**：這張表要看什麼、為何重要
   - **表格本體**：數據
   - **後解讀（2-3 句，強制）**：「這代表什麼 / 趨勢方向 / 投資意義」
   - 小表格（≤ 5 項）應優先改寫為 prose；大比較表（≥ 7 項）才保留
   - 違反 → QC-I24 fail 返工
...
每章分**兩層**（v1.6：完整層改為敘述主導）：

```
┌─────────────────────────────────────────────┐
│ 第 1 層：完整層（Depth，敘述主導 50-60%）       │
│ - 敘述段落（3-5 段）解釋趨勢、context、含義   │
│ - 1-2 張表格（支撐關鍵數據 / 多玩家比較）       │
│ - 每張表前言 1-2 句 + 後解讀 2-3 句（強制）     │
│ - 小資料（≤ 5 項）寫成 prose，不做表格         │
│ - 每個數字附 source（T1 優先）                 │
│                                              │
│ ➜ 目標：非專業讀者讀完敘述即能理解，不需解表    │
├─────────────────────────────────────────────┤
│ 第 2 層：洞見層（Insight）                    │
│ - 💡 2-3 條 Insight Bullet（非顯而易見）       │
│ - 引用第 1 層的具體事實 → 導出結論            │
│ - 對投資的具體意義（完整敘述，非 bullet 堆）   │
│                                              │
...
- **敘述主導（v1.6 反轉）**：章節整體**敘述 50-60%**，表格用於支撐具體數據；每張表格必須有**前言（1-2 句）+ 後解讀（2-3 句，強制）**
- 小資料（≤ 5 項）優先改寫為 prose；只在 ≥ 7 項或多維度比較時才用表格
- 表格後解讀要回答：「這代表什麼 / 趨勢方向 / 投資意義」

### 完整度（Depth）最低門檻（QC-I15，v1.6 敘述比例上修）

每個數據章節必須達到下列最低深度，否則返工。**v1.6 調整重點**：表格數**上限化**（太多表反而傷閱讀），敘述段落**最低門檻上調**。

| 章節 | 建議表格數（上限）| 最低 data point | **最低敘述段落（v1.6 調高）**|
|:---|:---|:---|:---|
| §1 定義與邊界 | 1-2（in-scope + out-of-scope）| 各 ≥ 5 項 | **4 段**（為何這樣切 + 每類 in/out 背景 + 灰色地帶解讀）|
| §2 技術演進 + 歷史類比 | 2-3（演進 + 歷史類比）| 時間點 ≥ 5、類比 ≥ 2 | **5 段**（每階段 1 段 + 每歷史類比 1 段）|
| §3 技術棧拆解 | 1-2 | 子技術 ≥ 5、瓶頸 ≥ 3 | **4 段**（每關鍵子技術 1 段含趨勢解讀）|
| §4 市場規模 | 1-2（TAM + 預估偏差）| 各年 ≥ 3 層 | **4 段**（多口徑對照敘述 + CAGR 假設 + 預估可信度 + TAM 偏差歷史）|
| §5 Value chain | 1 SVG + 1 表 | 每段 ≥ 3 玩家 | **5 段**（上/中/下利潤邏輯 + 議價權轉移敘述 + 毛利差異解讀）|
| §6 玩家矩陣 | 2-3（A/B/C 類玩家）| 每類 ≥ 3 公司 | **4 段**（市佔變動敘述 + 在位者 vs 挑戰者 + non-obvious 受益者理由）|
| §7 Unit Economics + ASP | 1-2（財務對照 + ASP 趨勢）| ≥ 5Y 歷史 + 2Y 預估 | **3 段**（ASP 動態 + 抗 commoditization 解讀）|
...
### QC-I24｜id-meta JSON 硬性 schema（v1.7 新增，validator 強制）

`scripts/validate_id_meta.py` 在 git pre-commit hook 跑 + GitHub Actions 雙重檢查；違規 → push 失敗。LLM 生成 HTML 前必須自我驗證 id-meta JSON。

**oneliner 欄位**：
- **硬上限 200 chars**。超過 → validator 拒絕。範例壓縮：「主題核心 ／ 3 個關鍵數字 ／ binary catalysts ／ 一句 thesis」分 4 段，全形分號 `；` 連接。
- 軟目標 ≤ 120 中文字。

**4 個 enum 欄位 — 只能用以下值，禁止自由發揮**：

| 欄位 | 允許值（白名單） |
|:---|:---|
| `thesis_type` | `event-triggered` ｜ `mixed` ｜ `structural` |
...
**禁止寫的範例**（這些都會被 validator 退件）：
- `growth_phase: "phase_iii_acceleration_with_oral_inflection"` → 用 `mid` 或 `late`
- `growth_phase: "early-mid"` → 必須二擇一，用 `mid`（因為跨期默認偏成熟一邊）
- `value_chain_position: "platform-layer"` → 用 `cross-tier`
- `value_chain_position: "transportation"` / `"distribution"` → 用 `downstream`
- `industry_structure: "K-shape oligopoly + ULCC death"` → 純標籤化，用 `oligopoly`
- `industry_structure: "duopoly + AI disruption"` → 用 `duopoly`
- `thesis_type: "structural+event"` → 用 `mixed`

**JSON 字串內禁止 HTML**：
- 嚴禁 `<a href="...">`、`<strong>`、`<br>` 嵌入 JSON value（典型出包：BoomerTravelSupercycle 在 role 欄位寫 `<a href="...">ID_xxx</a>` 直接觸發 JSON parse error）。
- 跨 ID 引用改純文字檔名：「詳見 ID_AirlineLoyaltyRepricing」。
- 換行用 `；` 或 `\n`，不要嵌 `<br>`。

**生成後自驗腳本**：
```bash
python3 -c "import json,re; t=open('docs/id/ID_X_YYYYMMDD.html').read(); m=re.search(r'<script id=\"id-meta\"[^>]*>(.*?)</script>', t, re.DOTALL); d=json.loads(m.group(1)); assert len(d['oneliner']) <= 200, f'oneliner {len(d[\"oneliner\"])} chars > 200'; allow={'thesis_type':{'event-triggered','mixed','structural'},'growth_phase':{'early','mid','late','declining'},'value_chain_position':{'upstream','midstream','downstream','cross-tier'},'industry_structure':{'monopoly','duopoly','oligopoly','fragmented'}}; [print(f'{k}={d[k]} VIOLATION') for k in allow if k in d and d[k] not in allow[k]]"
```

寫 HTML 前先在 LLM 內部對照白名單；若描述複雜，**強制 snap to 最近的 allowed value**，不要造詞。
