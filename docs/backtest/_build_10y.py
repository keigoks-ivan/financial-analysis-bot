...
SYS = [
    ...
    ("QQQ+SMH 六狀態機・三態無 Grid", "/backtest/six_state/", "ch8",
     "+22.77%", "-28.48%", "1.14", "0.80", "$7.77M", "+14.89%", "-37.78%", TAG["atk"]),
    ("QQQ 六狀態機・五態＋Grid・實盤", "/backtest/six_state_v1r1/", "v1r1",
     "+21.32%", "-26.79%", "1.10", "0.80", "$6.90M", "+14.69%", "-50.28%", TAG["live"]),
    ...
    ("SPY/ACWX/AGG 雙動能月切換", "/backtest/gem/", "gem",
     "+9.85%", "-21.54%", "0.72", "0.46", "$2.56M*", "+8.56%", "-21.54%", TAG["def"]),
    ("SPY/QQQ 雙軌多空（否決）", "/backtest/dual_track/", "dual",
     "+9.32%", "-19.61%", "0.73", "0.48", "$2.44M", "+4.38%", "-38.72%", TAG["fail"]),
...
MULTI = [
    ...
    ("📈 35 檔 ETF 跨資產趨勢（EMA＋突破）", "/backtest/clenow/", None,
     "+12.61%", "-42.14%", "0.57", "0.30", "$3.28M", "+13.14%", "-47.34%", TAG["ma"]),
]
...
SCATTER = [
    ...
    ("QQQ+SMH 六狀態", 28.48, 22.77, "#d32f2f"),
    ("QQQ 六狀態實盤", 26.79, 21.32, "#b45309"),
...
def render() -> str:
    ...
    rows += ('<tr><td colspan="7" style="background:#ecfdf5;font-size:.75rem;font-weight:600;'
             ...
             '🇹🇼 台股(2330/0050 E3・舊實倉對照 · NT$ · 不同市場/幣別，僅參照)</td></tr>'
             '<tr><td><strong><a href="/backtest/long_track_tw/">2330/0050 E3（舊實倉・對照）</a></strong>'
             '<br><span style="font-size:.72rem;color:var(--muted)">50/50 · {W40·W52·TSMOM} 各⅓ · '
             '2026-06-23 採用，2026-07-18 起降為對照(實單改用 W52 × 自適應波動率) · NT$ · 近 10 年(2016~) · 詳見 '
             ...
             '<td><span class="tag" style="background:#475569;color:#fff;border:1px solid #475569">'
             '舊實倉 · 對照</span></td></tr>')
    rows += "".join(
