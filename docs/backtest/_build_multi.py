...
LIVE_CARD = {
    ...
    "cagr": "+15.25%", "mdd": "-14.98%", "sharpe": "1.12", "calmar": "1.02",
...
SYSTEMS = [
    ("L4 採用候選", TEAL, [
        ("🧩 組合系統:STX50 + 商品 Sleeve（80/20）", "/backtest/turtle_adopt/",
         "商品 sleeve 疊上股票核心 · Calmar 0.66→1.02、MDD -21.9%→-15.0% · 參數高原 35/35 + 日線時間架構雙過審 · "
         "sleeve 採用待 L4 · 即時頁 <a href='/turtle-sleeve/'>/turtle-sleeve/</a>",
         "+15.25%", "-14.98%", "1.12", "1.02", "有效交換", tag_cand()),
        ("🐢 商品/債/匯/股 唐奇安突破（55/20）", "/backtest/turtle/",
         "USO/GLD/DBA/FXE/TLT/SPY · 55/20 突破(2007 起)· 2026-06 拆解:CAGR=3.3× 曝險的槓桿效果非過擬合;"
         "CMDTY 腿 L3 通過(+20% → 組合 Calmar 1.03),sleeve 即取自此",
         "+22.48%", "-38.12%", "0.73", "0.59", "支配候選", tag_cand()),
    ]),
    ("分散參照(資產池不同,僅供對照)", GREY, [
        ("🛡️ 跨資產防守趨勢（SPY/TLT/GLD/DBC）", "/backtest/crossasset_defense/",
         ...
         "Calmar 0.62/Sharpe 0.92 全勝 GEM/Clenow、對 QQQ 月相關僅 0.28 → 真分散補位",
         "+5.86%", "-9.39%", "0.92", "0.62", "有效交換", tag_ref()),
        ("📈 35 檔 ETF 跨資產趨勢（EMA＋突破）", "/backtest/clenow/",
         "35 ETF × 7 類別 · EMA50/100 + 突破 · 經典 Clenow 趨勢配置",
         "+13.28%", "-44.11%", "0.69", "0.30", "混合", tag_ref()),
    ]),
]

# benchmarks (single-asset US B&H) — shown for SCALE reference only
BH = [
    ("50/50 SPY/QQQ B&H", "+13.52%", "-53.66%", "0.25"),
    ("QQQ Buy & Hold", "+15.88%", "-53.40%", "0.30"),
    ("SPY Buy & Hold", "+11.01%", "-55.19%", "0.20"),
]

SCATTER = [
    ("🧩 組合 80/20(候選)", 14.98, 15.25, TEAL),
    ("🐢 唐奇安突破", 38.12, 22.48, "#0f766e"),
    ("🛡️ 跨資產防守", 9.39, 5.86, "#16a34a"),
    ("📈 跨資產趨勢", 44.11, 13.28, "#6366f1"),
    ("B&H 50/50 SPY/QQQ", 53.66, 13.52, "#94a3b8"),
    ("B&H QQQ", 53.40, 15.88, "#1565c0"),
    ("B&H SPY", 55.19, 11.01, "#757575"),
]
...
<div class="acard-note">
  ...
  <ul>
    ...
    <li>商品 sleeve 對 STX50 月相關 ≈ -0.03、三次危機窗全正,<b>+20% 權重把組合 Calmar 0.64→1.03</b> — 採用與工具(需期貨)待 L4。</li>
...
<div class="page-hdr"><div class="container">
  ...
  <div class="tabs"><a href="/backtest/">🇺🇸 美股</a><a href="/backtest/tw/">🇹🇼 台股</a><a class="on" href="/backtest/multi/">🧩 多資產</a></div>
</div></div>
...
<div class="note"><b>誠實歸因:</b>唐奇安突破 CAGR +22.48% 是 3.3× 曝險的<b>槓桿效果</b>,非規則本身更強;MDD -38% 也隨之放大。
真正可用的是它的<b>商品腿與股票核心低相關</b> — 取 20% 疊進 STX50 → 組合 Calmar 0.64→1.03。跨資產防守腿則用低相關換分散,CAGR 最低但 Calmar/Sharpe 全勝 GEM/Clenow。</div>
</div>
