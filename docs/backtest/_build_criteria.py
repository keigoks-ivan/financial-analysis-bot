...
SYSTEMS_20Y = [
    ...
    ("GEM 雙動能", "/backtest/gem/", 8.56, -21.54, 0.68, 0.40, True, ""),
    ("六狀態機 v1.1", "/backtest/six_state/", 14.89, -37.78, 0.86, 0.39, True, ""),
    ("六狀態機 v1.0r1 實盤", "/backtest/six_state_v1r1/", 14.69, -50.28, 0.81, 0.29, True, "壓力路徑 -77%"),
    ...
    ("🐢 Turtle(多資產)", "/backtest/turtle/", 7.72, -52.67, 0.39, 0.15, True, "2026-06 修正：停損成交假象改正，全額版為平庸高回撤系統"),
    ("📈 Clenow(多資產)", "/backtest/clenow/", 13.14, -47.34, 0.60, 0.28, True, "2026-09-05 修正：進場改用 MTM 帳戶權益 sizing(非僅實現資金)"),
]
...
SYSTEMS_10Y = [
    ...
    ("六狀態機 v1.1", 22.77, -28.48, 1.14, 0.80, "warn", "CAGR 進可疑帶(>20%)— 無 2008 的視窗"),
    ...
    ("GEM 雙動能", 9.85, -21.54, 0.72, 0.46, "pass", "合格"),
...
DOMINANCE = [
    ...
    ("六狀態機 v1.1", "/backtest/six_state/", "QQQ B&H",
     (14.89, -37.78, 0.86, 0.39), (15.88, -53.40, 0.78, 0.30), "trade",
    ...
    ("GEM 雙動能", "/backtest/gem/", "SPY B&H(同期)",
     (8.56, -21.54, 0.68, 0.40), (11.17, -50.78, 0.73, 0.22), "mixed",
     "同為「砍尾部、波動效率打平」型"),
    ("📈 Clenow(多資產)", "/backtest/clenow/", "等權多資產 B&H",
     (13.14, -47.34, 0.60, 0.28), (10.83, None, None, None), "mixed",
     "報酬軸贏 2.3pp 但 MDD -47% 過深(L1 不合格);資產池不同僅參照;2026-09-05 sizing 改用 MTM 帳戶權益後波動與回撤放大"),
    ("六狀態機 v1.0r1 實盤", "/backtest/six_state_v1r1/", "QQQ B&H",
     (14.69, -50.28, 0.81, 0.29), (15.88, -53.40, 0.78, 0.30), "weak",
...
<div class="section-sub">下游所有分析的前提。任何一條不過，該系統的數字視為無效，先修數再談評估。前五條來自 2026-06 修正週期；第 6~8 條為 v3 新增(可證偽性與預註冊承襲日內研究線，移植性來自 W52 多標的實驗)。</div>
<div class="card">
<ol class="issue-list">
...
目前無系統觸發「支配候選 ⚠」(兩軸全贏);Turtle 修正前曾以停損成交假象兩軸全贏(舊 ~22.5% CAGR),
...
<ol class="issue-list">
  <li><strong>危機期相關性優先於全期相關性</strong> — 全期低相關但 2008/2020/2022 同步 -30% 的 sleeve 沒有分散價值；
      相關性必須分段報：全期 / 各危機窗（2008/2011/2018/2020/2022）/ 高利率盤整段。</li>
  ...
  <li><strong>角色決定評分權重</strong> — 防守位(核心)看 Calmar 與危機行為；進攻位看 CAGR 與 regime 集中度(可疑帶);
      ...
      站內現行角色(2026-07-18 起)：實單攻擊位 = W52 × 自適應波動率 150%(美+台)、舊實倉對照 = STX50(SMH/QQQ)／E3(2330/0050)、
...
<div class="verdict">
  ...
  重審：滾動 3 年 Calmar 低於 SPY/QQQ 版，或半導體相對 QQQ 連 12 個月落後)— <strong>舊實倉，2026-07-18 起改對照(實單改用 W52 × 自適應波動率 150%)</strong>;
  Ensemble E3 = 股票趨勢核心(2026-06-11;Ch12 基準轉為對照組；重審：下一次盤整 + 下一次快崩兩個事件後)—
  <strong>已採用，待上實倉</strong>。採用 = 有記錄的決策；實倉 = 資金實際在跑，兩個狀態分開標示。<br>
  <strong>角色配置(2026-07-18 起)：</strong>實單攻擊位 = W52 × 自適應波動率 150%(美+台);舊實倉對照 = STX50／E3;六狀態機 v1.1 L1 不合格但可用部位大小納入
  ...
  與 STX50 月相關 -0.01、五個危機窗四正一負（2008 GFC +19.8%、2020 COVID +13.3%、2022 熊市 +14.2%、2018 Q4 +9.7%、
  2011 歐債 -7.2%；量級遠小於舊頁停損成交假象下的 +35~62%），+20% 權重使 STX50 核心組合 Calmar 0.65 → 0.85、MDD -21.9% → -16.3%,
  但 CAGR 幾乎不動(不再是舊頁誤植的 0.64 → 1.03)；+20% 靜態 GLD 對照已達 0.81，為正式替代方案（見採用評估頁 Dumb 對照）。
