...
v3.1 audit (2026-06-29) — correctness/comparability fixes:
  * SHY→BIL cash splice rescaled at the junction (removed phantom +22.16%
    one-day cash return on 2007-05-30): six_state 14.77→14.47%, e3 11.47→11.36%,
    v1r1 14.58→14.35%; fully-invested systems (ch12, B&H) unchanged.
  * W52 slope filter MDD/Sharpe/Calmar moved to a reconstructed DAILY equity
    (month-end sampling hid the 2008 hedge-leg drawdown): MDD -18.96%(2022) →
    -22.13%(2008), Sharpe 1.01→0.86, Calmar 0.57→0.48; "near-dominant" retracted.
  * GEM transaction cost folded into monthly returns (CAGR/Sharpe were gross,
    terminal/MDD net): 8.95→8.63%; Sharpe switched to rf=0 site convention
    (was excess-of-T-bill); trailing partial month dropped. GEM MDD stays
    month-end (monthly system; no daily data to reconstruct) — flagged.
...
RET = {
    "ch8":   [-0.12, 13.76, -28.61, 34.17, 16.87, -6.39, 11.38, 30.18, 21.50, 4.98, 7.46, 31.52, -1.83, 33.31, 31.54, 31.19, -23.24, 42.28, 30.26, 24.15, 27.84],
    "v1r1":  [6.63, 17.23, -37.81, 45.24, 16.86, -5.35, 13.15, 30.83, 18.22, 6.20, 5.53, 29.96, 0.38, 30.40, 41.67, 26.63, -24.73, 52.34, 25.03, 22.65, 15.87],
    ...
    "smh":   [-4.34, 4.94, -5.96, 21.50, 2.39, -7.94, -2.87, 29.73, 18.68, -4.83, 8.94, 34.99, -0.79, 22.24, 44.53, 35.93, -16.69, 37.38, 29.59, 24.25, 34.89],
    "dual":  [5.47, 2.83, 9.44, -0.21, -4.79, -20.27, 5.13, 27.96, 8.67, -18.22, -1.05, 24.50, 3.40, 1.96, 7.67, 19.91, -14.50, 13.51, 17.02, 6.55, 11.82],
    "gem":   [16.48, 5.74, 4.65, 7.67, 12.85, -3.61, 5.66, 25.19, 16.96, -5.22, 11.95, 17.37, -13.70, 16.45, 1.24, 24.86, -12.23, 1.96, 22.52, 9.99, 8.08],
    "w52":   [13.13, 5.33, -0.24, 3.62, 14.84, -1.43, 14.17, 29.00, 14.56, -3.75, 7.97, 20.78, -6.81, 22.27, 15.12, 30.51, -17.09, 19.06, 25.59, 11.36, 8.86],
    "qqq":   [4.81, 18.81, -40.79, 48.25, 18.41, 1.89, 15.89, 32.43, 20.12, 9.77, 9.41, 31.49, -1.85, 38.41, 45.97, 29.24, -33.22, 55.91, 27.74, 21.01, 17.80],
...
GROUPS = [
    ...
    ("合格候補(通過 L1 門檻，未採用)", [
        ("SPY/QQQ 三訊號趨勢集成（核心）", "/backtest/long_track_ensemble/",
         ...
         "+11.36%", "-21.15%", "0.87", "0.54", "trade", "$9.02M", TAG["def"]),
        ("SPY/AGG 週線斜率擇時（防守）", "/backtest/slope_filter/",
         "SPY/AGG 非對稱進出場 · 2026-06 審計：MDD/Sharpe 改還原日線基準(月底曾低估 −18.96%/2022;真實日線 −22.13%/2008 避險腿股債雙殺)· 仍勝 B&H 風險調整，惟非近支配；規則不可移植(QQQ 失敗/SMH 輸 Faber)",
         "+10.62%", "-22.13%", "0.86", "0.48", "trade", "$7.81M*", TAG["def"]),
        ("SPY/QQQ 週線長軌趨勢", "/backtest/long_track/",
         "50/50 SPY/QQQ · W52/104/250(2026-06 warmup 修正)",
         "+9.75%", "-20.08%", "0.81", "0.49", "trade", "$6.70M", TAG["def"]),
        ...
        ("SPY/ACWX/AGG 雙動能月切換", "/backtest/gem/",
         ...
         "+8.56%", "-21.54%", "0.68", "0.40", "mixed", "$5.39M*",
    ...
    ("L1 不合格但角色可用(部位大小解，不豁免門檻)", [
        ("QQQ＋SMH 六狀態機・三態無 Grid", "/backtest/six_state/",
         ...
         "+14.89%", "-37.78%", "0.86", "0.39", "trade", "$17.06M", TAG["atk"]),
        ("QQQ 六狀態機・五態＋Grid・實盤", "/backtest/six_state_v1r1/",
         "QQQ+IB01 · 五狀態+Grid · 2000-02 壓力路徑 -77% · 升級 v1.1 待決(2026-07 訊號改次日開盤成交)",
         "+14.69%", "-50.28%", "0.81", "0.29", "weak", "$16.46M", TAG["live"]),
    ]),
    ("🇹🇼 台股(2330/0050 E3・舊實倉對照 · 不同市場/幣別，不與美股同尺)", [
    ...
    ("🔬 實驗(未採用)", [
        ("50% A0＋50% D1 長波段舒服度組合", "/backtest/a0_d1_comfort/",
        ...
        ("SPY/QQQ/SMH 週線 Supertrend（ATR 10×3）", "/backtest/supertrend/",
         ...
         "+8.14%", "-17.77%", "0.78", "0.46", "trade", "$4.95M", TAG["exp"]),
    ...
    ("✗ 已否決", [
        ("SPY/QQQ 雙軌多空・短70＋長30（否決）", "/backtest/dual_track/",
         ...
         "+4.38%", "-38.72%", "0.37", "0.11", "dominated", "$2.40M", TAG["fail"]),
        ...
        ("SMH/QQQ 出場法斜率切換（否決）", "/backtest/exit_switch/",
         ...
         "—", "—", "—", "—", "na", "—", TAG["fail"]),
        ("F70 換 CTA 腿・10 市場多空期貨（兩輪否決）", "/backtest/futures_leg/",
         "把 F70 的 D1X 腿(TLT/GLD/DBC 只做多)換成自建多空期貨腿 · 第一輪使用者日線規則 1/3 過、"
         "第二輪 sg 20/120 交叉 2/3 過但 MDD 更深 · 兩輪皆未勝過 D1X",
         "—", "—", "—", "—", "na", "—", TAG["fail"]),
    ]),
]

BH_ROWS = [
    ("QQQ Buy & Hold", "+15.91%", "-53.40%", "0.78", "0.30"),
    ("SPY Buy & Hold", "+11.04%", "-55.19%", "0.64", "0.20"),
    ("50/50 SPY/QQQ B&H", "+13.55%", "-53.66%", "0.73", "0.25"),
]

# 分期間 CAGR (全期 / 近15y / 近10y / 近5y) — refreshed 2026-06-12; w52 = 引擎 v2
PERIOD_CAGR = [
    ("QQQ+SMH 六狀態機", "#d32f2f", "+14.89%", "+18.76%", "+22.77%", "+21.61%"),
    ("QQQ 六狀態機・實盤", "#b45309", "+14.69%", "+17.98%", "+21.32%", "+18.30%"),
    ...
    ("SMH/QQQ 進攻趨勢(採用)", "#b45309", "+13.84%", "+17.91%", "+25.74%", "+24.48%"),
    ("雙動能月切換", "#f57c00", "+8.56%", "+8.86%", "+9.85%", "+9.73%"),
    ("SPY/AGG 斜率(審計 v2)", "#0891b2", "+10.62%", "+12.04%", "+13.16%", "+11.27%"),
    ("SPY 週線 Supertrend(實驗)", "#be185d", "+8.14%", "+9.49%", "+11.86%", "+9.10%"),
    ("SPY/QQQ 均值回歸(實驗)", "#ca8a04", "+3.12%", "+3.14%", "+3.29%", "+5.64%"),
    ("QQQ B&H", "#1565c0", "+15.91%", "+19.81%", "+21.78%", "+16.65%"),
    ("SPY B&H", "#757575", "+11.04%", "+14.46%", "+15.41%", "+13.32%"),
]

SCATTER = [
    ("SPY/QQQ RSI2 回歸", 16.46, 3.12, "#ca8a04"),
    ("SPY 週線 Supertrend", 17.77, 8.14, "#be185d"),
    ("QQQ+SMH 六狀態", 37.78, 14.89, "#d32f2f"),
    ("QQQ 六狀態實盤", 50.28, 14.69, "#b45309"),
    ("SPY/QQQ 長軌", 20.08, 9.75, "#2e7d32"),
    ("QQQ 長軌純攻", 25.37, 10.83, "#16a34a"),
    ("SPY/QQQ 集成", 21.15, 11.36, "#d97706"),
    ("SMH/QQQ 進攻", 21.87, 13.84, "#b45309"),
    ("SPY/QQQ 雙軌", 38.72, 4.38, "#7c3aed"),
    ("雙動能", 21.54, 8.56, "#f57c00"),
    ("SPY/AGG 斜率", 22.13, 10.62, "#0891b2"),
    ("QQQ B&H", 53.40, 15.91, "#1565c0"),
    ("SPY B&H", 55.19, 11.04, "#757575"),
]
...
<div class="note">
  ...
  <strong>W52 斜率(2026-06 審計：① 修正 ~4 週 MA 對齊過時；② MDD/Sharpe 改還原日線基準 — 月底取樣曾低估 MDD 為 −18.96%/2022,真實日線 −22.13%/2008,Sharpe 1.01 → 0.86、Calmar 0.57 → 0.48,「近支配」撤回)</strong>。
  GEM 已完成審計(成本計入報酬 8.95 → 8.63%、Sharpe 改 rf=0;MDD 仍為月底基準)。
  全系統並另修正 SHY→BIL 現金拼接的 2007-05-30 幻影 +22% 跳空。
</div>
</div>
...
<ol class="issue-list">
  <li><strong>規則優勢 = 規則結構 × 標的回撤-修復幾何，不可假設可移植。</strong>
      W52 同規則零調整移植：SPY 勝 B&H 風險調整 → QQQ 失敗(無效交換)→ SMH 輸 Faber。
...
<ul style="font-size:.85rem;color:#374151;line-height:1.9;padding-left:1.2rem">
  <li><strong>全部美股系統(六狀態機、Long Track 家族、雙軌多空、GEM、W52 斜率、做空系統)</strong>:截至 2026-06,
      ...
      雙軌多空為 2026-06-12 修正基準重跑；W52 為 2026-06 審計引擎 v2(W-FRI/ME 對齊修正)。</li>
  <li><strong>2026-06-29 審計修正</strong>:① SHY→BIL 現金拼接在接點重標價位，移除 2007-05-30 的幻影 +22% 單日現金報酬
      (影響持現金的系統：六狀態 CAGR 14.77→14.47%、集成 11.47→11.36% 等);
      ② GEM 交易成本計入月報酬，CAGR 與期末淨值一致(8.95→8.63%),Sharpe 改 rf=0 與全站同尺；
      ③ W52 斜率的 MDD/Sharpe/Calmar 改以<strong>還原日線淨值</strong>量測 — 月底取樣曾把 MDD 低估為 −18.96%(2022),
      真實日線為 −22.13%(2008 避險腿股債雙殺),Sharpe 1.01→0.86、Calmar 0.57→0.48,<strong>不再為「近支配」</strong>;
      ④ Supertrend 交易數改計往返(原為進+出 ≈ 2×)。</li>
  <li>* GEM 與 W52 斜率規格定義初始資金 $100,000;表中 $M 值為換算 $1M 起始的等效值。
      <strong>GEM 的 MDD 仍為月底基準</strong>(月度決策系統，無逐日資料還原),與其他日線系統並列時其回撤偏淺，須留意。</li>
  <li>逐年報酬與圖表使用年頻資料；精確 MDD / Sharpe 以各系統詳頁計算為準(W52 已改日頻；GEM 為月頻)。</li>
</ul>
</div>
</details>
</div>
