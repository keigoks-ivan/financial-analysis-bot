...
SWING = [
    ("採用", GREEN, [
        ("2330/0050 三訊號趨勢集成（E3）", "/backtest/long_track_tw/",
         ...
         "+20.19%", "-23.75%", "0.85", "有效交換", tag_adopt()),
    ]),
    ("0050 單檔趨勢候補", GREY, [
        ...
        ("0050 六狀態機（移植）", "/backtest/tw_0050_six/",
         ...
         "+14.14%", "-23.73%", "0.60", "有效交換", tag_cand()),
...
BH = [
    ("2330/0050 50/50 B&H", "+24.28%", "-39.47%", "0.62"),
    ("2330 台積電 B&H", "+30.18%", "-44.80%", "0.67"),
    ("0050 B&H(已修復分割)", "+18.05%", "-33.83%", "0.53"),
    ("^TWII 加權指數 B&H(價格指數)", "+12.26%", "-31.63%", "0.39"),
]
...
SWEEP = [  # 2330 weight ladder (E3)
    ("0 / 100", "+14.78%", "-25.13%", "0.59"),
    ("25 / 75", "+17.53%", "-22.11%", "0.79"),
    ("50 / 50", "+20.19%", "-23.75%", "0.85"),
    ("75 / 25", "+22.76%", "-25.71%", "0.89"),
    ("100 / 0", "+25.22%", "-27.63%", "0.91"),
]

SCATTER = [
    ("2330/0050 E3(採用)", 23.75, 20.19, GREEN),
    ("0050 Ch12 長軌", 21.30, 14.39, "#64748b"),
    ("0050 六狀態", 23.73, 14.14, "#64748b"),
    ("0050 STX50", 26.69, 12.56, "#64748b"),
    ("0050 雙軌(否決)", 30.83, 12.17, RED),
    ("B&H 2330/0050", 39.47, 24.28, "#94a3b8"),
    ("B&H 0050", 33.83, 18.05, "#94a3b8"),
    ("B&H 2330", 44.80, 30.18, "#94a3b8"),
    ("B&H ^TWII", 31.63, 12.26, "#94a3b8"),
]
...
E3 2330/0050 能到 0.85,<b>領先全來自加入 2330</b>(台積電集中度),不是 E3 規則本身更強 — 見下方 2330 權重梯。
<b>補測(2026-07-02):</b>Ch12 規則的 2330/0050 50/50 組合對照首次補跑 — Calmar 0.86、Sharpe 1.18,
與 E3(0.85 / 1.16)幾乎持平(E3 的 CAGR 與 H1 較高；Ch12 的 H1 帶 0050 W250 暖身失真)。
<b>採用決策不變，待擁有者重審</b>，詳見 <a href="/backtest/long_track_tw/">詳細頁④</a>。</div>
</div>
...
<div class="section-sub">一份 TAIEX / TWVIX / TXO 資料層 ＋ 一個 Black-76 引擎，餵給多個選擇權策略的決策引擎與研究。無歷史選擇權鏈 → 決策引擎＋前瞻模擬＋有限 VRP proxy，非深歷史回測。 <a href="/backtest/tw_options/">→ 專區首頁與今日比較表</a></div>
<div class="card" style="overflow-x:auto">
<table><thead><tr><th>策略</th><th>本質</th><th>今日</th><th>Headline</th></tr></thead>
<tbody>
<tr><td><a href="/backtest/txo_vol_seller/" style="font-weight:600">台指選擇權賣方</a><br><span style="font-size:.72rem;color:var(--muted)">賣 TAIEX call 價差收租 · VRP 賣方 · 模擬實單每日自動更新</span></td><td>方向中性賣方</td><td><span style="background:var(--green-bg);color:var(--green);border:1px solid var(--green-border);padding:.08rem .4rem;border-radius:4px;font-size:.72rem;font-weight:600">GO</span></td><td>損益兩平勝率 93.1%</td></tr>
<tr><td><a href="/backtest/txo_put_seller/" style="font-weight:600">台指賣方下檔</a><br><span style="font-size:.72rem;color:var(--muted)">賣 put 收下檔權利金 · bull put spread</span></td><td>方向中性賣方</td><td><span style="background:var(--green-bg);color:var(--green);border:1px solid var(--green-border);padding:.08rem .4rem;border-radius:4px;font-size:.72rem;font-weight:600">GO</span></td><td>損益兩平勝率 91.7%</td></tr>
<tr><td><a href="/backtest/txo_iron_condor/" style="font-weight:600">台指雙賣</a><br><span style="font-size:.72rem;color:var(--muted)">同時賣兩翼 · short strangle / iron condor</span></td><td>雙邊賣方</td><td><span style="background:var(--green-bg);color:var(--green);border:1px solid var(--green-border);padding:.08rem .4rem;border-radius:4px;font-size:.72rem;font-weight:600">GO</span></td><td>獲利區間 ±30%（38,737〜52,363）</td></tr>
<tr><td><a href="/backtest/txo_covered_call_0050/" style="font-weight:600">台指 Covered Call</a><br><span style="font-size:.72rem;color:var(--muted)">0050 多頭上賣 call 收租</span></td><td>持股上賣</td><td><span style="background:var(--green-bg);color:var(--green);border:1px solid var(--green-border);padding:.08rem .4rem;border-radius:4px;font-size:.72rem;font-weight:600">GO</span></td><td>年化收租率 23.9%</td></tr>
<tr><td><a href="/backtest/txo_tail_hedge/" style="font-weight:600">台指尾端避險</a><br><span style="font-size:.72rem;color:var(--muted)">買 OTM put 崩盤保險 · 唯一買方</span></td><td>買方避險</td><td><span style="background:#fef2f2;color:var(--red);border:1px solid #fecaca;padding:.08rem .4rem;border-radius:4px;font-size:.72rem;font-weight:600">AVOID</span></td><td>年化保護成本 5.9%／年（IV 貴、買方吃虧）</td></tr>
</tbody></table>
</div>
