...
function configureMode(daily,length,saved=null){
  ...
  configureInstrument();
  text('dataMode',isStock()?'歷史個股日 K':daily?'真實分鐘行情聚合日 K':'真實逐筆資料');text('stepBtn',daily?'下一交易日 →':'＋10 秒');
...
async function loadStock(market,plan=null,saved=null){
  ...
  try{
    ...
    if(request!==loadId)return;engine=new StockReplayEngine(createStockRun(pool,days,plan,Math.random,stockActions),saved?.snapshot);configureMode(true,days,saved);
    ...
    notify(saved?'已恢復同一檔股票與同一局進度。':'已抽取 '+(hiddenIdentity()?'盲測標的':selected.name+'（'+selected.symbol+'）')+'，共 '+days+' 個交易日。買進扣現金，賣出須有持股。');
  }catch(e){loading=false;notify(e.message,true);renderControls();}
}
