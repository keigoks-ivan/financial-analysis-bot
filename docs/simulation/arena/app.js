import {ARENA_KEY,STRATEGIES,validateConfig,arenaUniverse,pickArenaPlan,runEpisode,arenaSummary,arenaPacket,readBattle,stockRules,fingerprint,arenaExplanation} from './model.js';
...
function setupHint(){const currency=$('market').value==='TW'?'TWD':'USD';$('setupHint').textContent=`這次會抽 ${$('count').value} 段行情，每段 ${$('days').value} 個交易日。每套策略各有 ${currency} ${money(Number($('capital').value))}，每次用 ${$('allocation').value}% 權益買進。按下方按鈕即可開始。`;}
if(battle){configure(battle.config);visible=battle.config.days;}
setupHint();
for(const key of keys.filter(k=>k!=='market'))$(key).onchange=setupHint;
$('beginner').onclick=()=>{const market=$('market').value;configure({market,days:120,count:1,capital:stockRules(market==='TW'?'TW':'US').capital,allocation:50,ma:60,lookback:20,band:2,stop:8});$('prediction').value='';setupHint();$('status').textContent='已套用下一場的入門設定。按「開始自動對戰」即可。';};
$('market').onchange=()=>{const market=$('market').value;$('capital').value=stockRules(market==='TW'?'TW':'US').capital;$('currency').textContent=market==='TW'?'TWD':'USD';setupHint();};
...
async function start(same){
 ...
 try{
  ...
  busy=true;$('tryNext').disabled=true;$('runProgress').hidden=false;$('progressBar').max=config.count;$('progressBar').value=0;$('settings').disabled=true;$('start').disabled=true;$('rerun').disabled=true;$('status').textContent='準備相同的行情與交易條件…';
  ...
  for(let i=0;i<config.count;i++){
   ...
   await new Promise(resolve=>setTimeout(resolve,0));episodes.push(runEpisode(pool,plan,config,actions));$('progressBar').value=i+1;
  }
  ...
  battle=result;visible=config.days;
  ...
  render();$('status').textContent='對戰完成。下面兩張解說卡會帶你看懂結果。';$('results').scrollIntoView({block:'start',behavior:'smooth'});
 }catch(e){$('status').textContent=e.message;}finally{busy=false;$('tryNext').disabled=false;$('runProgress').hidden=true;$('settings').disabled=false;$('start').disabled=false;$('rerun').disabled=!battle;}
}
$('start').onclick=()=>start(false);$('rerun').onclick=()=>start(true);
$('tryNext').onclick=()=>{if(!battle||busy)return;configure(battle.config);setupHint();return start(false);};
$('lookTrades').onclick=()=>{if(!battle)return;const guide=arenaExplanation(battle);$('episode').value=String(guide.episodeIndex);$('strategy').value=guide.strategyId;$('tradeReview').open=true;visible=battle.config.days;detail();$('tradeReview').scrollIntoView({block:'start',behavior:'smooth'});};
$('tradeReview').addEventListener('toggle',()=>{if($('tradeReview').open)draw();});
...
function render(){
 ...
 const explanation=arenaExplanation(battle);
 $('headline').textContent='先看結果，再看買賣位置。';$('resultScope').textContent=`這是已完成的 ${c.count} 段、每段 ${c.days} 日對戰。上方修改的設定只影響下一次計算。`;
 for(const key of ['profitTitle','profitText','riskText','nextText'])$(key).textContent=explanation[key];
 $('tradeReview').open=false;
...
function detail(){
 ...
 const buys=s.fills.filter(f=>f.side===1).length,sells=s.fills.filter(f=>f.side===-1).length,name=STRATEGIES.find(r=>r.id===s.id).name;
 $('tradeGuide').textContent=`目前看的是 ${e.name}（${e.symbol}），${e.startDate} 至 ${e.endDate}的「${name}」。${buys?`買進 ${buys} 次、賣出 ${sells} 次。${s.position?'期末仍持有 '+s.position+' 股。':'期末已沒有持股。'}`:'這段沒有買進成交；請看下方是否未出現訊號，或有委託未成交。'}此單一帳戶期末比起始${s.return<0?'減少':'增加'} ${pct(Math.abs(s.return))}。`;
...
$('episode').onchange=()=>{visible=battle.config.days;detail();};$('strategy').onchange=()=>detail();
...
function draw(){
 if(!battle||!$('tradeReview').open)return;const {episode:e,strategy:s}=selection();
 {
