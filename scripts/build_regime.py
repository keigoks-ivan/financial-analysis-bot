...
AXES = [
    ...
    dict(key="credit", name="信用", sub="Credit (HY vs IG)", pos=0.78,
         trail=None, pill="偏暖", pillcls="warm", short="HY spread 偏緊，風險偏好完好",
         reading="HY OAS 2.78%（6/25），落在 10 年區間第 17 百分位（歷史性偏緊）；IG OAS ~80bp。HYG 站上 52 週均線 +1.9%，信用風險偏好完好——本表最明確的 risk-on 軸；惟 spread 已現微幅走闊的早警。",
    ...
    dict(key="equity", name="股票風險偏好", sub="Equity Risk Appetite", pos=0.50,
...
TRIGGERS = [
    ...
    dict(ax="成長 vs 恐慌", off="銅金比轉跌、黃金獨強", on="銅金比上破前高、小型續領先",
         now="銅金比 56 百分位上行（偏暖）"),
    dict(ax="股票偏好", off="VIX 上破 20、Nasdaq 空頭回補失敗", on="Nasdaq 極端空頭軋空、VIX 續低",
...
RATIOS_EDITORIAL = [
    ...
    dict(key="hy_ig", name="高收益／投等", sub="HY / IG credit", pos=0.60,
         num="HYG", den="LQD",
         reading="HY OAS 2.78%（10y 第 17 百分位，偏緊）、HYG 站上均線 — 信用風險偏好存，惟動能百分位低",
