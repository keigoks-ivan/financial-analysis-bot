#!/usr/bin/env python3
"""
QQQ + SMH 長線 × 波動率目標（自適應 σ 版）— 前瞻 OOS 追蹤訊號頁產生器
========================================================================
/long-track-qs-vt/（固定 σ 版）的自適應 σ 姊妹子頁，輸出 adaptive.html。每交易日抓
QQQ、SMH 日線（auto-adjust），逐標的算：
...
同凍結日 2026-07-17），唯套袖 σ 改滾動中位數。研究判定（adaptive_sigma.json）：美股
雜訊級平手（FAIL＝無證據值得替換固定 σ，非更差）。與固定版並跑供 2027-01 複審。

閘門逐位元對齊 run_ext_ltrack_smh.longtrack_weight；套袖對齊 run_adaptive_sigma.
vt_weight_adaptive_median(n_days=756)。回測數字見 BACKTEST 常數（band_exec.json us/
adaptive/band10_round5 主列、live_engine_compare.json 實倉引擎列）。
...
兩 repo，輸出路徑自動解析為 <repo_root>/docs/long-track-qs-vt/adaptive.html。CI 由 fab 執行。

排程：每交易日美股收盤後（套袖 RV20／σ_t 為日頻量，閘門為週頻，故日更）。
...
OUTPUT = DOCS / "long-track-qs-vt" / "adaptive.html"
STATE_JSON = DOCS / "long-track-qs-vt" / "adaptive_state.json"
# 無 email／alert 檔：可行動變化通知由 /long-track-adaptive-vt/ 的既有 email 涵蓋，
# 本頁不另發，避免雙重通知（頁面亦註明此點）。

TICKERS = ["QQQ", "SMH"]
WEIGHTS = {"QQQ": 0.5, "SMH": 0.5}
SIGMA_BASE = {"QQQ": 0.20, "SMH": 0.25}    # 固定 σ 版對照值；本頁套袖不使用（僅卡片顯示）
...
BACKTEST = {
    ...
    "rows": [                              # (label, cagr, mdd, calmar, ui, martin)
        ("自適應中位數 3y・含 10pp＋5% 取整執行層（本頁追蹤）", 11.59, -22.1, 0.524, 7.87, 1.47),
        ("每日照調（理論・對照）", 11.51, -21.3, 0.540, 7.95, 1.45),
        ("固定 σ 正式版・含執行層（對照）", 11.78, -21.9, 0.538, 8.24, 1.43),
        ("純長軌（無套袖）", 12.36, -34.0, 0.363, 9.77, 1.27),
        ("50/50 買進持有", 17.65, -56.9, 0.310, 13.99, 1.26),
        ("實倉引擎對照 50/50 STX50（QQQ+SMH）", 12.91, -21.2, 0.609, 9.53, 1.35),
    ],
    "sigma_ladder": [                      # (SMH σ, Calmar) — QQQ 20% 固定、每日照調
        (25, 0.547), (30, 0.566), (35, 0.552), (40, 0.530),
    ],
...
def _sleeve_from(px: pd.Series, win: int = 20):
    ...
    逐位元對齊 run_adaptive_sigma.vt_weight_adaptive_median（n_days=756）。"""
...
def backtest_section() -> str:
    ...
    for ss, calmar in BACKTEST["sigma_ladder"]:
        ladder += (f'<tr><td>QQQ 20% / SMH {ss}%</td>'
...
兩標的日報酬 50/50 加權合成（等效每日再平衡，如實揭露）。判準基準＝<b>固定 σ 正式版</b>（本頁與固定 σ 版差異只在套袖，閘門完全相同）。<b>主列與固定 σ 對照列皆含 10pp 門檻＋5% 取整執行層</b>（apples-to-apples）；「每日照調」為理論對照。「實倉引擎對照」＝SMH/QQQ STX50（0..1 分數倉＋Supertrend 半倉出場閘門），同權重 50/50、同窗、同成本，<b>非本頁 0/1 閘門機制</b>。含執行層數字自 band_exec.json、實倉引擎自 live_engine_compare.json。</p>
...
<tbody>{rows}</tbody></table>
<div class="takeaway"><b>美股判定＝雜訊級平手（FAIL）</b>：主列（含執行層）vs 固定 σ 正式版（同含執行層）Calmar 0.538 → 0.524、Martin 1.43 → 1.47，
差距在四捨五入級、互有高低——<b>FAIL 的意思是「無證據值得替換固定 σ」，不是「更差」</b>。每日照調理論版 Calmar 0.540，執行層對美股自適應略有折損（0.540 → 0.524）。
與台股（自適應過關）並置＝同一機制、兩市場、兩個相反判定，正是與固定版並跑要檢驗的（見 <a href="/long-track-adaptive-vt/">自適應美台總覽</a>）。</div>
</div>

<div class="card">
<h3>固定 σ 階梯對照（QQQ 20% 固定、SMH σ 掃描・每日照調）</h3>
<p style="font-size:.8rem;color:var(--muted);margin-bottom:.7rem">
手動調高 SMH σ 的增益全集中在 2014 後與 AI 牛、GFC/QE 幾乎零差異——<b>自適應機制以滾動中位數自動取代此手動選擇</b>：波動 regime 高時 σ_t 自動上移（少欠載）、低時下移（多減碼），不必人工選一個固定 σ。此階梯僅供理解「為何要自適應」，不追蹤。</p>
...
def generate_html(sigs: dict, changes: list | None, last_change_date: str | None,
    ...
    H_QSIG = _col(lambda r: r["tickers"]["QQQ"]["sigma_t_pct"])
    H_SSIG = _col(lambda r: r["tickers"]["SMH"]["sigma_t_pct"])
    ...
    H_QEXE = json.dumps(exec_replay["executed"].get("QQQ", []), separators=(",", ":"))
    H_SEXE = json.dumps(exec_replay["executed"].get("SMH", []), separators=(",", ":"))
...
<head>
  ...
  <title>QQQ + SMH 長線 × 波動率目標（自適應 σ 版）｜前瞻 OOS 追蹤 | InvestMQuest Research</title>
  <meta name="description" content="QQQ 50% + SMH 50% · 週線長軌閘門 × 自適應波動率套袖（σ_t＝近 3 年 RV20 中位數）· 前瞻 OOS 紙上追蹤（未採用）">
...
<div class="page-hdr">
  <div class="container">
    <div class="crumb"><a href="/">首頁</a> / QQQ + SMH 長線 × 波動率目標（自適應 σ 版）</div>
    <h1>QQQ + SMH 長線 × 波動率目標（自適應 σ 版）— 前瞻 OOS 追蹤</h1>
    <div class="sub">50% QQQ + 50% SMH · 週線長軌閘門 × 自適應波動率套袖（σ_t＝近 3 年 RV20 中位數，動態）· Long only · 紙上追蹤（未採用）</div>
    <div style="margin-top:.6rem;display:flex;gap:.4rem;flex-wrap:wrap">
      <a href="/long-track-qs-vt/adaptive.html" style="font-size:.82rem;font-weight:600;padding:.35rem .8rem;border-radius:6px;background:var(--text);color:#fff;text-decoration:none">自適應 σ 版（本頁）</a>
      <a href="/long-track-qs-vt/" style="font-size:.82rem;font-weight:600;padding:.35rem .8rem;border:1px solid var(--border);border-radius:6px;color:var(--muted);text-decoration:none">固定 σ 版</a>
...
<div class="oos-banner">
  ...
  <div style="font-size:.86rem">本頁＝<a href="/long-track-qs-vt/">固定 σ 版</a>的<b>自適應對照</b>：唯一差異是套袖 σ 改用<b>近 3 年 RV20 滾動中位數</b>（動態），閘門／執行層／權重／窗／成本全同，<b>同凍結日 {FREEZE_DATE}</b>。
  研究判定＝美股<b>雜訊級平手（FAIL＝無證據值得替換固定 σ，不是更差）</b>；與固定版並跑，供 <b>2027-01 或滿 120 個追蹤交易日</b>複審。
  <b>本頁不另發 email</b>——可行動變化通知由 <a href="/long-track-adaptive-vt/">自適應美台總覽</a> 的既有排程涵蓋（避免雙重通知）。</div>
</div>
...
④ <b>機制與視窗凍結</b>：滾動中位數 3 年（756／252）鎖死，<b>此後不再調整</b>；不掃 N 擇優、不改機制形狀。固定 σ 對照值（QQQ 20%／SMH 25%）僅供卡片顯示，套袖不使用。<br>
...
<p style="font-size:.8rem;color:var(--muted);margin-bottom:.6rem">
回測快照（{BT_NAV['labels'][0]} → {BT_NAV['labels'][-1]}）。淨值線＝<b>自適應中位數 3y（含 10pp 門檻＋5% 取整執行層，與追蹤操作同規則）</b>，由 run_band_exec_ablation 月度 NAV 靜態轉錄；B&amp;H 線不變；<b style="color:var(--amber-text)">琥珀虛線＝實倉引擎（SMH/QQQ STX50，同權重 50/50、同窗、同成本對照）</b>。
本頁系統（0/1 長軌閘門 × 自適應套袖 × 執行層）與實倉引擎（E3 分數倉＋Supertrend 半倉出場閘門，0..1）是不同機制；對照僅供「同一筆錢若改放實倉引擎規則」參考，<b>非替代關係</b>。固定 σ 版深度分析見 <a href="/backtest/vol_targeting/">波動率目標回測</a>。</p>
<div class="grid2">
...
<footer class="imq-foot">
  <div>&copy; {datetime.now().year} InvestMQuest Research · QQQ+SMH 長線 × 波動率目標（自適應 σ 版・前瞻 OOS 追蹤・未採用）</div>
...
new Chart(document.getElementById('chart-weights'),{{
  ...
  data:{{labels:W_LAB,datasets:[
    ...
    {{label:'{TICKERS[0]} 執行',data:{H_QEXE},borderColor:BLUE,borderWidth:2,stepped:true,pointRadius:0,pointHoverRadius:3,segment:{{borderDash:segDash}}}},
    {{label:'{TICKERS[1]} 執行',data:{H_SEXE},borderColor:AMBER,borderWidth:2,stepped:true,pointRadius:0,pointHoverRadius:3,segment:{{borderDash:segDash}}}},
    {{label:'合成目標（理論）',data:{H_COMB},borderColor:'rgba(22,163,74,0.40)',borderWidth:1,pointRadius:0,pointHoverRadius:2,tension:0.15}},
    {{label:'{TICKERS[0]} 目標',data:{H_QQQ},borderColor:'rgba(21,101,192,0.35)',borderWidth:1,pointRadius:0,pointHoverRadius:2,tension:0.15}},
    {{label:'{TICKERS[1]} 目標',data:{H_SMH},borderColor:'rgba(217,119,6,0.35)',borderWidth:1,pointRadius:0,pointHoverRadius:2,tension:0.15}}
  ]}},
...
// ---- RV20 vs dynamic sigma_t (adaptive median) ----
new Chart(document.getElementById('chart-rv'),{{
  ...
  data:{{labels:W_LAB,datasets:[
    ...
    {{label:'{TICKERS[0]} σ_t（動態）',data:{H_QSIG},borderColor:'rgba(21,101,192,0.55)',borderWidth:1.4,borderDash:[6,4],pointRadius:0,pointHoverRadius:3,tension:0.15}},
    {{label:'{TICKERS[1]} σ_t（動態）',data:{H_SSIG},borderColor:'rgba(217,119,6,0.55)',borderWidth:1.4,borderDash:[6,4],pointRadius:0,pointHoverRadius:3,tension:0.15}}
  ]}},
  options:{{responsive:true,maintainAspectRatio:false,interaction:{{mode:'index',intersect:false}},
...
def main():
    ...
    state_json = {
        ...
        "status": "forward OOS paper tracking, not adopted (adaptive-σ sibling of /long-track-qs-vt/; US verdict: noise-level tie)",
