#!/usr/bin/env python3
"""Build per-category drilldown pages from docs/id/index.html.

Usage:
  python3 scripts/build_id_category_pages.py

Reads docs/id/index.html (single-page source of truth) and writes
docs/id/cat-{mega}.html for each of the 15 mega-categories.
...
industry-analyst skill can still insert new cards via subgroup-anchor markers.
...
TAXONOMY = [
    {
        ...
        "chinese": "半導體 / AI 基建",
        "icon": "🔷",
        "hint": "AI Accelerator / 封裝 / 記憶體 / 製程 / 互連",
    },
    {
        "mega": "bio",
        "data_cat": "bio",
    ...
    },
    {
        ...
        "chinese": "雲端 / SaaS / 軟體",
        "icon": "☁️",
        "hint": "資安 / Agentic AI / Token / 媒體 SaaS",
    },
    {
        "mega": "energy",
        "data_cat": "energy",
    ...
    },
    {
        ...
        "chinese": "消費 / 零售 / 精品",
        "icon": "🛍️",
        "hint": "服飾 / 餐飲 / 奢侈品 / E-commerce",
    },
    {
        "mega": "finance",
        "data_cat": "finance",
        "chinese": "金融 / 保險 / FinTech",
        "icon": "🏦",
        "hint": "支付 / 銀行 / Stablecoin / 財富",
    },
    {
        "mega": "industrial",
        "data_cat": "industrial",
    ...
    },
    {
        "mega": "housing",
        "data_cat": "housing",
        "chinese": "住房 / 房貸 / Builders",
        "icon": "🏠",
        "hint": "美國 builders / 海外房產 / 房貸",
    },
    {
        "mega": "transport",
        "data_cat": "transport",
...
def extract_head(html: str) -> str:
    """Extract everything between <head> and </head> (exclusive)."""
...
def extract_nav_block(html: str) -> str:
    """Extract the site nav: <style>.imq-nav-root...</style> + <header> + <script> for dropdown."""
    # Find from the imq-nav-root style through to the dropdown script
    m = re.search(
...
def extract_cat_jump_links(html: str) -> list[dict]:
    """Extract individual <a> elements from the cat-jump bar.

    Returns list of dicts with keys: href_original, text (full inner HTML).
    """
    # Find the cat-jump div
...
def extract_filter_bar(html: str) -> str:
    """Extract the toolbar-inner content (search + type filter buttons)."""
    m = re.search(r'<div class="toolbar-inner">(.*?)</div>\s*<div class="cat-jump">', html, re.DOTALL)
    if not m:
        return ""
...
def extract_filter_js(html: str) -> str:
    """Extract the filter JS block (from var filterState through applyFilters() call)."""
    # Grab from // Filters & search through the closing applyFilters(); call
    m = re.search(
...
def extract_category_block(html: str, mega: str) -> tuple[str, int]:
    ...
    For 'staples' we need special handling since it uses data-cat="consumer" legacy.
    Returns (block_html, article_count).
    """
    # staples has data-cat="consumer" and id="cat-consumer" (second occurrence)
    # We identify it by the 必選消費 title pattern
    if mega == "staples":
        # Find the second div with data-cat="consumer" (staples is legacy)
        # It has 必選消費 as h2 text
        pattern = r'(<div class="category[^"]*"[^>]*data-cat="consumer"[^>]*>)'
        matches = list(re.finditer(pattern, html))
        # Find the one with 必選消費 in nearby content
        block_start = None
        for m in matches:
            # Check if 必選消費 appears within ~200 chars after opening tag
            after = html[m.start():m.start() + 300]
            if "必選消費" in after:
                block_start = m.start()
                break
        if block_start is None:
            # Try alternate: look for cat-icon 🛒
            m2 = re.search(r'<div class="category[^>]*>(?=\s*<div class="cat-head">.*?🛒)', html, re.DOTALL)
            if m2:
                block_start = m2.start()
            else:
                print(f"  WARNING: staples category block not found; generating empty page", file=sys.stderr)
                return "", 0
    else:
        # Standard: find by id="cat-{mega}"
        pattern = rf'<div class="category[^"]*"\s+id="cat-{re.escape(mega)}"[^>]*>'
        m = re.search(pattern, html)
        if not m:
            # Try without class constraints
    ...
    # Now we need to find the matching closing </div> using div nesting
    # Walk from block_start to find balanced closing
    block = _extract_balanced_div(html, block_start)
    if block is None:
        print(f"  WARNING: could not extract balanced div for mega={mega!r}", file=sys.stderr)
        return "", 0
...
def _extract_balanced_div(html: str, start: int) -> str | None:
    """Extract a balanced <div>...</div> block starting at position start."""
    # Find the first <div at start
...
def build_cat_jump_bar(links: list[dict], active_mega: str) -> str:
    """Build cat-jump bar with href="#cat-X" → cat-X.html and active class on current."""
    items = []
    # Add "↩ 全部" link first
    items.append('<a href="/id/">↩ 全部</a>')
    for link in links:
        href = link["href_original"]
        text = link["text"]
        # Convert #cat-X to cat-X.html
        anchor_m = re.match(r"#cat-(.+)", href)
        if anchor_m:
            cat_slug = anchor_m.group(1)
            new_href = f"cat-{cat_slug}.html"
            # Determine if this link corresponds to active_mega
            is_active = (cat_slug == active_mega)
            # Special case: staples uses href="#cat-consumer" for the staples link
            # We need to distinguish consumer from staples by text content
...
def force_open_category(block: str, mega: str) -> str:
    ...
    if mega == "staples":
        # Fix the id and data-cat to be staples-specific
        block = re.sub(
            r'(<div class="category[^"]*"\s+)(id="cat-consumer")(\s+data-cat="consumer")',
            r'\1id="cat-staples"\3',
            block,
            count=1,
        )
        # Also fix data-cat to staples
        block = re.sub(
            r'id="cat-staples"(\s+data-cat="consumer")',
            r'id="cat-staples"\1'.replace('\1', ' data-cat="staples"'),
            block,
            count=1,
        )
        # Simpler approach: just replace the first occurrence of the opening tag
        block = re.sub(
    ...
    if 'class="category open"' not in block[:80] and 'class="category"' in block[:80]:
        ...
        # Replace whatever class is there
        block = re.sub(r'class="category[^"]*"', 'class="category open"', block, count=1)
    # Remove cursor:pointer from cat-head (it's a navigation page, not collapsible)
    # We do this via inline style override; the JS won't fire since we remove the listener
    return block
...
def generate_cat_page(
    ...
    adapted_head = re.sub(
        r"<title>.*?</title>",
        f"<title>{chinese} — 產業深度 ID 索引</title>",
        head_content,
        flags=re.DOTALL,
    )
    # Add cat-page-specific CSS for breadcrumb nav + active link style + footer
    cat_css = """
/* Cat page extras */
...
/* Force cat-head non-clickable on standalone pages */
...
"""
    ...
    # Build cat-jump bar with active class on current mega
    cat_jump_bar = build_cat_jump_bar(cat_jump_links, mega)

    # Force open the category block
    open_block = force_open_category(category_block, mega)

    # Generate empty-state if no articles
    empty_note = ""
    if article_count == 0:
        empty_note = f"""
  <div style="text-align:center;padding:40px 20px;color:#94A3B8;font-size:14px;">
    ...
    <p>此大類尚無 ID 報告</p>
    <p style="font-size:12px;margin-top:4px">對 Claude Code 說「研究 {chinese} 產業」即可新增</p>
  </div>"""

    # Filter JS needs visible-count and filter-status elements to exist
    # Adapt it so missing elements don't throw errors (they exist in our template)
...
<div class="cat-page-footer">
  本頁由 scripts/build_id_category_pages.py 從 docs/id/index.html 自動生成 · 最後生成：{timestamp}
</div>
...
// Open ID report links in new tab
...
def main():
    ...
    for meta in TAXONOMY:
        ...
        if not block:
            # Generate a minimal placeholder block
  ...
  <div class="cat-body">
    <div class="topic-grid">
        ...
        print(f"  {mega:12s} → {article_count:3d} articles → {out_path.name}", file=sys.stderr)
