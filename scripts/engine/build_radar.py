...
<h3>三閘主榜 <span class="cnt">{len(board)} 檔全過三閘（顯示 top 30，按上修幅度）</span></h3>
...
｜市值門檻管主榜與席位資格（本期另有 {payload.get('grp_subfloor_n', 0)} 檔過三閘但未達市值，留在形狀欄）。
...
<b>主榜＝三閘評分（GRP）</b>（高成長 × EPS 上修 × 位置適合，按上修幅度排）——這是選股準則本身；
