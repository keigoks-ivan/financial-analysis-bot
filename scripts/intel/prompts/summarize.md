...
- **白話關卡**：`summary_zh`／`why_zh`／`new_title_zh` 是寫給投資人看的外資券商式白話句，
  不是內部工程備忘——不得出現卡片輸入資料的欄位名或站內模組代號本身（例：`source_id`、
  `tier`、`corroboration`、`site_snapshot` 這類 JSON 欄位名絕不能原樣出現在你寫的中文句子
  裡）。若原文本身用了金融專業術語（如利差、存續期、期限溢價），字數許可時比照外資報告
  習慣帶一句簡短白話；`summary_zh` 只有 40–90 字，字數優先給事實，不要為了硬塞術語解釋
  擠掉正文內容。
