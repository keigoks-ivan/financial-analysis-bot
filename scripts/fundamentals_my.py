"""
馬股 (Bursa Malaysia) Fundamentals fetcher
產出: docs/screener/my_fundamentals.json (mirror US/TW/JP schema)
...
註: 馬股中小型股 yfinance 分析師 estimate 覆蓋偏稀疏,欄位缺值會 graceful 降為 None (頁面顯示 —)。
"""
...
OUTPUT_PATH = SCREENER_DIR / 'my_fundamentals.json'

# Universe from screener_my (避免重複維護)
sys.path.insert(0, str(ROOT / 'scripts'))
from screener_my import KLCI_30, MID_70
...
def calc_roic(t, info=None):
    ...
    try:
        ...
        if op_inc is None or pretax is None or pretax <= 0:
            ...
            eff_tax = 0.24  # MY corporate tax 24%
        else:
            eff_tax = (tax / pretax) if (tax is not None) else 0.24
...
def fetch_my_fundamentals(ticker: str) -> dict:
...
def main():
    ...
    print(f"=== MY Fundamentals fetcher: {today} ===\n")

    # Build universe — KLCI30 first so dedup keeps KLCI30 as the canonical entry
    universe = {}
    for d in (KLCI_30, MID_70):
        for t in d:
    ...
    for i, ticker in enumerate(tickers, 1):
        f = fetch_my_fundamentals(ticker)
        data[ticker] = f
        if i % 25 == 0 or i == len(tickers):
            elapsed = time.time() - t0
            print(f"  [{i:>3}/{len(tickers)}] {ticker:>10s} eps={f['eps_cagr']} rev={f['rev_cagr']} roic={f['roic']} fcf={f['fcf_margin']}  ({elapsed:.0f}s)")
