#!/usr/bin/env python3
"""W52 × 自適應波動率（美股 QQQ+SMH ＋ 台股 0050+2330）— 前瞻 OOS 追蹤主系統產生器
================================================================================
2026-07-17 起唯一顯示的前瞻 OOS 追蹤主系統（用戶傾向採用，實單前須經回撤檢驗，見頁尾關卡）。
研究代號＝變體 C（見 docs/Adaptive_Sigma_Spec.md）。舊追蹤家族（固定 σ／自適應 A）已歸檔凍結。
...
ALERT_FILE = DOCS.parent / "lt_w52a_alert.txt"
...
<h3>{mkt['short']} 頭對頭 — 本頁 W52 × 自適應波動率 vs 實倉引擎 {h2h['engine']}（同權重、同窗）</h3>
...
<tr><td>實倉引擎 {h2h['engine']}（分數倉）</td><td class="num">{ee_c:.3f}</td><td class="num">{ee_m:.1f}%</td><td class="num">{ee_g:.2f}%</td></tr>
</tbody></table>
<div class="takeaway">兩者<b>長期平均曝險相近（皆約七成）</b>、分期 MDD 本頁多期較淺；<b>弱點期錯開</b>——本頁怕{h2h['c_weak']}、{h2h['engine']} 怕{h2h['e_weak']}。這是<b>不同機制、互補而非替代</b>：本頁把「水下體驗」擺第一（門檻式減碼＋快速再進場），實倉引擎把「趨勢報酬」擺第一。</div>
</div>"""
    return f"""<div class="card">
...
數字轉錄自 results/vol_targeting/w52_adaptive.json、實倉引擎自 live_engine_compare.json。</p>
...
<head>
  ...
  <title>W52 × 自適應波動率（美+台）｜前瞻 OOS 追蹤主系統 | InvestMQuest Research</title>
  <meta name="description" content="美股 QQQ+SMH ＋ 台股 0050+2330 · 週線 W52 單線閘門 × 自適應 σ 波動率目標 × 執行層 · 前瞻 OOS 追蹤主系統（傾向採用、實單前須經回撤檢驗）">
...
<div class="page-hdr">
  <div class="container">
    <div class="crumb"><a href="/">首頁</a> / W52 × 自適應波動率（美+台）</div>
    <h1>W52 × 自適應波動率 — 前瞻 OOS 追蹤主系統</h1>
    <div class="sub">美股 QQQ+SMH ＋ 台股 0050+2330 · 週線 W52 單線閘門 × 自適應 σ 波動率目標 × 執行層 · Long only · 前瞻 OOS 追蹤主系統（用戶傾向採用；實單前須經回撤檢驗，見下方關卡）· <span style="opacity:.7">研究代號 變體 C，見 Adaptive_Sigma_Spec</span></div>
    <div style="margin-top:.6rem;display:flex;gap:.4rem;flex-wrap:wrap">
      <a href="/long-track-w52-adaptive/" style="font-size:.82rem;font-weight:600;padding:.35rem .8rem;border-radius:6px;background:var(--text);color:#fff;text-decoration:none">W52 × 自適應波動率（本頁）</a>
      <a href="/long-track-w52-adaptive/leverage.html" style="font-size:.82rem;font-weight:600;padding:.35rem .8rem;border:1px solid var(--border);border-radius:6px;color:var(--muted);text-decoration:none">150% 槓桿影子</a>
      ...
      <a href="/long-track-tw/" style="font-size:.82rem;font-weight:600;padding:.35rem .8rem;border:1px solid var(--border);border-radius:6px;color:var(--muted);text-decoration:none">台股長線（實倉）</a>
      <a href="/long-track-qs-vt/" style="font-size:.82rem;font-weight:600;padding:.35rem .8rem;border:1px solid var(--border);border-radius:6px;color:var(--muted);text-decoration:none">歸檔：舊追蹤家族</a>
    </div>
  </div>
</div>
<div class="container">
...
<div class="oos-banner">
  <span class="tag-loud">前瞻 OOS 追蹤主系統・用戶傾向採用・實單前須經回撤檢驗</span>
  <div style="font-size:.86rem">本頁是<b>{FREEZE_DATE} 起唯一顯示的前瞻 OOS 追蹤主系統</b>：<b>W52 單線閘門 × 自適應 σ 波動率目標 × 10pp＋5% 取整執行層</b>（美台四腿、各市場 50/50 獨立組合）。設計取向＝<b>水下體驗優先</b>（門檻式減碼＋跌破 W52 快速出場／收復快速再進場），與用戶個性相符，故列為傾向採用中。
  每交易日台股／美股收盤後更新，<b>可行動變化以 email 提醒</b>（任一腿閘門翻轉，或今日目標與現持執行層差 ≥ 10pp）。舊追蹤家族（固定 σ／自適應 A）已於本日<b>歸檔凍結</b>，其回測證據保留於<a href="/backtest/vol_targeting/adaptive.html">美股</a>／<a href="/backtest/vol_targeting/tw.html">台股</a>變體實驗室。
  <br><br><b>誠實定位（必讀）：本頁是 2026-07-17 樣本內迭代的產物</b>（規則同日凍結、此後不調）——把舊系統 score5 閘門換成快閘門 W52 是「看了樣本內診斷後」的選擇，非先驗設計；<b>快閘門的慢熊保護較弱</b>（見下方誠實揭露與 2330 長窗）。所以雖回測全窗較優，仍<b>設實單前複審關卡</b>（見頁尾），不無條件上實單。</div>
</div>
...
<h3 style="color:var(--blue-text)">實單前複審關卡（用戶傾向採用中）</h3>
<div class="rule-list" style="font-size:.82rem">
用戶傾向以本系統作為實單（水下體驗優先，與其個性相符）。<b>正式上實單前須通過以下實錄檢驗，二擇一：</b><br>
• <b>滿 60 個追蹤交易日</b>的前瞻 OOS 實錄（非回放），確認實際持股率／事件節奏與回測分布一致；<b>或</b><br>
• <b>經歷一次 ≥ 10% 組合回撤事件</b>的實錄檢驗，親眼確認 W52 快閘門＋自適應套袖在真實下殺中的水下體驗符合預期（尤其慢熊誘多情境）。<br>
通過後，<b>按 charter 寫採用規格</b>（部位、資金比例、與現有 STX50／E3 實倉的關係），才由研究轉實盤。<b>在此之前本頁為前瞻 OOS 追蹤，不構成倉位建議。</b>
</div>
</div>

</div>
<footer class="imq-foot">
  <div>&copy; {datetime.now().year} InvestMQuest Research · W52 × 自適應波動率（美+台，前瞻 OOS 追蹤主系統・傾向採用未定案）</div>
...
def main():
    ...
    if changes:
        ...
        lines = [f"W52 × 自適應波動率（美+台）— 可行動變化（數據截至 {data_date}）", ""]
        for c in changes:
        ...
        lines += ["", f"美股組合曝險 {exp['us']:.0f}% · 台股組合曝險 {exp['tw']:.0f}% · 下一個交易日調整至上列目標。", "",
                  "W52 × 自適應波動率 前瞻 OOS 追蹤主系統（傾向採用、實單前須經回撤檢驗）。詳細： https://research.investmquest.com/long-track-w52-adaptive/"]
    ...
    state_json = {
        ...
        "mechanism": "W52 single-line gate x adaptive median σ_t rolling(756,252) x exec layer 10pp+5%round",
        "status": "forward OOS main tracking system (user leaning to adopt; pre-live review gate: 60 tracking days OR one >=10% drawdown event)",
