...
def build_price_cache(skip_fetch):
    ...
    if not skip_fetch:
        ...
        # 擋——2026-09-23 那次 SPY/QQQ/IWM/AGG 全部卡 09-21，同批次抓的 statlab
        # ^VIX 卻抓到 09-22/09-23），舊邏輯只要 got 非空就視為成功、不再重試，
        # 會把卡住的舊資料直接寫入快取。對「這次沒有比既有快取新」的既有 ticker
        # 做單檔重抓，最多 2 次，救回批次抓漏但單檔可抓到的情況。
        for t in existing:
