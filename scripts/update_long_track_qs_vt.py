...
兩標的日報酬 50/50 加權合成（等效每日再平衡，如實揭露）。判準基準＝<b>純長軌組合</b>（非買進持有）。<b>主列已含 10pp 門檻＋5% 取整執行層</b>（與本頁追蹤操作同規則）；「每日照調」為理論對照。</p>
...
淨值線＝<b>正式追蹤版（含 10pp 門檻＋5% 取整執行層，與追蹤操作同規則）</b>；B&amp;H 線不變。深度分析（純長軌對照、σ 敏感度、分期、年報酬）請點連結過去。</p>
<div class="grid2">
