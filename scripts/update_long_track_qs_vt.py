...
BACKTEST = {
    ...
    "rows": [                              # (label, cagr, mdd, calmar, ui, martin)
        ("正式追蹤版 σ Q20/S25・含 10pp＋5% 取整執行層（本頁追蹤）", 11.78, -21.9, 0.538, 8.24, 1.43),
        ("每日照調（理論・對照）", 11.90, -21.8, 0.547, 8.21, 1.45),
...
BT_NAV = {
    ...
    "combo": [1.0,1.0019,1.0043,1.0066,1.009,1.0116,1.0141,1.0173,1.0202,1.0233,1.0266,1.0299,1.0238,1.0147,1.0275,1.0285,0.9949,0.999,1.0029,1.0075,1.0159,1.0485,1.0687,1.0608,1.0742,1.0675,1.0729,1.1137,1.1329,1.1563,1.1514,1.1695,1.2065,1.211,1.1662,1.1679,1.1392,1.1411,1.1423,1.1435,1.1581,1.1249,1.1265,1.1154,1.1164,1.1171,1.1172,1.1172,1.1173,1.1176,1.1178,1.1179,1.118,1.1182,1.1184,1.1185,1.1186,1.1187,1.1187,1.1188,1.1188,1.1189,1.1527,1.1659,1.1309,1.1075,1.1001,1.071,1.0925,1.1269,1.1263,1.153,1.1695,1.1881,1.1876,1.2409,1.22,1.1768,1.1511,1.082,1.0694,1.0738,1.0501,1.0284,1.0635,1.0973,1.1249,1.1187,1.0789,1.099,1.1048,1.1333,1.1277,1.0978,1.0926,1.0899,1.11,1.1252,1.1488,1.1882,1.2294,1.2051,1.2571,1.2314,1.3066,1.3603,1.3845,1.4372,1.4021,1.4806,1.4928,1.4751,1.5359,1.612,1.6098,1.6985,1.6825,1.6916,1.7958,1.7711,1.7221,1.8538,1.8053,1.8256,1.9173,1.8087,1.8286,1.715,1.6897,1.7266,1.7428,1.6887,1.5626,1.563,1.6013,1.5417,1.5727,1.5543,1.6722,1.7157,1.7752,1.7483,1.7875,1.8118,1.8895,1.9516,2.0118,2.0366,2.1525,2.0786,2.1672,2.2221,2.2782,2.4264,2.4321,2.4263,2.6305,2.5961,2.4814,2.4141,2.5798,2.5395,2.6157,2.729,2.6944,2.4786,2.4571,2.4614,2.4662,2.4784,2.5635,2.7541,2.4295,2.4575,2.5565,2.5061,2.5575,2.6948,2.8016,2.9568,2.9658,2.8286,2.5577,2.5802,2.673,2.8302,3.0366,3.2861,3.1721,3.1121,3.5173,3.683,3.7527,3.8536,3.8649,3.9236,3.946,4.1628,4.2296,4.376,4.1461,4.4357,4.7053,4.7204,4.29,4.2378,4.1207,4.0236,4.0267,4.0314,4.0383,4.0477,4.058,4.0703,4.0842,4.0985,4.1132,4.0934,4.2633,4.1549,4.4675,4.6991,4.908,4.8125,4.5354,4.4049,4.9499,5.3033,5.5035,5.9944,6.1868,5.9154,6.3905,6.8713,6.6495,6.5826,6.734,6.6606,6.8466,6.8633,6.9767,6.731,6.4642,6.4868,6.6329,7.1164,7.3306,7.3859,8.0408,8.6282,8.4642,8.5268,9.0132,8.8866,8.337,9.7348,10.9448,11.2049,10.9609],
...
兩標的日報酬 50/50 加權合成（等效每日再平衡，如實揭露）。判準基準＝<b>純長軌組合</b>（非買進持有）。<b>主列已含 10pp 門檻＋5% 取整執行層</b>（與本頁追蹤操作同規則）；「每日照調」為理論對照。「實倉引擎對照」＝SMH/QQQ STX50（0..1 分數倉＋Supertrend 半倉出場閘門），同權重 50/50、同窗、同成本，<b>非本頁 0/1 閘門機制</b>。</p>
...
<tbody>{rows}</tbody></table>
<div class="takeaway">套袖把純長軌的 Calmar 0.363 → <b>0.538</b>、MDD −34.0% → <b>−21.9%</b>；主數字已含執行層（10pp 門檻＋5% 取整），
相對每日照調理論版（Calmar 0.547）近乎免費——執行層省下大量微調換來的偏差可忽略。
對 50/50 買進持有則是「風險調整支配、CAGR 讓步」（B&amp;H 17.65%／−56.9%）。<b>唯一系統性代價集中在 AI 牛（高波急彈仍減碼、漏接反彈）。</b></div>
</div>
...
<div style="font-size:.75rem;color:var(--muted);margin-top:.5rem">套袖邊際貢獻的分期分解（每日照調基準）：五期 MDD 全數改善（+0.0～+14.5pp）；CAGR 代價集中在 AI 牛（−7.1pp）。</div>
</div>"""
...
淨值線＝<b>正式追蹤版（含 10pp 門檻＋5% 取整執行層，與追蹤操作同規則）</b>；B&amp;H 線不變；<b style="color:var(--amber-text)">琥珀虛線＝實倉引擎（SMH/QQQ STX50，同權重 50/50、同窗、同成本對照）</b>。
本頁系統（0/1 長軌閘門 × 波動率套袖 × 執行層）與實倉引擎（E3 分數倉＋Supertrend 半倉出場閘門，0..1）是不同機制；對照僅供「同一筆錢若改放實倉引擎規則」參考，<b>非替代關係</b>。深度分析（純長軌對照、σ 敏感度、分期、年報酬）請點連結過去。</p>
<div class="grid2">
