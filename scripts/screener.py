...
import tech_core
# find_local_highs / VCP_MIN_BARS / calc_vcp moved to scripts/vcp_core.py on
# 2026-09-18 (VCP depth 1) so build_dd_screener.py can reuse calc_vcp() without
# importing this module (screener.py has an import-time side effect two lines
# below — writing docs/screener/history/ just by being imported — that isn't
# safe to trigger from an unrelated pipeline). Re-imported here so
# `from screener import find_local_highs, calc_vcp` (screener_tw.py/
# screener_jp.py/screener_my.py) keeps working unchanged.
from vcp_core import find_local_highs, VCP_MIN_BARS, calc_vcp, _vcp_empty_result
