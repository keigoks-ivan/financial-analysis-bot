...
## 規則預審計 joiner（rule_audit.py，2026-07-17 起）

`rule_audit.py` 把 `rule_ledger.md` 的審計方法（「grep 報告中的規則觸發標記 ×
settlement forward return」）機械化：解析 ledger 第一張表全部判斷類規則，對每條在
`docs/dd/*.html`（v13/v14 有 dd-meta 者）偵測**觸發**，接上 `settlement.json` 的
forward return，寫 `knowledge/rule_audit.json`（衍生物，gitignore；staleness 比照 q.py
自動先重跑 build_knowledge / settle_outcomes）。是 2026-10 校準要**直接貼**的證據素材，
不自動改 ledger 審計欄。

```bash
python knowledge/rule_audit.py            # 重算 rule_audit.json
python knowledge/rule_audit.py --report   # 印 markdown 審計預覽表（貼進校準）
```

覆蓋三級（誠實優先，寧可標 manual 也不弱 pattern 亂命中——誤 join 比漏 join 毒）：
`mechanical`＝dd-meta 欄位確定性判定（估值燈基線 / Hard Veto）；`partial`＝欄位代理或
本文 fired-state regex（命中為觸發子集，非全集）；`manual`＝觸發標記不在 DD 掃描範圍
（ID/macro/detective/GRP/supply-chain/synthesis 等其他檔族或引擎碼），或屬 always-on
鷹架、交人判。detector 規格與每條 manual 理由見 `rule_audit.py` 的 `SPECS`。
...
**vault/notes 的同步（2026-07-11 起）**：repo 是 PUBLIC，手寫筆記**不進 git**——
`vault/notes` 是指向 iCloud 的 symlink（`~/Library/Mobile Documents/com~apple~CloudDocs/BrainVaultNotes`），
兩台 Mac 由 iCloud 自動同步。另一台 Mac 初次設置：
```bash
mkdir -p "$HOME/Library/Mobile Documents/com~apple~CloudDocs/BrainVaultNotes"
rm -rf knowledge/vault/notes
ln -s "$HOME/Library/Mobile Documents/com~apple~CloudDocs/BrainVaultNotes" knowledge/vault/notes
```
注意：在 Finder 對 BrainVaultNotes 右鍵「永遠保留下載項目」，避免 iCloud 最佳化把檔案搬離本機害入腦讀不到。
...
捷徑 C「剪貼簿進大腦」（FB／X 場景——app 不給乾淨分享文字時，複製是實際動線）：
1. 新增捷徑 → 加動作「取得剪貼簿」。
2. （推薦）加動作「要求輸入」→ 提示「為什麼存？可留白」——你的一句話才是知識點，原文只是證據。
3. 加動作「文字」→ 內容放變數：`提供的輸入` 換行 `剪貼簿`。
4. 加動作「儲存檔案」→ iCloud Drive → `BrainInbox/` → 關「詢問儲存位置」→ 檔名 `剪貼-〔目前日期〕.txt`（格式 `yyyy-MM-dd-HHmmss`）。
5. 命名「剪貼簿進大腦」；設定 → 輔助使用 → 觸控 → **輕點背面** → 點兩下 → 綁此捷徑。
用法：FB 長按貼文複製／X 長按內文拷貝 → 敲手機背面兩下 → （可選）打一句為什麼 → 完事，回 Mac 自動入腦。
