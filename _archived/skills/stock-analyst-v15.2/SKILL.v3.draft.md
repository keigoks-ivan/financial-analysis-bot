...
4. **散文 agent：只讀 judgment.json 鋪陳**（另 spawn，sonnet，固定，不受 `--judgment-model` 影響）：
   ...
   - **產表、算預算、產數字白名單**：`gen_dd_tables.py {JUDGMENT} --out .dd_build/{T}_{D}.tables --scenario-html .dd_build/{T}_{D}.tables/e11.html --scenario-meta .dd_build/{T}_{D}.scenario_meta.json`（dashboard／E2／E3／E5-E10／E11／E12／dd-meta／appA 表；audit 表視 `decision_out.audit_rows` 非空）→ `dd_prose_budget.py {JUDGMENT} --tables .dd_build/{T}_{D}.tables` 得各段散文目標 bytes（表格計入分子）→ `validate_prose.py --dump-numbers {JUDGMENT} --evidence {EVIDENCE} --out .dd_build/{T}_{D}.numbers_whitelist.txt`（2026-09-04 新增，CRDO 教訓：散文動筆前先讀這份逐字數字清單、逐字複製含 −/%/$ 符號，避免符號漏帶多跑一輪）→ 依表逐段**一次 `Write`** `.dd_build/{T}_{D}.prose/{sid}.html`（**禁 `Edit`，任何 Edit 視為無效輸出**，render-rules §0 一次寫條款）。
   - **組裝與驗證（改呼叫 `dd_gates.sh`，2026-09-04 起）**：`bash scripts/dd_gates.sh {T} {D} {OUT_HTML}`——內部依序跑 `render_dd.py --assemble` 組裝→六支驗證（`validate_prose.py`正文數字⊆判斷物數字集合／`dd_sections.py bytes`／`dd_sections.py leaks`／`qc.py`／`validate_dd_meta.py --report`／`verify_dd_math.py`），彙總輸出、任一真正擋下的閘 exit 1。**只准呼叫這一支**，不拆開個別跑。`{OUT_HTML}`＝`--dry-run` 時 `.dd_build/DD_{T}_{D}.v16dryrun.html`，否則 `docs/dd/DD_{T}_{D}.html`。超標或命中**只重寫該段 prose 檔**（整檔 Write，非 Edit）、重跑 `dd_gates.sh`；每段最多重寫一次，不動 judgment.json。**驗證輪次 ≤2**（CRDO 拆開跑吃到 3 輪的教訓）。
   - **交稿前自查⑧＋QC-54**（取代原獨立 critic 的呈現層部分）：散文一致性（有沒有與 judgment.json 矛盾／出現判斷物沒有的數字）＋白話開場，逐條 🟢/🟡/🔴 作答，🔴 先重寫該段一次再交稿；自查表為回報必要欄位。
   - **禁**：WebSearch/WebFetch、Read 任何 `docs/dd/`、重讀自己寫過的 prose/tables 檔、改 `judgment.json` 任何欄位、拆開個別呼叫 `dd_gates.sh` 內的六支腳本。
